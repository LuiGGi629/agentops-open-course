...
url: "/8-community/8-4-documentation/"
---
...
Run `mise run serve` now and keep the preview open at http://localhost:8003 while you read. Every rule below is about something you can see there. {{% /admonition %}}
...
{{% collapsible note "Deeper: why the checker parses instead of pattern-matching" %}}
...
You can see this working in the deepened chapters: [2.1. First Agent]({{< relref "/2. Agents/2.1. First Agent.md" >}}) embeds the `root-agent` region from `composition.py`, and [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md" >}}) embeds the `get-runbook` region from `memory.py`.
...
{{% collapsible note "Deeper: the publication gate, for whoever hosts the site" %}}
...
1. Repository source links in the built site open without maintainer credentials; edit links may ask a contributor to sign in.
...
Return to [8. Community]({{< relref "/8. Community/_index.md" >}}) and pick your next maintenance question when both gates pass on a page you edited yourself.
