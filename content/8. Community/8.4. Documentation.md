...
This checkout deliberately does not publish. `build:docs` writes `site/` and shows that the current checkout renders locally; there is no Pages workflow, CNAME, DNS record, or deployment contract here. Publication, repository identity, URL redirects, DNS, and TLS stay owner-gated, so a green local build says nothing about a public site; only a request to a live hostname returning these pages would.
