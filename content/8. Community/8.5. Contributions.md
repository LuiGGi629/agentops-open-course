...
`cd evals && mise run eval:validate` is offline and belongs to CI. `eval:dev`, `eval`, `eval:workflow`, `eval:a2a`, `eval:cost`, `eval:ground`, and judge calibration start the Go agent and call a configured model, so they are scheduled or operator-triggered evidence. The canonical `eval` and `eval:a2a` tasks also require the repository release policy to be approved; `eval:dev` is the learner path while calibration remains open.
...
The Go suite reports measured coverage, but no percentage is enforced because the owner has not selected one. Do not turn a report into policy in a contribution.
