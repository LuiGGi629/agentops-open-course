...
Behavior changes also require model-backed evaluation against an explicitly recorded model. The `results.json` that `mise run eval` writes must name the candidate commit and the representative evalset digest, clear the minimum pass rate the command asked for, and pass every required case. A judged run should be accompanied by a current calibration artifact, which reports agreement rather than granting a pass.
