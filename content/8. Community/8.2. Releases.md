...
1. Promotes both source digests into single-source version indexes only after both publication jobs succeed; failure reconciliation rediscovers current registry state and removes only indexes that still prove exact ownership.
...
1. **Python 3.13**: `requires-python` in the pyprojects and both image base pins. {{% /collapsible %}}
