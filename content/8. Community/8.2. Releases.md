...
- The container image (`agent`) published for that version in GHCR, GitHub's container registry.
...
1. **A publication release** additionally requires the anonymous repository/site/source-link gate from [8.4. Documentation]({{< relref "/8. Community/8.4. Documentation.md" >}}).
...
- **buildx** — Docker's workflow-pinned builder, used here to build the image without pushing it.
- **SBOM** — a machine-readable inventory of everything inside an image, generated by **syft** ([0.7. Glossary]({{< relref "/0. Overview/0.7. Glossary.md#sbom" >}})).
...
1. Builds an exact archive for every entry of `scripts/container-matrix.json` — one today, the agent — with source, revision, version, creation time, license, title, description, and documentation labels.
1. Loads, runtime-smokes, and scans each of those images, then generates its SPDX SBOM before any job can write to GHCR; vulnerabilities/secrets fail at HIGH/CRITICAL, and licenses fail outside `trivy.yaml`.
...
1. Promotes each source digest into a single-source version index only after every publication job succeeds; failure reconciliation rediscovers current registry state and removes only indexes that still prove exact ownership.
...
Buildx preserves the source manifest family. A Docker archive becomes an annotation-free Docker manifest list; an OCI source becomes an annotated OCI index. Both formats must contain one exact signed child whose revision and version labels bind the source.

Those steps form one fail-closed chain. Each protected write/OIDC job receives authority only after qualification and every read-only preflight.
...
A failure before publication may leave source-SHA images, an exact annotated tag, or a resumable draft; none is a published version by itself. Re-run the same version and SHA to resume. Reconciliation removes only version indexes whose package record, registry digest, and single source manifest prove ownership. OCI indexes must also carry the exact revision and version annotations; Docker manifest lists must carry no unsupported annotations. An ambiguous registry or release lookup preserves every index and fails for manual review.
...
flowchart LR
    ...
    Qualify --> Preflight
    subgraph Preflight["preflight — every matrix image, no registry write"]
        direction TB
    ...
    Publish --> Promote["promote source digests<br/>to single-source version indexes"]
...
It watches four targets, because Dependabot takes no globs and each project needs its own entry: GitHub Actions, the two `uv` projects, and the agent Dockerfile. Routine minor and patch bumps arrive as grouped pull requests; majors plus `google-adk` and `mlflow` stay separate. Docker updates ignore Python major/minor jumps so the build stage cannot silently diverge from the Python 3.13 runtime; patch and digest updates still arrive.

{{% admonition warning "Dependabot does not see every pin in this repository" %}}
...
Those four are updated by hand, and the quarterly [docs-freshness issue](https://github.com/MLOps-Courses/agentops-open-course/blob/main/.github/ISSUE_TEMPLATE/docs-freshness.md) is what makes sure someone looks. The coordinated-pins checklist below is the other half of that discipline. {{% /admonition %}}
...
{{% collapsible note "Deeper: the six coordinated pins" %}}
...
1. **MLflow**: `mlflow-skinny` in the `dev` group and full `mlflow` in the `eval` group of `agents/python/pyproject.toml` must stay on the same minor line, with the exact resolution in `uv.lock`. It is an evaluation-time dependency only, so no manifest or image pin follows it.
1. **OpenTelemetry Collector, Tempo, and Loki**: each image digest appears twice — in `infra/observability/compose.yaml` for host Compose, and in its `infra/k8s/base/` manifest for the cluster. Bump one without the other and the two profiles silently run different versions of the same store.
1. **Python 3.13**: `requires-python` in the pyprojects plus both interpreter pins inside `agents/python/Dockerfile` — the `python:3.13` build stage and the Wolfi `python-3.13` apk pin in the runtime stage. {{% /collapsible %}}
...
Return to [8. Community]({{< relref "/8. Community/_index.md" >}}) and pick your next maintenance question when you could reproduce this release pattern — tag, changelog entry, matching metadata, signed image — in a repository of your own.
