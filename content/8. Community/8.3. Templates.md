...
| evaluation expectations     | `evals/*.evalset.json`, `evals/cost_baseline.json`, `evals/judge-calibration.json` |
...
The current Go suite enforces behavior but no coverage percentage. A generator must not invent a coverage policy the owner has not selected.
