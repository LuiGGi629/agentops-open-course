...
- **You need:** `mise run install:maintainer` finished, so the full license gate can synchronize all four lock-owned dependency profiles.
...
{{% admonition warning "Copying a page copies two licenses" %}}

Copy a rendered chapter and you copy material under two licenses at once. The surrounding explanation is CC-BY, so attribute it; the quoted code block is MIT, so preserve its notice. {{% /admonition %}}

## Which license covers the code snippets shown in the course?

The chapters are CC-BY, but many of them **embed real source code** rather than paraphrasing it. The FAQ pages mirror source through `pymdownx.snippets`: a `--8<--` marker pulls a named region out of a file under `agents/` straight into the rendered page. The guardrail excerpt on [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}), for instance, is a live include of a region in `agents/python/src/agent/composition.py`, rebuilt from the file at every site build. So the code you read in a fenced block is the MIT-licensed source, verbatim, not a CC-BY retelling of it.
...
None of this is exotic. It is the ordinary consequence of putting code and prose in one document, and it is easy to miss when you only think of the page as "the course". How the snippet mirroring is wired is owned by [8.4. Documentation]({{< relref "/8. Community/8.4. Documentation.md" >}}).
...
1. **Inventory** — creates isolated temporary environments and collects a `pip-licenses` manifest, in parallel, for four lock-owned profiles: documentation development, agent runtime, agent development, and agent evaluation. It then injects one fake proprietary distribution, proves the inventory sees it, resynchronizes from the lock, and requires the clean and pre-populated inventory verdicts to be byte-identical. MLflow is still audited here, but as part of agent evaluation: it is an evaluation-time dependency rather than a shipped runtime service, so it no longer earns a profile of its own.
...
flowchart TD
    ...
    B --> C["sync 4 lock-owned profiles:<br/>docs dev · agent runtime/dev/eval"]
...
**Diagram in words:** The full gate checks repository notices, synchronizes four isolated profiles, proves a pre-populated environment returns to the clean inventory, rejects unapproved licenses, and verifies reviewed embedded license text.
...
`mise run check:licenses:core` is the smaller learner path: documentation development plus agent runtime and development. The full command adds agent evaluation, which is where MLflow and the rest of the evaluation chain are reviewed. Each profile is synchronized into a fresh temporary environment from its lock. The deliberate `agentops-ambient-contaminant` fixture makes that isolation an executed regression, so an optional package left in a workstation venv cannot change the verdict. A cold machine may still download missing locked wheels.
...
agent evaluation: embedded license verified for google-crc32c
...
{{% admonition warning "What a failure looks like" %}}
...
A package whose embedded text cannot be confirmed fails the same way, printing `agent: could not verify the embedded license for <package>`. {{% /admonition %}}
...
{{% collapsible note "Deeper: how the container scan complements this gate" %}}
...
That same `trivy.yaml` policy is reused wherever images are built. The [`scan.yml`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/.github/workflows/scan.yml) container matrix reads `scripts/container-matrix.json`, which now holds a single entry — the agent image — and runs Trivy twice on it: vulnerabilities and secrets at HIGH/CRITICAL, then licenses at UNKNOWN/HIGH/CRITICAL. The release workflow applies the identical license gate before any image is pushed or published (see [8.2. Releases]({{< relref "/8. Community/8.2. Releases.md" >}})). The two gates are complementary by design: `check-licenses.sh` reads the four locked Python environments where the course adds most of its dependencies, while the Trivy image scan reaches the OS-level packages baked into the container that a Python inventory never sees. {{% /collapsible %}}

## What license should my own agent use?

When you turn the reference agent into your own project (see [8.3. Templates]({{< relref "/8. Community/8.3. Templates.md" >}})), pick the license that matches your goal:
...
- `mise run check:licenses` prints `repository licenses: MIT software + CC BY 4.0 course content + OFL 1.1 fonts`, then the inventory and package-count lines for all four profiles, and exits zero.
...
Return to [8. Community]({{< relref "/8. Community/_index.md" >}}) and pick your next maintenance question when you can name the license of any file in this repository from its path alone.
