...
Every command must pass and the status must contain no unexplained change. Coverage output may be recorded as measured evidence, but no Go percentage is enforced because the owner has not selected one.
...
| model behavior     | model-backed eval artifact bound to source, model, evalset, and release-policy identity |
...
1. Deterministic gate results and measured coverage, clearly marked as non-policy.
...
No row awards points for meeting an unselected Go coverage percentage.
