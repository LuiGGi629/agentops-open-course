...
## Ana is fiction; your Tuesday is not
...
```text
[test:go] DONE 1815 tests, 1 skipped in 4.127s
...
[test:go]   ok      87.6%  agents/go/memory
...
Those are the `agents/go` lines, trimmed to eight of the twenty-two packages the task reports, with the `evals` and `tools` sections cut; the floor line is the last thing that module prints. `tools` sits at 98.5% and `policy` at 90.9% — those are the packages whose failure modes were worth writing tests about. The thin ones are `data` and `state` at 82.7%, both still above the floor. Copy the table into your brief: the percentages go stale the moment you edit.
...
## Your turn: rename one incident and read the blast radius
...
## The receipt you can hand to a reviewer

Once the tracked work is committed and the tree is clean, make the offline half checkable by others. One command writes two files:
...
The tool reruns the deterministic learner checks and records the exact Git revision plus SHA-256 digests of the artifacts listed in `tools/internal/courseevidence/artifacts.txt`: `mise.lock`, the three module `go.sum` files, the seed database, and one evalset. That inventory is a committed file rather than a compiled list, so changing what a certificate covers is a reviewable diff. It records no environment values, command output, model content, credentials, or runtime state.

A reviewer at that revision runs the second half, which needs no argument — pass a path to check someone else's manifest instead:
...
Score bounded outcomes and what backs them, never tool count. No row awards points for a coverage percentage: clearing the 80% floor is the entry price, and credit goes to tests that would fail if the behavior regressed. The nine criteria sum to 100.
...
Then tell someone. Open a showcase issue with the tier you reached, what your agent does and for whom, and the manifest revision behind it; the `capstone` label is where those land, and reading what other people bounded is the fastest way to see what a second domain looks like.
