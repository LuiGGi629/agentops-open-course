...
- **Time:** about 60 minutes to plan and measure it; the build itself is open-ended, hands-on. {{% /admonition %}}
...
| Platform and telemetry    | `infra/k8s/`, `infra/observability/`                                                                                |
...
Required drill — the [Chapter 8 checkpoint]({{< relref "/8. Community/_index.md#what-proves-this-chapter-worked" >}}) asks for its result. Break one seed row and let the suite show you the blast radius, instead of planning a port around a guess.
...
Each milestone is taught by the chapter labelled on its node, so a stalled milestone tells you exactly where to re-read: milestone 3 in [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}) for the guarded write, milestone 7 in [Chapter 5]({{< relref "/5. Gateway/_index.md" >}}) for the host path, milestone 8 in [Chapter 6]({{< relref "/6. Platform/_index.md" >}}) for local delivery, and milestone 9 in [Chapter 7]({{< relref "/7. Observability/_index.md" >}}) for the operations loop. Keep the core gate green after every milestone, then add the relevant runtime proof. Run the full maintainer gate before the final handoff.
...
{{% collapsible note "Deeper: the eight contracts, in full" %}}
...
- Offline tests never require a model, provider account, cluster, or cloud resource. {{% /collapsible %}}
...
{{% collapsible note "Deeper: the eleven items the handoff lists" %}}
...
- Cleanup commands and confirmation that disposable processes/resources were stopped. {{% /collapsible %}}
...
{{% collapsible note "Deeper: the hundred-point self-review rubric" %}}
...
Continue to [the optional OSS maintenance tour]({{< relref "/8. Community/8.0. Repository.md" >}}) when you plan to publish or sustain the result, or return to [the course home]({{< relref "/_index.md" >}}).
