...
That synthetic id still links the pause, the resume, the mutation, the session, the invocation, and the MLflow trace into one correlatable thread. What it does not do is name a person. The [5.3. A2A Gateway]({{< relref "/5. Gateway/5.3. A2A Gateway.md" >}}) integration test asserts exactly this default shape (`approved_by == f"A2A_USER_{context_id}"`).
...
1. Prometheus metrics age out at 2 days in the Kubernetes overlay (7 days on the host Compose profile) and Loki logs at 7 days by explicit configuration, so the metrics and log lines that gave an action its context are gone within a week — that retention is [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}})'s lane.
1. MLflow ships no retention or garbage-collection job, so its traces and its human and judge assessments persist for the life of the volume (the `mlflow-data` volume on the host, the 5 Gi PVC in Kubernetes, GCS artifacts on GKE) — [7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md#where-does-an-assessment-live-and-for-how-long" >}})'s lane.
...
Do not infer runtime storage from telemetry policy. The two content-capture flags keep message bodies out of exported OpenTelemetry and MLflow spans by default ([7.5. Online Evaluation]({{< relref "/7. Observability/7.5. Online Evaluation.md#why-cant-you-re-score-a-stored-runtime-trace" >}})). They do not change what ADK or the A2A server writes to `runtime.db`.
