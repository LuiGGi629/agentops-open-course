...
description: Trace the authority chain from a real caller to an append-only audit row and its Tempo trace, then reckon honestly with evidence lifetime and who can still alter it.
url: "/7-observability/7-6-governance/"
---
...
flowchart LR
    ...
    Row --> Trace[("Tempo trace<br/>same session and invocation ids")]
```

**Diagram in words:** A real operator reaches the agentgateway edge, which validates a JWT. When `AGENT_TRUSTED_IDENTITY_HEADER` is set, the verified subject is threaded into `user_id`; when it is unset — the default — ADK substitutes a synthetic per-conversation user instead. Either value reaches the `ToolContext` and is written into the audit row's `approved_by`, alongside the session and invocation ids that also appear on that turn's Tempo trace.

On the default course path the A2A listener is unauthenticated ([4.6. Security]({{< relref "/4. Quality/4.6. Security.md" >}})), so there is no real subject to carry. ADK derives a synthetic per-conversation user `A2A_USER_<context-id>`, and that value lands in `approved_by`.

That synthetic id still links the pause, the resume, the mutation, the session, the invocation, and the Tempo trace into one correlatable thread. What it does not do is name a person. The [5.3. A2A Gateway]({{< relref "/5. Gateway/5.3. A2A Gateway.md" >}}) integration test asserts exactly this default shape (`approved_by == f"A2A_USER_{context_id}"`).
...
Owned by [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-does-the-gateway-verified-identity-reach-the-audit-row" >}}), which shows the header contract.
...
Read the rows the agent wrote. One has to exist first: approve a guarded action through the confirmation flow of [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md#how-does-human-confirmation-work" >}}) before you run either command below.

On the Kubernetes path this needs the local platform from [6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}), with the agent deployed as in [6.6. Platform Delivery]({{< relref "/6. Platform/6.6. Platform Delivery.md" >}}). Read the table inside the pod:
...
{{% admonition warning "An empty list is not a broken log" %}}

Both commands print `[]` until a guarded action has been approved and executed. The committed seed carries no audit rows, so an empty list is the starting state rather than a failure. {{% /admonition %}}

Add `session_id, invocation_id` to the `SELECT` list and compare those values with the corresponding trace in Grafana's Tempo view: this is the right-hand join in the authority diagram above, made concrete. Do it inside the retention window — the trace side of that join expires in seven days while the row does not. Treat the synthetic `A2A_USER_<context-id>` as correlation evidence, not authenticated identity. Do not expose audit rows through an unrestricted agent tool — a read tool the model can call turns the append-only log into model-reachable data.
...
One honesty note carried from [7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md#where-does-an-assessment-live-and-for-how-long" >}}): `redact_persisted_text` guards this `rationale`, but the parallel free-text field a human types into an offline MLflow assessment is _not_ redacted — and that store sits outside the cluster, under no retention policy at all. Keep reviewer notes about behavior and off the record for specifics.
...
| Traces           | Tempo blocks on a PVC                     | 7 days (`block_retention: 168h`)        |
| Assessments      | offline MLflow store, outside the cluster | life of that file or server — no GC job |
...
Four rows use the glossary's [PVC]({{< relref "/0. Overview/0.7. Glossary.md#pvc" >}}) and [RWO]({{< relref "/0. Overview/0.7. Glossary.md#rwo" >}}) storage contracts.

Those lifetimes are set in different places:

1. Prometheus metrics age out at 2 days in the Kubernetes overlay (7 days on the host Compose profile), Loki logs at 7 days, and Tempo trace blocks at 7 days, all by explicit configuration. The three runtime pillars behind an action therefore expire together, within a week — that retention is [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}})'s lane. It is a shorter window than an unbounded trace store gives you, and a far easier one to state truthfully to an auditor.
1. The offline MLflow store ships no retention or garbage-collection job, so evaluation runs and their human and judge assessments persist for the life of whatever file or server you chose — [7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md#where-does-an-assessment-live-and-for-how-long" >}})'s lane. It is also the one row in this table that lives outside the cluster's storage and access controls entirely, which is a governance fact in both directions: it survives a cluster teardown, and it is protected by nothing the cluster enforces.
...
{{% collapsible note "Deeper: how the nightly snapshot is taken, and what restores it" %}}

The only copy off that single PVC is the nightly snapshot. The `agentops-state-backup` CronJob runs `schedule: "30 3 * * *"` (daily 03:30 UTC), mounts the state PVC read-only, uses the stdlib `sqlite3` backup API for a consistent page-by-page copy while the agent keeps writing, verifies each file with `PRAGMA integrity_check`, and keeps the newest seven completed snapshots on a second PVC. Its own manifest comment is honest about scope: a backup PVC in the same single-node cluster is crash and mistake recovery, not disaster recovery, and `skaffold delete` removes both PVCs together. The course does ship a restore path — `restore-state.sh` reverses one snapshot into a state directory after you stop every writer, and `backup-drill.sh` proves offline that a snapshot restores before you need it — but no cluster-level disaster-recovery runbook. {{% /collapsible %}}

So one action's evidence still has heterogeneous lifetimes — 2 days of metrics, 7 days of logs and traces, PVC-lifetime sessions, tasks, and audit rows, plus seven daily state snapshots, and assessments that never expire on their own. Losing the cluster or both PVCs loses all of it at once, except the one store that was never in the cluster.
...
Do not infer runtime storage from telemetry policy. The two content-capture flags keep message bodies out of the exported OpenTelemetry spans and GenAI log events by default ([7.5. Online Evaluation]({{< relref "/7. Observability/7.5. Online Evaluation.md#why-cant-you-re-score-a-stored-runtime-trace" >}})). They do not change what ADK or the A2A server writes to `runtime.db`.
...
{{% collapsible note "Deeper: what engineering cannot decide about retention" %}}

A _legal basis_ is the lawful reason a regulation permits processing personal data. Engineering can minimize fields, separate stores, and expose deletion mechanisms. It cannot choose a legal basis, decide whether an exception applies, or set a lawful retention period. The controller should document those decisions with its data-protection owner or counsel before production. {{% /collapsible %}}
...
| Tempo            | trajectory metadata; message content when capture is enabled            | Grafana Explore, by trace id       | none — blocks expire at 7 days, which is not erasure |
| Offline MLflow   | evaluation traces and assessment text                                   | manual trace lookup in that store  | none — no subject erasure or GC workflow             |
...
- The mutation and its audit row commit in one `BEGIN IMMEDIATE` transaction in `data.py`, so there is no window with a state change and no record — the atomicity [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md#why-are-mutation-and-audit-one-transaction" >}}) owns.
...
{{% collapsible note "Deeper: which GKE identity may do what" %}}

The same principle — one workload identity per external authority, no long-lived JSON key — governs the cloud side on GKE:

- Every workload ServiceAccount in the namespace (agent, MCP server, agentgateway, Tempo, Loki, the collector, and the backup job) sets `automountServiceAccountToken: false`; GKE Workload Identity Federation uses the metadata server instead.
- The agentgateway WIF identity is the only one annotated for a Google service account, and it can consume Vertex and service usage.
- Node identity reads Artifact Registry and performs GKE node duties.
- The telemetry workloads need no cloud authority at all: Tempo and Loki write to their own PVCs, so there is no artifact bucket to scope and no third identity to review.

A blast radius stays scoped to one workload because no workload shares a credential with another. {{% /collapsible %}}
...
The full reproducibility tuple — code, dependencies, image, model path, prompt, data, tool contract, runtime, evaluation — is [7.0. Reproducibility]({{< relref "/7. Observability/7.0. Reproducibility.md" >}})'s lane, and its one-pass commands capture it. Do not re-list it here. This page's addition is the authority-and-retention layer on top of that tuple:

- Who approved the release and which residual risks they accepted, recorded rather than implied.
- The audit-and-trace correlation for any guarded action taken during validation — session and invocation ids agree across the row and the Tempo trace, captured while that trace still exists.
...
Approve one mock action, then verify that the action result, the audit row, and the trace in Grafana's Tempo view agree on the same session and invocation ids. Attempt to rewrite an existing row and confirm the trigger aborts it — an identity-only update needs no string literal and still fires `BEFORE UPDATE`:
...
- That row's `session_id` and `invocation_id` match the ids on the corresponding Tempo trace.
...
- You can say, per pillar, how long the evidence survives: metrics, logs, traces, runtime sessions and tasks, audit rows, snapshots, and offline assessments.

Continue to [7.7. Incident Response]({{< relref "/7. Observability/7.7. Incident Response.md" >}}) when you can point at the row for a change you approved, say who authorized it, and admit who could still rewrite it.
