...
A log line records that something happened; a trace records _how the work was structured in time_. Every unit of work becomes a **span** with a start time, an end time, a status, and a parent, and every span in one request shares one **trace id**. The result is a tree you read top-down — the turn, the model calls under it, the tool calls under those, the MCP round trip under a tool.

Here is one real turn, pulled out of Tempo by trace id and flattened into start offset, duration, status, and span name, with both times in milliseconds:
...
That is one question — "what is the status of the checkout service?" — asked on a CPU-only laptop, and yours will be far faster. Read the shape rather than the numbers. The tool call, the part everyone blames first, took **five milliseconds** out of five and a half minutes. Two model calls took everything else, and the second never returned, so its span carries `STATUS_CODE_ERROR`. A flat log of the final answer would have said "the model provider is unavailable" and left you guessing across four boxes. The tree tells you which box to open.
...
The `gen_ai.*` and `gcp.vertex.agent.*` keys are ADK's, following the OpenTelemetry GenAI conventions. The four `agentops.*` keys are this repository's own: `RecordTokenUsage` stamps the session's running totals onto whichever span is active, because a Prometheus counter could only answer "what did this conversation cost" by making the session id a label — and a session id as a label is an unbounded series. So yes, the root span is where the totals live, and [7.3. Costs]({{< relref "/7. Observability/7.3. Costs.md" >}}) gives them a budget.
...
**Row 3, `execute_tool get_service_status`, starting at 152,568 ms and lasting 5 ms.** The interesting number is the start offset, not the duration: the tool was never slow and never waiting, it simply could not run until the model decided to call it. Two attributes carry its payload:
...
Look inside that response: the tool result reaches the trace already wrapped in the `TOOL_DATA` fence from [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md#how-is-prompt-injection-handled" >}}), so you can watch the untrusted-content boundary do its job. That serialized response is also the exact reason this page has a risk gate, which is the next section.
...
**Four rows, one verdict: the agent's code and its tool spent five milliseconds, the model provider spent the rest and then timed out. You can say that in standup without hedging, and you wrote no instrumentation to earn it.**

Two habits carry over. Sort by start offset rather than duration, because a child that starts late tells you what it waited for. And read `UNSET` as success: ADK sets `ERROR` only where it records one, so a quiet span is healthy, not silent.

## The switch that keeps ADK spans off by default

You will not find that tree in Tempo yet, and the reason is deliberate. Every `agent` entrypoint applies a privacy rule before dispatch, and runtime assembly reapplies it before either provider-ownership path constructs an ADK provider:
...
The asymmetry behind that warning is the sharpest thing on this page. Structured logs are _actively_ defended: before a record reaches the console or OTLP, independent handlers redact personal-data and credential patterns, cap every string at 2048 characters, and keep only sanitized error evidence. **Spans get no such filter.** Their defense is simpler and stronger — they are non-recording by default. Turning them on is a risk decision, not a verbosity setting.

The obvious follow-up — then where do prompts, responses, and eval verdicts live? — has an answer this course deliberately does not ship. **Langfuse** and **Arize Phoenix** are the open-source category that owns exactly that: prompt, response, and evaluation storage with its own retention window and access model, rather than a general-purpose trace store every engineer already has read access to. Keeping that content out of a generic backend is why the pinned Kubernetes profile ships `OTEL_TRACES_SAMPLER=always_off` in `infra/kagent/agent.yaml`, and why `agents/go/telemetry` pins the same value at startup unless `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS` is the exact literal `true`. The OTel collector is the seam such a backend attaches to; adding one is a decision about who may read a prompt, not a configuration change.

Three variables sit near that decision, and the one whose name mentions spans is not the one that fills spans. The pinned Go ADK never reads the first; this repository owns it as the explicit risk gate.
...
## One collector, two kinds of evidence
...
Tempo has no notion of an experiment or a project: a trace is addressed by its `trace_id` and nothing else, so there is no name to keep in sync and no header that can silently reroute your spans. That is what lets runtime and evaluation evidence share one stack — the second thing this collector is for.

`mise run eval` writes `results.json`, and that file is the gate. Export `EVAL_OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318` first, though, and the same run pushes an `agentops.eval.run` span tree and five gauges through the collector you just watched. Grafana already has the dashboard that reads them:
...
That is `evals/grafana-dashboard.json`, mounted into Grafana by the observability Compose file: the run-comparison view an experiment tracker would have charged you for. Its two controls, **Reviewed baseline run** and **Candidate run**, are populated from the `eval_run_id` label, and both stay empty until your first exported run — so name your runs or you will be choosing between two uuids: `mise run eval -- --run-id baseline-tone`, then `--run-id candidate-tone` after you change the instruction.
...
Those names hide the most confusing failure in this chapter. The harness declares `agentops.eval.score` as a **gauge with unit `1`**, and the collector's Prometheus exporter renames every instrument on the way out: dots become underscores, a gauge whose unit is `1` gains a `_ratio` suffix, and a unit in braces such as `{token}` is dropped as an annotation. Guess wrong and Grafana says "No data" while every component is healthy.
...
- **Files to touch**: none. No tracked file and no `.env` entry changes; every variable lives in the shell you are sitting in.
...
A slow turn used to be an anecdote you retold in standup. You can open one now, point at the hop that spent the time, and say out loud what you had to accept in order to see that much.
