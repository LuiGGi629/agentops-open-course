...
| ---------------------------------------------------- | -------------------------------------------- | --------------------------------------- |
| `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS`               | span attributes on the model and tool spans  | the trace store (MLflow `:5000`)        |
...
1. Issue one read-only request, find its trace in MLflow, and identify the agent, model, tool, and MCP spans with their status and timing arranged as one tree.
