...
- **You need:** [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}) and [7.3. Costs]({{< relref "/7. Observability/7.3. Costs.md" >}}) finished, with the host observability stack still running.
...
The reference agent spends the whole course triaging incidents for a fictional service. It is itself a running workload now — a host process behind the gateway, or a Kubernetes workload if you have done [Chapter 6]({{< relref "/6. Platform/_index.md" >}}) — and a workload has incidents of its own. Six symptoms all mean the agent itself is misbehaving, not the platform it watches:
...
- **Sending only one turn.** `Policy.EnforceTokenBudget` refuses once the session's spent tokens reach the limit, and a fresh session starts at zero. The first turn always reaches the model. Send two.
- **Setting the variable on a different command.** Keep the inline assignment on `mise run run`; a value exported in another terminal does not reach this process. Run `AGENT_MAX_TOKENS_PER_SESSION=1 mise run config:check` from `agents/go` to print the configuration the agent resolves.
- **Expecting the alert immediately.** Prometheus needs one scrape to mark the target down, then the whole `for:` window before `pending` becomes `firing`. An alert stuck in `pending` usually means you restarted the collector too early. {{% /admonition %}}
...
| -------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| `AgentErrorBudgetBurn`           | Spans are failing faster than the 99% budget allows    | [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}) → the trace |
...
| `ObservabilityCollectorDown`     | Prometheus cannot scrape the collector — you are blind | [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}})             |
...
{{% collapsible note "Deeper: the two incidents nothing will page you about" %}}

Two incident classes have no alert because they are judgements, not thresholds: a **quality** incident (a cluster of thumbs-down [feedback]({{< relref "/7. Observability/7.4. Feedback.md" >}}) or judge disagreement on answers that are wrong, not just terse) and a **cost** incident (a prompt or model change that quietly doubled tokens, surfaced by the [token-drift warning]({{< relref "/4. Quality/4.4. Evaluations.md#why-does-one-green-run-prove-so-little" >}}) rather than a live gauge). Treat both as first-class incidents even though nothing pages you. {{% /collapsible %}}
...
1. **Metric** — confirm the alert is real and scope it: is the error ratio or p95 rising for every turn or one tool? ([7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}))
1. **Trace** — open one failing turn in Grafana's Tempo view and read the span tree: which model or tool span failed, retried, or ran long, and what were its token counts? ([7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}))
1. **Logs** — follow that span's logs link into Loki for the error text the span only summarizes; under pressure, a provisioned link beats a copied identifier. ([the collector fans logs to Loki]({{< relref "/7. Observability/_index.md" >}}))
1. **Audit** — if a guarded remediation changed state, read the append-only audit row: who approved which action, with what rationale, against which incident. ([7.6. Governance]({{< relref "/7. Observability/7.6. Governance.md" >}}))
...
Most agent incidents can be mitigated with existing configuration or a prior image. `config.Load()` reads environment values once at process startup, so changing a flag requires restarting the host process or rolling out the workload configuration; none of these flags hot-reloads.
...
The runtime has no mutable prompt-registry seam. Prompt rollback means redeploying the prior evaluated image digest and its matching Git revision, which is the instruction authority.
...
{{% collapsible note "Deeper: the full template, and one filled in" %}}
...
Prompt version 5 reworded the report section and dropped the sentence naming every required field, so the model stopped emitting severity. domain.ParseTriageReport decodes with DisallowUnknownFields and severity has no default, so every report failed validation.
...
Reproduce the behavior on the committed seed with `mise run data:reset`. Never paste a real production conversation into the repository. Transcribe the _behavior_ onto sanitized seed data, exactly as the [feedback loop]({{< relref "/7. Observability/7.4. Feedback.md#how-should-recurring-feedback-change-the-course" >}}) prescribes.
...
- A cost incident is explained before it is accepted. The harness only warns when a run's token total drifts more than 25% from the previous one, so the fix is an account of what changed, never a quiet re-run that makes the warning stop.
...
1. Induce a capacity incident: from `agents/go`, run `AGENT_MAX_TOKENS_PER_SESSION=1 mise run run` and send two turns. The second is refused with `TOKEN_BUDGET_EXHAUSTED`. Observe the refusal message and the `agentops.tokens` counter, exactly as [7.3. Costs]({{< relref "/7. Observability/7.3. Costs.md" >}}) describes.
1. Walk the signals: find the turn's trace ([7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}})), confirm the token attributes on its spans, and confirm no audit row was written because no write was approved ([7.6. Governance]({{< relref "/7. Observability/7.6. Governance.md" >}})).
...
1. Promote one action to a repeatable path: add a unit test asserting `Policy.EnforceTokenBudget` refuses past the limit and run it, or add a Prometheus rule and prove both its CI manifest validation and runtime drill.
...
- The second turn of your budgeted session was refused with `ErrorCode: "TOKEN_BUDGET_EXHAUSTED"`, and the first was not.
- You can open that turn's trace in Grafana's Tempo view and read `agentops.tokens.session.total` off its spans.
...
Continue to [8.7. Capstone]({{< relref "/8. Community/8.7. Capstone.md" >}}) when the relevant evidence is green for the exact `main` revision. The Chapter 8 maintenance pages remain optional references for publishing or sustaining an OSS project.
