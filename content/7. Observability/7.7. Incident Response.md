...
Two incident classes have no alert because they are judgements, not thresholds: a **quality** incident (a cluster of thumbs-down [feedback]({{< relref "/7. Observability/7.4. Feedback.md" >}}) or judge disagreement on answers that are wrong, not just terse) and a **cost** incident (a prompt or model change that quietly doubled tokens, caught by the [`eval:cost`]({{< relref "/4. Quality/4.4. Evaluations.md#how-does-cost-scoring-avoid-silent-regressions" >}}) baseline rather than a live gauge). Treat both as first-class incidents even though nothing pages you. {{% /collapsible %}}
