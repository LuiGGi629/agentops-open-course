...
1. **Trace** — open one failing turn in MLflow and read the span tree: which model or tool span failed, retried, or ran long, and what were its token counts? ([7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}))
1. **Logs** — pivot to the correlated Loki logs for that window for the error text the span only summarizes. ([the collector fans logs to Loki]({{< relref "/7. Observability/_index.md" >}}))
