...
- Run id, source tuple, platform identity, model name, evalset id, and transport.
...
The judge is another scorer, not a human and not an authority. Its verdict becomes an `agentops.eval.score` span only after the configured judge passes the balanced calibration floor.

Calibration writes a sanitized schema-3 artifact containing the exact source and policy identities, typed judge provider/name/digest, calibration digest, agreement floor, matches, total, agreement, and per-case match fields. A release qualifier requires its floor to equal the reviewed policy value exactly.
