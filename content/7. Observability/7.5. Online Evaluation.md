...
- **You need:** [7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}) finished, with some real turns stored in the `agentops-agent` experiment.
...
{{% admonition warning "Your query also returns eval traffic" %}}
...
No. It ships trace collection ([7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}})), MLflow storage, the offline scorers above, and trace-linked human feedback ([7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md" >}})). It does not schedule any scorer over live traffic, so it makes no claim that drift detection is active.
...
1. Human MLflow assessments ([7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md" >}})) are the only shipped _quality_ signal on real turns, and they are sparse and manual.
...
1. Write assessments back to the source trace without changing it — the same append-only assessment model human and judge feedback already use ([7.4. Feedback]({{< relref "/7. Observability/7.4. Feedback.md#how-do-human-and-judge-assessments-share-one-store" >}})).
...
First, inspect real telemetry: run the metadata-only `mlflow traces search` above against experiment `0` and confirm you can distinguish live turns from `eval_id`-tagged eval cases; then open the [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md#which-dashboard-is-shipped" >}}) dashboard and name which shipped signals would move first under input or model drift. Do not enable an automated live judge.
