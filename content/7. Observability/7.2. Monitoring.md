...
- **You need:** [7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}) finished, with the host Compose stack running and traces landing in MLflow; the k6 half also needs `mise run mcp:http`, `mise run a2a`, and `mise run gateway:host` up.
...
Spans become RED metrics and Loki logs at the collector; page only on sustained, user-visible symptoms, not raw errors. {{% /admonition %}}
