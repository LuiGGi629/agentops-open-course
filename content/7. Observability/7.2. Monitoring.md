...
This page reads the dashboard's six expressions, follows one trace id into a **sanitized** log record — one stripped of anything that could carry a credential, an endpoint, or customer data — and measures each layer under load.
...
The shipped `AgentOps overview` dashboard has six metric panels and one logs panel. Read the six expressions once, because you write the seventh yourself:
...
The `clamp_min(..., 1)` in the error ratio floors the denominator at one, so a single failure on an idle lab reports a small ratio instead of a 100% spike. The `Agent logs` panel below them queries Loki for `agentops-agent` lines, filtered by an optional `trace id` textbox.
