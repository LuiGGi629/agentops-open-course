...
1. **Diagnose**: `docker compose -f infra/observability/compose.yaml ps otel-collector` (host) or `kubectl -n agentops get pods -l app.kubernetes.io/name=otel-collector`; then read the collector's `:8889` exporter to see whether the counter is exported at all. Compose keeps `:8889` on the internal network, so on the host ask the scraper that already reaches it: `docker compose -f infra/observability/compose.yaml exec -T prometheus wget -qO- http://otel-collector:8889/metrics | grep agentops_tokens`. In Kubernetes, `kubectl -n agentops port-forward svc/otel-collector 8889:8889` first, then `curl -fsS http://localhost:8889/metrics | grep agentops_tokens`. Empty output is the answer, not a broken command: it means the counter is genuinely absent, which is the exact condition `AgentTokenTelemetryMissing` fires on.
