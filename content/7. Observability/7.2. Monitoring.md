...
url: "/7-observability/7-2-monitoring/"
---
...
- **You need:** [7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}) finished, with the host Compose stack running and traces landing in Tempo; the k6 half also needs `mise run mcp:http`, `mise run a2a`, and `mise run gateway:host` up.
...
Those panels are not a second instrumentation. The collector derives [RED metrics]({{< relref "/0. Overview/0.7. Glossary.md#red-metrics" >}}) — rate, errors, duration — from the spans you already exported in [7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}).
...
You click. And from a log line you click back to the trace. Both directions are provisioned, and neither asks you to copy an identifier.
...
Grafana's provisioned datasources turn that shared id into two links ([`grafana/datasources.yaml`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/observability/grafana/datasources.yaml)):
...
**Diagram in words:** One turn's OTLP export enters the collector and leaves as three things — spans to Tempo, log records to Loki, and connector-derived metrics that Prometheus scrapes. Grafana reads all three, and Prometheus routes fired alerts to Alertmanager. The two dotted edges are the provisioned datasource links rather than data flows: Tempo's `tracesToLogsV2` points at Loki, and Loki's `trace_id` derived field points back at Tempo.
...
Spans become RED metrics and Loki logs at the collector, and one `trace_id` walks you between the trace and its logs in either direction; page only on sustained, user-visible symptoms, not raw errors. {{% /admonition %}}
