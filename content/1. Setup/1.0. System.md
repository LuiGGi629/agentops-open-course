...
Here is what it says before anything is installed: every line names a tool and its remedy, and the run exits non-zero. This is a real run with `PATH` stripped to the few shell utilities the script itself calls, so nineteen tools come back missing; these are its first four lines:
