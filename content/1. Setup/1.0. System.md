...
Use [0.6. Troubleshooting]({{< relref "/0. Overview/0.6. Troubleshooting.md" >}}) when the failing layer is not obvious.
...
Formatting may update files; inspect those changes before keeping them. No coverage percentage is a policy gate: the Go suites report measured coverage as evidence, while correctness is enforced by tests, race detection, formatting, linting, and the repository contracts.
