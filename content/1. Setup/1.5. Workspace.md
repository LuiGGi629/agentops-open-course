...
The canonical clone and install steps live in [1.0. System]({{< relref "/1. Setup/1.0. System.md" >}}). Here, confirm that installation wired the workspace correctly:
...
As [1.0. System]({{< relref "/1. Setup/1.0. System.md" >}}) defines, `mise run install` builds both learner environments and wires **lefthook** into `.git/hooks`. The two `test` commands are silent passes; a missing hook exits non-zero.
...
{{% collapsible note "Deeper: what else is ignored, and why?" %}}
...
Your editor is a personal tool, not a production dependency: it never appears in a lockfile, an image, or a deployment, so it cannot affect whether someone else can reproduce your results. That is why the course pins the runtime stack precisely and stays silent about your desktop. See [0.5. Resources]({{< relref "/0. Overview/0.5. Resources.md" >}}) for the same reasoning applied to coding assistants.
...
Copy the annotated `.env.example` to `.env` at the repository root only when you want to change a default — every variable documents its purpose, default, and which learning path needs it. [1.4. Providers]({{< relref "/1. Setup/1.4. Providers.md" >}}) walks through that for the optional Gemini path. Either way, resolve and validate the combination:
...
[1.1. Python]({{< relref "/1. Setup/1.1. Python.md" >}}) owns which tasks load the root `.env` and why deterministic gates do not. This page only verifies the resolved configuration.
...
{{% collapsible note "Deeper: what does `config:check` print, and what does it hide?" %}}
...
- On commit, `format-dprint` reformats staged config/Markdown files, `format-go` reformats Go, and `format-python` reformats Python. All three use `stage_fixed: true`, which re-adds the reformatted result to the index for you.
- Then eleven path-scoped checks cover formatting, Go, Python, shell, workflows, docs, links, skills, data, release metadata, and core licenses.
...
Chapter 2 is where this environment starts paying off: it runs and inspects the reference agent on your local model. Continue to [Agents]({{< relref "/2. Agents/_index.md" >}}) when `git status --short` prints nothing after `format:core`, `check:core`, and `test`.
