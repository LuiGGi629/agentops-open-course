...
The shipped deadline is sixty seconds with two bounded retries, sized for a warm GPU. On a CPU-only laptop it is not enough, and the reason is worth understanding before you tune it: the load is not the cost. Every model call re-reads the whole context from scratch, a grounded turn makes at least two of them — one to choose a tool, one to answer from its result — and the second is the larger, because it carries everything the first did plus the tool's output.

`mise run doctor:model` now measures the floor for you. Its third line times one single-token call:

```text
inference  ok in 29s (a full turn needs several calls of this size or larger)
```

Read that number as a threshold. Under about ten seconds, the shipped default is fine. Above it, expect turns in minutes rather than seconds and raise the budget accordingly — or use a smaller model, which is the lever that actually changes the shape of the curve rather than the wait for it.
...
Then set one line in it: `AGENT_MODEL_TIMEOUT_S=180` for a CPU-only laptop whose inference line reads a few seconds, or `600` and upwards when it reads thirty. Confirm the file was read at all with `mise run config:check`, which prints every resolved setting with secrets masked.

Predict what happens if you get greedy and set it to `4000`. Then try it:
...
One hour is the ceiling, and it is a ceiling on purpose: a turn that never finishes under an unbounded deadline is a hang, not a slow model, and it deserves to be found rather than waited out. It is deliberately far above what any comfortable machine needs, because the alternative — a ceiling a genuinely slow host cannot reach — turns the required, account-free path into one that cannot complete its first turn at all. [0.7. Troubleshooting]({{< relref "/0. Overview/0.7. Troubleshooting.md#why-does-every-model-turn-fail-at-about-60-seconds-on-my-cpu" >}}) walks through timing one turn before you tune, if you would rather measure than guess.
...
There is now a model on your own hardware, a check that proves it without spending a token, and a deadline set from your machine's measured behavior rather than from a default written for a GPU. The one number worth carrying forward is the `inference` line: everything Chapter 2 asks of the model costs several multiples of it.
