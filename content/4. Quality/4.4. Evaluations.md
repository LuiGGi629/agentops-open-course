...
description: Evaluate exact ADK tool trajectories and full conversations, then log prompt/model lineage and optional judge evidence in offline, self-hosted MLflow.
url: "/4-quality/4-4-evaluations/"
---
...
- **Time:** about 55 minutes, hands-on. {{% /admonition %}}
...
Catching a wasteful-but-correct trajectory needs a different signal: the token and model-call tripwire in [How do you catch a correct-but-expensive regression?](#how-do-you-catch-a-correct-but-expensive-regression), plus the cost and latency signals owned by [4.3. Metrics]({{< relref "/4. Quality/4.3. Metrics.md" >}}) and Chapter 7.
...
{{% collapsible note "Deeper: how the harnesses isolate and serialize each case" %}}
...
The scheduled job captures those full-conversation outputs once, and its cost and groundedness steps reuse the exact transcript that MLflow scored instead of making thirty duplicate conversations. The predictor creates its trace explicitly and disables MLflow's usual sample probe, because a model generation is not a harmless validation call. The capture is bound to the provider/model digest, prompt selection, normalized eval contract, and source revision; a new MLflow run removes any older capture first. {{% /collapsible %}}

{{% admonition info "What these labels do not prove" %}}

`redteam` is the deterministic offline regression owned by [4.6. Security]({{< relref "/4. Quality/4.6. Security.md" >}}), not live model red-teaming. The optional MLflow judge is advisory evidence and has no enforced threshold unless you add one to release policy. The audit trail elsewhere in this chapter is append-only SQLite on a writable volume, not an immutable or externally shipped sink. A green interactive demo cannot substitute for any of these checks. {{% /admonition %}}
...
`eval:retrieval` is the odd one out. It needs the local embeddings endpoint rather than a chat model, and it belongs to the memory chapter. Its ground truth comes free from the seed: every incident names its runbook, so **hit-rate@k** (how often the right runbook lands in the top _k_ results) needs no hand labeling. See [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md#how-do-i-evaluate-retrieval-quality" >}}).

{{% admonition tip "Before you run any model-backed task" %}}
...
That store is **offline by construction**. MLflow is not part of the running agent's telemetry path: the agent exports OTLP to the collector, which stores traces in Tempo and logs in Loki, and nothing MLflow-shaped runs until you start an evaluation task yourself. The `mlflow ui` command above is the only MLflow server in the course, you start it deliberately, and you can stop it the moment you are done reading. {{% /admonition %}}
...
{{% admonition tip "Raise the bar when your model can clear it" %}}
...
`--min-pass-rate=1.0` requires every ADK case to pass; `AGENT_EVAL_MIN_SCORE=1.0` requires every MLflow scorer mean to be perfect. The override can only raise a committed floor, never weaken it: completeness therefore remains `1.0` even when you request `0.8`. Treat a sustainable floor as a property of the model you chose, and record it beside the model in your scorecard ([4.3. Metrics]({{< relref "/4. Quality/4.3. Metrics.md#what-does-a-filled-scorecard-look-like" >}})). {{% /admonition %}}
...
Grow the set from real failures: when a trace in Grafana's Tempo view shows a wrong trajectory or an unsafe proposal, distill it into one case that tests that one behavior. That is the loop between the two stores — a production span in Tempo becomes an offline case in MLflow — and it only closes because each one keeps to its own side.
...
All three are properties of a deliberate offline run. MLflow is this course's **experiment tracker** — prompt registry, run comparison, scorer means, and the optional judge — driven by the evaluation harness against a file store on your own machine. It is not a telemetry backend and never sits in the collection path: a production trace belongs to the OTel collector and lands in Tempo, where Grafana reads it. Keeping the two apart is why the serving image can omit MLflow entirely and still be fully observable.
...
{{% collapsible note "Deeper: how the scorers are implemented and pinned by tests" %}}
...
[`tool_policy`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/evals/mlflow_eval.py) separately filters the three state-changing tools before exact comparison. `test_tool_policy_requires_exact_writes_but_allows_extra_reads` proves an exact write surrounded by reads passes, while duplicated or wrong-argument writes fail. {{% /collapsible %}}

{{% collapsible note "Deeper: how does a guarded write terminate without an assistant message?" %}}
...
A **judge** is a second model asked to grade the agent's answer. It stays off during the account-free Chapter 4 path. After the default host gateway from [5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md" >}}) is running, opt in with an explicit route:
...
{{% admonition info "Why the judge waits for Chapter 5" %}}
...
The three `MLFLOW_JUDGE_*` variables remain required together. {{% /admonition %}}
...
{{% admonition tip "A good stopping point" %}}
...
Everything below is the evaluation lifecycle: grounding, cost, prompt versioning, and side-by-side comparison. Each one adds a signal on top of what you already ran, and none of them changes it. Come back to them in a second sitting if this one has been long. {{% /admonition %}}
...
{{< include path="agents/python/evals/groundedness_eval.py" region="unsupported-claims" lang="python" >}}
...
Treat a jump the way you treat a trajectory miss: open the trace — the harness's own MLflow trace for an evaluation run, or the turn's spans in Grafana's Tempo datasource when the regression showed up in production — and find the new tool loop, retry storm, or verbose instruction that caused it.
...
Use [the decision table](#which-evaluation-task-should-you-run-and-when) as the runbook: validate first, then choose the cheapest task that answers your question and record its context with the result. {{% /admonition %}}
...
Continue to [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}) when a green evaluation reads to you as evidence about a fixed set of cases, not as proof the agent is safe.
