...
`--min-pass-rate=1.0` requires every ADK case to pass; `AGENT_EVAL_MIN_SCORE=1.0` requires every MLflow scorer mean to be perfect. The override can only raise a committed floor, never weaken it: completeness therefore remains `1.0` even when you request `0.8`. Treat a sustainable floor as a property of the model you chose, and record it beside the model in your scorecard ([4.3. Metrics]({{< relref "/4. Quality/4.3. Metrics.md#what-does-a-filled-scorecard-look-like" >}})).
{{% /admonition %}}
