...
- **Time:** about 10 minutes, hands-on. {{% /admonition %}}
...
- **`S` (flake8-bandit)** is the security family: hardcoded secrets, `subprocess` with `shell=True`, unsafe deserialization, `assert` in production paths. It is the static half of the story [4.6. Security]({{< relref "/4. Quality/4.6. Security.md" >}}) tells at runtime.
...
{{% collapsible note "Deeper: the other rule families and why they are selected" %}}

- **`LOG` (flake8-logging)** catches misused logging. The safe-error guardrail in [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}) depends on logging the real exception for operators while returning a stable message to the client — this family keeps that discipline honest.
...
- **`PGH` (pygrep-hooks)**, among other checks, rejects a blanket `# noqa` or `# type: ignore` with no rule code — it enforces the exact discipline the next section preaches. {{% /collapsible %}}
...
flowchart TD
    subgraph vocab["One vocabulary — root tasks delegate to the component task"]
        ...
        FMTC["mise run format:dprint {staged}<br/>+ format:go + format:python"]
        SCOPED["eleven globbed check:* tasks"]
...
**Diagram in words:** Developers run format, check, and test directly. Pre-commit orders three formatters, eleven path-scoped checks, and the staged security scan. Pre-push runs the complete core check before tests. CI installs the validation profile, runs the base doctor, then the full format, check, test, host smoke, red-team, evaluation-validation, and clean-tree gates.

- **`lefthook` pre-commit** runs `format:dprint`, `format:go`, `format:python`, eleven globbed checks, then `secure:staged`; **pre-push** runs `check:core` before `test`. Priorities keep formatting ahead of checks and the staged scan.

<!-- lefthook-tasks: pre-commit=format:dprint,format:go,format:python,check:format,check:go,check:python,check:shell,check:workflows,check:docs,check:links,check:skills,check:data,check:release-metadata,check:licenses:core,secure:staged; pre-push=check:core,test -->
...
- Carry the page frame: an "In one glance" block after the H1, one of three closing headings, and the collapsible, link-label, and snippet rules that keep every page reading as one course ([8.4. Documentation]({{< relref "/8. Community/8.4. Documentation.md#what-does-the-structural-checker-enforce" >}}) lists them all).
...
{{% collapsible note "Deeper: why a config block pasted into a page drifts from its source" %}}

dprint formats embedded TOML/JSON/YAML/Dockerfile blocks _inside_ Markdown, so a hand-pasted config excerpt can drift from the source's exact bytes on the next `format` — and a drifted excerpt on a page teaching that source is worse than no excerpt. Prefer a `--8<--` snippet include that pulls verbatim from the real file at build time, and only paste an excerpt when it is already in the formatter's canonical form. {{% /collapsible %}}
...
Continue to [4.2. Testing]({{< relref "/4. Quality/4.2. Testing.md" >}}) when `mise run check:core` is green and you can justify every suppression in the repository in one sentence each.
