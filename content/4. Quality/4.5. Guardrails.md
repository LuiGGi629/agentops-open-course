...
## The fence, and why closing it counts as an attack

Neutralizing a known phrase is the tripwire. The thing that still holds when the tripwire misses is structural: untrusted free text is wrapped in delimiters that tell the model this region is data, and the delimiters are two exported constants rather than a convention:
...
Run the log line from the top of this page through it and both halves are visible at once — the phrase replaced, and the whole thing fenced:
...
Three rules in that code are worth stating, because none of them is obvious from reading it once.
...
## One plugin, attached once, in a fixed order
...
## Your turn: find the payload the tripwire misses, and prove the fence still holds

The file's own header comment concedes the limit: a pattern list catches known payload shapes and regressions, and nothing more. So go and find one it misses, and check that the answer is not "the agent is now compromised" but "the fence is doing its job unaided".
...
- **Preflight**: `git diff --quiet -- agents/go/policy/guardrails_test.go`, and `mise run redteam` green before you start.
...
Two passing assertions in one test is the point. The first says the tripwire missed, which is expected and not a bug. The second says it did not matter, because the payload arrived inside a fence that says "this is data" — and that is the property you would want to still hold on the day someone writes a payload nobody has seen.
...
- You wrote a payload the pattern list misses and proved the spotlight fence carries it anyway.
