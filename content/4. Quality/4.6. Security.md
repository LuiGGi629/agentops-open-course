...
- Session, incident, audit, trace, metric, and offline evaluation stores retain sensitive evidence.
...
flowchart LR
    ...
    otel[OTLP collector →<br>Tempo / Loki / Prometheus]
...
**Diagram in words:** Every arrow into the agent carries a control, and every arrow out of it carries one too. The client reaches the agent only through agentgateway, the model only proposes, tool and skill text is hardened before it reaches the model, writes reach SQLite only through typed arguments and an audited transaction, and telemetry leaves with content capture off. The single OTLP export fans out inside the collector to Tempo, Loki, and Prometheus, so no signal needs its own credential or its own route out of the agent.

The honest gap is the leftmost edge. On the default course path the A2A listener is **unauthenticated**, and the caller identity is a synthetic, context-bound id — see [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md" >}}) — not a human. Authentication is an opt-in deployment choice ([5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md" >}})), not a built-in guarantee.
...
{{% collapsible note "Deeper: the other ten cases in the suite" %}}
...
1. `test_a_foreign_tool_named_load_skill_does_not_inherit_the_trust_carve_out` — a foreign tool registered under the name `load_skill` stays data-hardened, because the carve-out is keyed on the ADK `LoadSkillTool` type, not on a string. {{% /collapsible %}}
...
See [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}) for the input-validation and untrusted-output callbacks these controls build on.

{{% collapsible note "Deeper: what the Kubernetes path adds (Chapter 6 preview)" %}}
...
| BYO agent pod             | Gateway (MCP reads + model), collector over OTLP      | Raw MCP server, the trace and log stores, or anything outside the cluster |
...
| otel-collector            | Tempo, Loki, gateway metrics                          | Model and tool backends                                                   |
| MCP server / Tempo / Loki | Nothing outbound but DNS                              | Any pod they do not serve (ingress-only contract)                         |
| GKE service accounts      | Vertex via Workload Identity Federation               | Long-lived keys — there are none to steal                                 |
...
Vanilla Kubernetes NetworkPolicy cannot allow an FQDN. The local model exception therefore permits any IPv4 destination on `:11434`, and GKE permits any IPv4 destination on `:443` for agentgateway alone. Workload Identity IAM narrows which Google APIs accept its credentials, but it does not prevent HTTPS exfiltration to another host. Production needs an egress proxy, firewall, or network layer with FQDN-aware policy.

That the exception covers one pod and not the telemetry stack is worth naming, because it is a design choice rather than luck: Tempo and Loki both keep their blocks on a PersistentVolumeClaim, so neither overlay needs an object-storage exception for the stores that hold your traces and logs. Those pods stay confined to `default-deny-egress` plus DNS, and agentgateway is the only workload in the namespace with a public HTTPS route at all. {{% /collapsible %}}
...
{{% collapsible note "Deeper: how infrastructure secrets are handled later" %}}
...
Infrastructure secrets take a different path: SOPS with age encrypts only the `data`/`stringData` values of Kubernetes Secret manifests (`encrypted_regex: ^(data|stringData)$`) so kind and metadata stay reviewable in diffs. The committed recipient is a demo public key whose private half is gitignored; learners run `infra/scripts/secrets.sh keygen` and swap in their own — see [6.5. Platform Gateway]({{< relref "/6. Platform/6.5. Platform Gateway.md" >}}). On GKE, Workload Identity Federation authenticates to Vertex and GCS with no service-account keys to leak. {{% /collapsible %}}
...
{{% admonition warning "What a red gate looks like" %}}

Both Trivy passes run with `--exit-code 1`, so a finding at the gate's severity stops the task instead of scrolling past. The report summary then carries a count instead of a `0` next to the offending target. That is a finding to fix or to document, not a scanner to turn off. {{% /admonition %}}
...
- `mise run redteam` exits zero with every adversarial test passing and no model, key, or network involved.
...
Continue to [Gateway]({{< relref "/5. Gateway/_index.md" >}}) when every one of those four commands is green and you can state the residual risk you are accepting.
