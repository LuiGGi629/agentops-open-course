...
`mise run test` adds two things to a plain `go test`: the race detector, and a per-package line-coverage floor of 80%. The floor is what fails the build; the percentage it prints is diagnostic. New code with no test lands under the floor and fails, instead of quietly dragging a repository-wide average down by a tenth of a point where nobody notices. Coverage still measures reach rather than correctness, which [0.2. Evidence]({{< relref "/0. Overview/0.2. Evidence.md" >}}) is blunt about.
