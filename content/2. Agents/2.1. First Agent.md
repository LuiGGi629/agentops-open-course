...
The serving window that also governs speed and truncation is covered in [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md#why-can-a-long-conversation-fail-or-forget-its-start" >}}).
{{% /admonition %}}
...
Re-run `mise run doctor:model` to confirm the endpoint and that `AGENT_MODEL` is pulled, then retry. See [0.6. Troubleshooting]({{< relref "/0. Overview/0.6. Troubleshooting.md#why-cant-the-agent-reach-ollama-or-the-model-gateway" >}}) for the full checklist.
{{% /admonition %}}
...
- **Assuming the default commands run the workflow graph.** `mise run run`, `web`, and `a2a` launch the conversational agent. Use `mise run workflow` for the bounded graph described in [3.5. Workflows]({{< relref "/3. Capabilities/3.5. Workflows.md#how-do-you-run-the-planning-workflow" >}}).
{{% /admonition %}}
