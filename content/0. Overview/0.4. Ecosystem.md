...
## Three front doors, and the port each one answers on {#what-does-agentgateway-own}
...
## Two producers share one telemetry stack {#how-do-opentelemetry-and-evaluation-fit-together}
...
Treat each manifest as the authority, change one component at a time, then run the three commands that own it:
...
Continue to [0.5. Providers]({{< relref "/0. Overview/0.5. Providers.md" >}}) when every boundary on this page has one named owner.
