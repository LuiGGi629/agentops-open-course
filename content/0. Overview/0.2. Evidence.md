...
The floor's scope is narrower than it looks. Two Go modules carry it: `agents/go`, the code the course teaches from, and `evals`, the harness that drives the agent from outside and scores what comes back. `cmd/` packages are excluded by kind in both, because their behavior is exercised through subprocess tests that Go attributes to the test binary rather than to the package under test. The repository's own tooling module carries no floor, and says so in a comment.
