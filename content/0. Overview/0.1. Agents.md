...
The agent in that transcript is the reference **AgentOps Agent**, an on-call assistant for a fictional platform. Its persona and operating rules are not implied by a diagram; they live verbatim in the `INSTRUCTION` string in `compose/composition.go`, kept explicit "so behavior is reproducible and evaluable".
...
1. **Model** — selected by `build_model()` in `model/model.go`; local Ollama by default.
1. **Tools** — `ALL_TOOLS` (reads) in `tools/read.go`, `KNOWLEDGE_TOOLS` in `memory/knowledge.go`, plus `MEMORY_TOOLS` and the guarded `ACTION_TOOLS` in their own modules.
...
1. **Policy** — ADK hooks: validation and error handling from `policy/guardrails.go`, PII redaction from `policy/pii.go`, and the token budget from `policy/budget.go` — composed once by `AgentOpsPolicyPlugin` at the app boundary rather than buried in the prompt. {{% /collapsible %}}
...
| -------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool use                         | Default interactive agent         | The tool-calling loop; `ALL_TOOLS` in `tools/read.go` plus the knowledge tools in `memory/knowledge.go` ([3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}})) |
| Memory / long-term notes         | Default interactive agent         | `MEMORY_TOOLS` (`recall_incident_context`, `save_incident_note`) in `memory/notes.go` ([3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md" >}}))                 |
...
1. **Input validation is deterministic.** `normalize_slug` and `normalize_incident_id` (`domain/record.go`) parse and canonicalize a service name or incident id at the boundary — turning `inc-002` into `INC-002` — rather than trusting the model to judge what is well-formed. The guardrail layer re-runs the same functions before any write ([4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}})).
