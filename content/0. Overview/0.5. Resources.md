...
| [MLflow GenAI documentation](https://mlflow.org/docs/latest/genai/)   | The prompt-registry, evaluation, and scorer API used by the offline quality chapters.                 |
...
- The **Pinned contracts** section of `AGENTS.md` records the coordinated cross-component versions — agentgateway, kagent charts and API version, MLflow, the OTel Collector, and the stable network ports — in one place. {{% /collapsible %}}

If an external example uses a field, flag, or endpoint that none of those pins recognize, treat the example as ahead of or behind this repository, not as ground truth. When you do upgrade, follow the one-component-at-a-time procedure in [0.3. Ecosystem]({{< relref "/0. Overview/0.3. Ecosystem.md#what-should-you-verify-before-upgrading-the-stack" >}}) and re-run the gates. If a course link or command has genuinely drifted, open a documentation issue with the page, the observed output, and your platform.
