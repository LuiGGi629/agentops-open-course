...
url: "/0-overview/0-6-troubleshooting/"
---
...
| Coverage below 95% after an edit                | `cd agents/python && uv run pytest --cov=agent --cov-branch --cov-report=term-missing` | [coverage gate](#why-does-mise-run-test-fail-the-coverage-gate-after-my-edit)         |
...
Run `cd agents/python && uv run pytest --cov=agent --cov-branch --cov-report=term-missing` to collect coverage and show the uncovered lines, then add a deterministic test for each. Never lower the threshold or add `# pragma: no cover` to force green.
