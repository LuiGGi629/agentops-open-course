...
- **Time:** no reading time — this is a lookup page. Find your symptom and leave. {{% /admonition %}}

{{% admonition tip "Bookmark this page — do not read it now" %}}

Nothing here is worth reading straight through. Come back when a checkpoint fails, match your symptom in the table below, and ignore the rest. {{% /admonition %}}
...
| Configuration validation fails before startup   | `mise run config:check`                                   | [configuration error](#why-does-the-agent-fail-at-startup-with-a-configuration-error) |
...
| You need to inspect measured Go coverage        | `cd agents/go && mise run coverage`                       | [coverage report](#how-do-you-inspect-measured-go-coverage)                           |
...
flowchart LR
    ...
    E -->|environment or install| D0["mise run doctor"]
...
**Diagram in words:** Start from the failure type. Environment and installation failures use the base doctor; model, gateway, Kubernetes, and optional Google Cloud failures use their matching progressively scoped doctor profile.
...
Symptom: a task exits immediately complaining that Go, Hugo, kubectl, agentgateway, or sops is missing.
...
1. Run the doctor for the failing layer. `mise run doctor` checks only the base docs and Go entry path; use `doctor:model`, `doctor:gateway`, `doctor:platform`, or `doctor:gcp` for their additional external prerequisites.

## Why does the agent fail at startup with a configuration error?

Symptom: the agent, A2A server, or a task exits with a configuration error before doing any work.
...
Confirm the agent's resolved configuration with `mise run config:check` (from `agents/go`), which is one of the tasks that does read `.env`. To exercise a variable inside the deterministic suite, set it in the test itself or pass it inline for that one command.
...
This is the most likely failure on the required path, and it is not a bug in your setup. The shipped deadline is 60 seconds with two retries (`model_timeout_s` and `max_retries` in `agents/go/config/config.go`), sized for a warm GPU. A local Qwen3-4B on CPU takes seconds to minutes per turn — a multi-tool investigation makes several such calls, and the first one also pays the model load.
...
A GPU or Apple Silicon machine rarely needs this. Reach for it when the first agent run in [2.1. First Agent]({{< relref "/2. Agents/2.1. First Agent.md" >}}) or a model-backed evaluation fails on time rather than on content.
...
Something already owns the port. The stable network contract is fixed (MCP `:3000`, A2A `:3001`, model `:4000`, gateway metrics/readiness `:15020/:15021`, raw MCP `:8000`, raw A2A `:8080`, ADK web UI `:8002`, documentation preview `:8003`, Tempo `:3200`, Prometheus `:9090`, host Grafana `:3002`).

The two most likely local squatters are tools whose upstream defaults can use `:8000`: the ADK developer web UI and a documentation preview. This repository moves them to `:8002` and `:8003` so neither can shadow raw MCP. Use the repository tasks rather than a bare upstream command.
...
Run the race-enabled test task first, then render its coverage profile:
...
The report helps identify unexercised code, including error branches. `mise run test` already failed if any package fell below its 80% line-coverage floor, so use this report to find _which_ lines are unexercised rather than to discover that some are; whether the exercised lines behave correctly is still decided by tests, race detection, formatting, linting, and explicit repository contracts.
...
{{% collapsible note "Deeper: why must I remove generated secrets even when the scan passes?" %}}
...
Git ignores those paths, while staged/full-history gitleaks protects the repository boundary. Tear generated material down using each lab's command, and never force-add it. `git status --ignored --short` can confirm what remains; a green scanner never authorizes keeping a private key indefinitely. {{% /collapsible %}}

{{% collapsible note "Deeper: why does the docs check reject my new page?" %}}

Symptom: the docs check fails on a page you edited.

Every course page is an FAQ with a fixed frame: it must start with `---` and a `description:` front-matter line, carry an "In one glance" block before the first `##`, contain at least one `##` heading with every `##` ending in `?`, and close on one of the three standard headings. The check also rejects machine-specific absolute paths and stale registry hostnames. Read the failing line it prints — it names the page and the rule — and fix that one thing. {{% /collapsible %}}
...
Continue to [0.7. Glossary]({{< relref "/0. Overview/0.7. Glossary.md" >}}) when this page is bookmarked, because nothing on it needs reading before something breaks.
