...
- **Time:** no reading time — this is a lookup page. Bookmark it now; about 18 minutes if you read it end to end later. {{% /admonition %}}

## Which components form the course stack?

Eight layers, one owner each. ADK runs the agent, agentgateway carries the traffic, kagent runs it on Kubernetes, Ollama serves the model, OpenTelemetry moves the signals into Tempo, Loki, Prometheus, and Grafana while the agent runs, and MLflow keeps the evaluation record afterwards.
...
| Traces and logs    | [Tempo](https://grafana.com/oss/tempo/) and [Loki](https://grafana.com/oss/loki/)                          | Trace store and log store, cross-linked in both directions.         |
...
| Offline evaluation | [MLflow](https://mlflow.org/docs/latest/genai/)                                                            | Prompt versions, evaluation runs, and judge scorers — never online. |
...
flowchart TD
    ...
    Loki -. logs to trace .-> Tempo
```

**Diagram in words:** The ADK application calls agentgateway for both tool and model traffic; the gateway forwards to Ollama on the host and k3d profiles, or to Vertex AI on GKE. kagent reconciles the same application as a Kubernetes workload without the application knowing. Separately, the application exports OTLP to the collector, which fans traces into Tempo, metrics into Prometheus and Grafana, and logs into Loki — and Tempo and Loki point at each other, so a trace opens its logs and a log line opens its trace. MLflow is absent because it is never in this path; it is the offline evaluation workspace covered further down.

The model plane is owned by [0.4. Providers]({{< relref "/0. Overview/0.4. Providers.md" >}}); the rest of this page walks each remaining plane in turn.
...
1. **The application does not know its observability backend.** It emits OTLP and stops there. Tempo, Prometheus, and Loki are wiring in the collector, not imports in the agent. That boundary is what let the course swap its trace store without touching a line of agent code.
...
The concrete code is owned by [2.1. First Agent]({{< relref "/2. Agents/2.1. First Agent.md" >}}) and [2.2. Models]({{< relref "/2. Agents/2.2. Models.md" >}}): how the root agent is constructed, how the model is built, how tools and callbacks attach. This page only fixes ADK's boundary.
...
The policy details behind each listener are owned by [5.0. Gateway]({{< relref "/5. Gateway/5.0. Gateway.md" >}}) and [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md" >}}).
...
| `:3200`              | Tempo                                    | Trace store and query API                            | Ch. 7     |
| `:3100`              | Loki                                     | Log store keyed by trace id                          | Ch. 7     |
| `:5050`              | registry.localhost                       | k3d local image registry                             | Ch. 6     |
| {{% /collapsible %}} |                                          |                                                      |           |

When two of these collide with something already listening, [0.6. Troubleshooting]({{< relref "/0. Overview/0.6. Troubleshooting.md" >}}) walks the `ss -ltnp` and gateway-status checks that free them.
...
The application stays responsible for its logic and sessions; kagent is responsible for translating those custom resources into a running Kubernetes workload. That clean split is exactly why the same container runs without kagent during host development. The full walkthrough is owned by [6.0. Platform]({{< relref "/6. Platform/6.0. Platform.md" >}}).

## Why use both OpenTelemetry and MLflow?

Because they work at different times. OpenTelemetry is the **online** path: it moves what the running agent produces into Tempo, Prometheus, and Loki. MLflow is the **offline** path: prompt versions, evaluation runs, and judge scorers, opened by the evaluation harness and never by the running agent. They are not redundant, and since the split they no longer overlap at all.
...
1. Traces to Tempo (`otlp_http/tempo` at `tempo:4318`, read back over Tempo's query API on `:3200`).
...
Grafana then reads all three, so no signal needs a UI of its own. Traces and logs also reference each other, which is the part worth learning: in [`infra/observability/grafana/datasources.yaml`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/observability/grafana/datasources.yaml) the Tempo datasource carries a `tracesToLogsV2` link into Loki, and the Loki datasource carries a `trace_id` derived field back into Tempo. Both directions resolve only because the agent stamps `trace_id` and `span_id` onto every log record it writes — correlation is a property the application pays for, not one the backend invents.

The trade is a good one on both sides. The collection path lost a Python service that existed only so a trace could be read, and gained a store the OTLP exporter talks to natively; MLflow lost nothing, because prompts, runs, and scorers were always evaluation-time concerns rather than request-time ones. Neither side hides behind a hosted SaaS account: Tempo, Loki, Prometheus, Grafana, and MLflow are all open source and all run without one.

The reproducibility side (prompts, runs, logged models) is owned by [7.0. Reproducibility]({{< relref "/7. Observability/7.0. Reproducibility.md" >}}) and tracing by [7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}}); this page only names the boundary and the fan-out.
...
One OTLP export becomes three things: a trace you can open, metrics you can alert on, and logs you can correlate by trace id. [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}) walks that correlation end to end.

{{% collapsible note "Deeper: how the collector splits one export into three signals" %}}
...
flowchart TD
    ...
    Traces --> TempoExp["otlp_http/tempo to tempo:4318"]
...
**Diagram in words:** One OTLP receiver accepts everything the agent and gateway send on `:4317`/`:4318` and splits it into three pipelines. The traces pipeline forks: one branch exports to Tempo over OTLP/HTTP, the other feeds the `span_metrics` connector, whose output re-enters the metrics pipeline alongside the counters the agent reported directly. The metrics pipeline leaves through the Prometheus exporter on `:8889`, and the logs pipeline leaves for Loki.

Because traces feed both Tempo and the span-metrics connector, one turn becomes a trace you can open, a set of RED metrics you can alert on, and a stream of logs you can correlate by trace id — from a single OTLP export. [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}) walks that three-pillar correlation end to end. {{% /collapsible %}}
...
On the host Compose profile these surface on well-known loopback ports — Prometheus on `:9090`, Grafana on `:3002`, Tempo on `:3200` — while the Kubernetes overlays keep them as ClusterIPs behind port-forwards. Every alert is grounded in a metric the stack verifiably exports, and none routes to an external pager.

Read the names as a preview, not as homework. [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md" >}}) owns the alert semantics, the dashboard, and the alert-response runbooks.
...
Continue to [0.4. Providers]({{< relref "/0. Overview/0.4. Providers.md" >}}) when you can name the tool that owns each boundary without scrolling back up.
