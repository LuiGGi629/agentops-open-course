...
- **You need:** [5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md" >}}) finished, with `mise run mcp:http` and `mise run gateway:host` still running.
...
**Key term:** [CEL]({{< relref "/0. Overview/0.7. Glossary.md#cel" >}}) expresses the route's boolean authorization rules.
...
{{% admonition info "Where these rules go next" %}}

Naming the CEL layer matters, because it is the exact seam Chapter 5.5 extends: once a verified identity exists, the same rules gain a `jwt.sub` term (`jwt.sub == "ops-admin" && mcp.tool.name == "search_service_logs"`) and become per-caller — same mechanism, one more clause. {{% /admonition %}}
...
async def main() -> None:
    async with streamable_http_client("http://127.0.0.1:3000/mcp", terminate_on_close=False) as (read, write, _):
        async with ClientSession(read, write) as session:
            await session.initialize()
            result = await session.call_tool("restart_service", {"name": "api"})
            print(result)
...
- **The script cannot reach `:3000` at all.** That is the gateway itself being down rather than a policy decision. Start it again with `mise run gateway:host` ([5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md" >}})).
- **Every call returns `500` with `-32603` when you did not stop anything.** The gateway is up and the MCP server is not. Restart `mise run mcp:http`; that response is `failClosed` working, not a bug. {{% /admonition %}}
...
Tighten the CEL rules once and every current and future caller of the [data plane]({{< relref "/0. Overview/0.7. Glossary.md#data-plane" >}}) is bound by them; there is no code path where a new client can quietly acquire a seventh tool. That is the argument the allowlist encodes: policy defined once at a boundary cannot be forgotten by the next integration.
...
{{% collapsible note "Deeper: the Host check underneath the allowlist" %}}
...
Owned by [`mcp_client.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/mcp_client.py); the same excerpt is build-checked into [3.3. MCP]({{< relref "/3. Capabilities/3.3. MCP.md" >}}).
...
The k6 load-test scenario defaults to 60 tool calls per minute, comfortably under 120, so a single `429` fails the run and tells you the request rate crossed the gateway budget instead of surfacing real backend latency. {{% /collapsible %}}
...
Continue to [5.3. A2A Gateway]({{< relref "/5. Gateway/5.3. A2A Gateway.md" >}}) when the gateway refused the seventh tool without the MCP server ever being asked.
