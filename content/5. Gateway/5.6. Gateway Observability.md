...
1. **Then go inside the application.** The agent's own trace in MLflow, its Loki log lines, and the persisted audit state ([7.6. Governance]({{< relref "/7. Observability/7.6. Governance.md" >}})) cover everything behind the proxy — and only exist for requests the gateway actually forwarded.
