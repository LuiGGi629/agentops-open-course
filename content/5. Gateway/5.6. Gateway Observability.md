...
1. **[OTLP]({{< relref "/0. Overview/0.7. Glossary.md#otlp" >}}) gateway traces** in the k3d and GKE profiles only, whose in-cluster collector is part of the deployment. OTLP is the OpenTelemetry wire protocol a process uses to push telemetry to a collector.
...
That exact `true` is risk acceptance, not a redaction switch. Pinned ADK Go v2.1.0 always serializes tool arguments and results on `execute_tool` spans and may place raw error text in exception events. Its end-of-span data has no stable application filtering seam.
