...
url: "/5-gateway/5-6-gateway-observability/"
---
...
With the host gateway from [5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md#how-do-you-start-the-host-data-plane" >}}) running, send the prompt-guard curl from [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-does-the-prompt-guard-work" >}}). The email address in the prompt is what trips the guard:
...
`agentgateway_guardrail_checks_total` is the only place where a policy decision becomes a number. Its `phase` label separates the request guard (`phase="Request"`, the email/override regex, status 400) from the response guard (`phase="Response"`, status 502) configured in [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-does-the-prompt-guard-work" >}}). Without it, a rejection is indistinguishable from a client-side bug: both look like a 400 to the caller.
...
**Diagram in words:** The gateway writes metadata-only request logs to container stdout. Its metrics reach a loopback host port and Compose Prometheus directly over their shared scoped bridge, then Prometheus feeds Grafana. The host gateway sends no OTLP traces; only an explicitly configured agent sends OTLP HTTP to the collector, which stores traces in Tempo. Grafana reads both stores, so metrics and traces share one UI.

The dashed edge is the deliberate gap: on the host, gateway spans do not exist, and only the agent's own spans reach Tempo. Do not spend an afternoon looking for a gateway hop in a host trace.
...
Review every exporter and its retention before pointing this stack at real data. Where application log records go is owned by [7.2. Monitoring]({{< relref "/7. Observability/7.2. Monitoring.md#where-do-my-agent-logs-go" >}}).
...
1. **Then go inside the application.** The agent's own trace in Tempo, its Loki log lines, and the persisted audit state ([7.6. Governance]({{< relref "/7. Observability/7.6. Governance.md" >}})) cover everything behind the proxy — and only exist for requests the gateway actually forwarded. Trace and logs are not two searches: Loki's OTLP endpoint promotes each log record's trace id to a `trace_id` field, and Grafana's provisioned datasources link both directions on it, so a span jumps to its log lines and a log line jumps back to its span.
...
1. Tempo shows the host application's trace; the Kubernetes profile later adds gateway traces.
...
- Tempo holds the agent's own trace, and no gateway span — an absence you can now explain.
