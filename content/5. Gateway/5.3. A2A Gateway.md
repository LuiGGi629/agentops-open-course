...
Rewriting that address is this listener's main job. Here you find the field the gateway changed, learn why a hostile browser origin still gets a `200`, approve a guarded write from a browser and read the audit row it left, and separate the bounds that apply to one task. Ask the gateway for the card:
...
One row, naming the caller, the reason, the session and the invocation. That row is the evidence, not the badge flipping to `completed`: a screen can render anything, and only the row is still there after you close the tab.
