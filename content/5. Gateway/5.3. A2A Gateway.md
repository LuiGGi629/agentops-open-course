...
Run the black-box A2A evaluation only when a configured model is intended:

```bash
mise run eval:a2a
```

The harness consumes protocol events through the governed endpoint, excludes partial text from final-answer scoring, and writes `evals/a2a-results.json`. The release-bearing REST artifact remains `evals/eval-results.json`.
