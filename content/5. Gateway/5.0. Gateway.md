...
The general pattern that fixes this is the **reverse proxy**: put one process in the request path, in front of shared backends, and move the cross-cutting traffic concerns into it. A control enforced once at a shared boundary cannot be forgotten by the next client — the same argument [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#could-an-optional-managed-classifier-screen-prompts-here-instead" >}}) uses for tool allowlists and rate limits.
...
The model side is the smallest possible change: this chapter swaps only `OPENAI_BASE_URL` to point at the gateway listener. Owned by [5.4. Model Gateway]({{< relref "/5. Gateway/5.4. Model Gateway.md#what-is-the-application-contract" >}}).
...
Any term on this page you do not recognize is defined in one line in [0.8. Glossary]({{< relref "/0. Overview/0.8. Glossary.md" >}}).

{{% collapsible note "Deeper: where a service mesh would fit" %}}

The **control plane** is the out-of-band system that _configures_ the data plane: it computes routes, pushes policy, and discovers backends, often dynamically over a protocol like xDS while the data plane keeps serving. Service meshes ship both; a mesh sidecar is a data-plane proxy fed by a control plane. {{% /collapsible %}}
...
The host profile is a **single replica** with a per-instance token-bucket rate limit: a fixed allowance of requests that refills on a timer. [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#why-is-the-local-rate-limit-not-a-quota" >}}) is explicit that this is not a distributed quota. Do not read "gateway" as "mesh"; the value here is a readable boundary and its policy, not dynamic fleet management.
...
This is a course-scope decision, not a universal ranking. Envoy AI Gateway is a reasonable comparison when a platform already operates Envoy Gateway and needs its LLM/MCP capabilities.

## Which named gateway owns each protocol?
...
Separate ports keep routing unambiguous: a catch-all rule can send one protocol to the wrong backend. That is why the smoke test asserts each listener by protocol, not by a bare TCP connect. The last two rows are not in the config files at all — the host wrapper's render step injects `statsAddr`/`readinessAddr`, covered in [5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md#how-do-you-verify-the-listeners" >}}).

Those ports come from named gateways and top-level routes in `infra/agentgateway/host/config.yaml`, and every later page in this chapter reuses that vocabulary. Reading the relevant keys top to bottom:

1. `gateways` — the named entry ports. This course declares `mcp:3000`, `a2a:3001`, and `llm:4000`.
1. `routes` — top-level forwarding rules, each attached to one named gateway through its `gateways` list.
...
The named gateways and head of the MCP route make the relationship concrete:

{{< include path="infra/agentgateway/host/config.yaml" region="host-mcp-gateway" lang="yaml" >}}

The `mcpAuthorization` and `backends` blocks follow under that same top-level route (an MCP target with `failureMode: failClosed`); Chapter 5.2 owns what those policies decide.
...
Owned by [5.2. MCP Gateway]({{< relref "/5. Gateway/5.2. MCP Gateway.md#how-is-the-mcp-route-secured" >}}) and [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#which-controls-are-active-in-every-profile" >}}).
...
Chapter 5.5 owns the caller-auth details ([5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-do-callers-authenticate-to-the-gateway" >}})); Chapter 5.6 explains why the host profile leaves gateway OTLP off ([5.6. Gateway Observability]({{< relref "/5. Gateway/5.6. Gateway Observability.md#why-does-the-host-gateway-expose-metrics-but-not-traces" >}})). {{% /collapsible %}}
...
The full wrapper walkthrough is [5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md#what-does-the-host-wrapper-actually-run" >}}).
...
1. MCP tool authorization and a fail-closed backend — [5.2. MCP Gateway]({{< relref "/5. Gateway/5.2. MCP Gateway.md#how-is-the-mcp-route-secured" >}}).
1. Per-listener request rate limits — [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#why-is-the-local-rate-limit-not-a-quota" >}}).
1. A2A protocol-aware forwarding — [5.3. A2A Gateway]({{< relref "/5. Gateway/5.3. A2A Gateway.md#how-does-the-a2a-request-path-differ-from-mcp" >}}).
1. Model request/response prompt guards — [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-does-the-prompt-guard-work" >}}).
1. Upstream model authentication at the deployment-identity boundary — [5.4. Model Gateway]({{< relref "/5. Gateway/5.4. Model Gateway.md#how-is-local-qwen3-configured" >}}) and [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#how-is-cloud-authentication-separated" >}}).
1. Structured access logs, metrics, and tracing — [5.6. Gateway Observability]({{< relref "/5. Gateway/5.6. Gateway Observability.md#which-gateway-signals-are-available" >}}).
...
1. **Authenticated tool context.** The policy plugin's `BeforeToolCallback` is `Policy.ValidateActions` from `policy/guardrails.go`, and `Tools.validatedApproval` in `tools/action.go` reads the approval off the ADK `agent.Context`. The gateway has no session to read.
1. **Human confirmation.** Both mutating tools are built with `functiontool.Config{..., RequireConfirmation: true}` in `tools/tools.go`, so ADK pauses for a person before the handler runs. A rate limit or allowlist cannot express "a human said yes to this exact action."
1. **Write-plus-audit as one transaction.** `Store.RestartServiceWithAudit` and `Store.ResolveIncidentWithAudit` in `data/write.go` perform the mutation and record who approved, why, and what changed in the same transaction, so a crash cannot commit one without the other. {{% /collapsible %}}
...
Chapter 4.5 owns all three ([4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md#how-are-tool-arguments-constrained" >}})); this page only draws the line.
...
Treat this chapter as learning the mechanism and the boundary, not as a hardened production edge. Owned by [5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md#which-security-controls-are-intentionally-absent" >}}), which lists exactly what is deliberately absent.
...
Then run `mise run smoke:host` as the deterministic proof that the composed data plane behaves ([5.1. Gateway Setup]({{< relref "/5. Gateway/5.1. Gateway Setup.md#what-exactly-does-the-host-smoke-prove" >}})), and hold every later chapter check to gateway ports only.

**You are done when:**

- You can say which of `:3000`, `:3001`, and `:4000` carries MCP tools, A2A clients, and OpenAI-compatible model calls.
- You have opened `infra/agentgateway/host/config.yaml` and traced `gateways` → a route's `gateways` attachment → `policies` → `backends`.
