...
1. **Authenticated tool context.** `AgentOpsPolicyPlugin.before_tool_callback` calls `validate_actions` from `policy/policy.go`, and `_validated_approval` in `tools/action.go` reads the approval off the `ToolContext`. The gateway has no session to read.
...
1. **Write-plus-audit as one transaction.** `restart_service_with_audit` and `resolve_incident_with_audit` perform the mutation and record who approved, why, and what changed in the same transaction, so a crash cannot commit one without the other. {{% /collapsible %}}
