...
- **You need:** Chapters 1-4 finished and [1.2. Containers]({{< relref "/1. Setup/1.2. Containers.md" >}}) verified with Docker running.
- **Time:** about 45 minutes, hands-on. {{% /admonition %}}

## Start four processes in the order that works {#how-do-you-start-the-host-data-plane}

Four processes, and the order is not a preference. Each one is the next one's upstream, so starting the gateway before the MCP server gives you a data plane that answers every connection with `502` and tells you nothing about why.
...
Ollama must already be running when you type that. If it is not managed as a service on your machine, leave `ollama serve` in its own terminal first — with the daemon down, `doctor:model` stops at `ollama     start Ollama: it is not answering on 127.0.0.1:11434` and never gets as far as asking which models you have. The message that names `ollama pull qwen3:4b-instruct` is the next check along and means the opposite: the daemon answered, and the tag simply is not there. `doctor:gateway` then checks the base tools plus `yq`, `docker`, and `openssl`, asks the Docker daemon whether it is alive, and confirms `docker compose` exists. That `yq` is not decoration: the wrapper rewrites the gateway config with it before every start.

Before you commit a terminal to a long-lived process, run the deterministic harness. It allocates its own ephemeral ports, so it can run while your manual stack is up and it cannot collide on `3000`:
...
That single line stands for isolated fake-model, MCP, A2A, and gateway processes, four verified gateway surfaces, a CORS preflight from an allowed origin and from a hostile one, host and container metrics scrapes, and a teardown that leaves nothing behind. Two of its habits are worth stealing: it drives the real `infra/scripts/gateway-host.sh` rather than a reimplementation of it, and it compares the MCP allowlist by equality, so a tool leaking _in_ fails as loudly as one going missing. What it does not touch is the Ollama stack you are about to start by hand — it proves the composition, not your terminals, and keeping those two claims apart is what [0.2. Evidence]({{< relref "/0. Overview/0.2. Evidence.md" >}}) is for. <a id="what-exactly-does-the-host-smoke-prove"></a>
...
Read that block once more for what is absent: no `:8000`, no `:11434`. The agent has lost the ability to name its own backends, which is the entire point of the chapter.

Do not substitute the raw `agentgateway -f ...` binary here. Its host listeners bind every interface; the wrapper is the checked loopback boundary, and the next section is what it buys you.

## Prove every listener answers {#how-do-you-verify-the-listeners}

Three requests tell you whether the data plane is real. Predict each answer before you press enter — in particular, predict which address the agent card will advertise, given that the agent behind it is bound to `:8080` and has never heard of the gateway:
...
## What the wrapper does to your machine {#the-wrapper}

<a id="what-does-the-host-wrapper-actually-run"></a>

`mise run gateway:host` is one line of `mise.toml`: `infra/scripts/gateway-host.sh run`. Everything below is that script, and it is worth reading once, because a gateway is the one process in this lab that terminates traffic from clients you do not control. The more traffic a component sees, the fewer privileges it should hold — which is why this course never tells you to run the binary directly.
...
**The config is rendered, not used as-is.** The checked-in file names `localhost` upstreams, which is correct on the host and wrong inside a container, where `localhost` is the container. Ask the wrapper what it actually mounts and diff that against the file in your tree:
...
`--user 65532:65532` with `--cap-drop ALL` means a container escape starts as an unprivileged user holding no Linux capability at all; `--read-only` stops it persisting a payload, `--security-opt no-new-privileges=true` stops a setuid binary regaining privilege, and the one writable path is a 16 MB `tmpfs` that cannot execute what is dropped into it. That block is only the hardening core; the same function then appends the network, the publishes, the read-only config mount, and the pinned image. `infra/scripts/gateway-host.sh args` prints the whole argv, one flag per line, and its last lines are where the loopback boundary becomes literal:
...
That `src` directory is a throwaway this subcommand renders into and changes on every invocation; `run` and `start` mount from your session's private runtime directory instead.

Read the publish line carefully, because it explains the one override people get wrong: the **left** side moves when a port is taken, the **right** side is fixed at `3000`/`3001`/`4000` because the named gateways declare those ports. `AGENTOPS_GATEWAY_MCP_PORT=3100` still publishes the container's `3000`; you point `AGENT_MCP_URL` at `http://127.0.0.1:3100/mcp` and nothing else moves.

**The wrapper only ever touches its own container.** It labels what it starts with `dev.fmind.agentops.host-gateway=true`; `assert_container_absent()` then refuses to start over an existing name, and `stop_managed_container()` refuses to stop anything without that label. Steal the rule for your own lab scripts: a script may only destroy what it created. A `docker rm -f agentgateway` in a helper that hits an unrelated container of the same name is a lesson nobody wants twice.

On native Linux the wrapper also starts a small relay, and there is nothing for you to run. The container reaches your machine through its Docker network's bridge address, which cannot reach services bound to `127.0.0.1` — where your MCP, A2A, and Ollama servers all sit. `tools/bin/loopback-relay` binds only the wrapper-owned bridge address and forwards to loopback, so no service has to leave `127.0.0.1` and only containers on that dedicated network can reach it. On Docker Desktop none of this applies, and the default `AGENTOPS_GATEWAY_LOOPBACK_RELAY=auto` starts nothing.

## When it breaks, the message names the layer
...
No container was touched and no port was taken. Most failures below carry that same `gateway-host:` prefix, added by the wrapper's own `die()`. The first row is the exception, and the exception is informative: it comes from the shared tool check that runs before the wrapper can say anything at all.

| Message or symptom                                                                                  | Cause                                                      | Fix                                                                   |
| --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------- |
| `missing yq: run 'mise install', then 'mise run doctor:gateway' to check the whole tier`            | `yq` is absent, so the render step cannot run              | `mise run install:platform` — the tier that ships `yq`                |
...
`infra/scripts/gateway-host.sh --help` prints its own copy of this list, minus `AGENTOPS_GATEWAY_RUNTIME_DIR`. It also has a trap worth knowing about: `main()` validates its inputs before it looks at the subcommand, so on a machine with no `yq` even `--help` exits with the missing-tool message. Read the table here when the wrapper will not start. {{% /collapsible %}}

Two more commands belong to this stack. `mise run observability:up` starts the optional telemetry stack, best before the gateway if you want complete startup traces; give the agent `OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:4318`, `OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf`, and `OTEL_SERVICE_NAME=agentops-agent`, then read Grafana on `127.0.0.1:3002` and Prometheus on `127.0.0.1:9090`. When you are done, `Ctrl-C` the foreground processes and stop the rest explicitly:
...
A foreground `run` cleans up through an `EXIT` trap, so `Ctrl-C` leaves nothing behind. A detached `start` deliberately has no such trap, which is why `stop` exists and is the only thing that removes the relay and rendered config a `start` left running. `observability:down` preserves its volumes; delete them only when [Chapter 7]({{< relref "/7. Observability/_index.md" >}}) asks you to.

## Your turn: read the config before you run it

The most common gateway bug is not a bug in the gateway. It is a policy edit that never reached the process, because the file the gateway reads is not the file you edited. Three subcommands answer "what is this really going to run?" before anything starts.
...
An hour ago the gateway was a container you were told to trust. You can now print the exact configuration it reads, the exact arguments it runs under, and the exact addresses it rewrites — before you start it.
