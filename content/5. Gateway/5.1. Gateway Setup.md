...
- **You need:** Chapters 1-4 finished and [1.2. Container Engine]({{< relref "/1. Setup/1.2. Container Engine.md" >}}) verified with Docker running.
...
Before you start the container you can print the exact configuration it reads, the exact arguments it runs under, and the exact addresses it rewrites. Nothing about it has to be taken on trust, which is the only footing on which the next four pages are worth reading.
