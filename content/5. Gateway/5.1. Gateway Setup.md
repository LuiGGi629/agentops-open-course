...
url: "/5-gateway/5-1-gateway-setup/"
---
...
Read that publish line carefully: the **left** side is what changes when a port is taken, the **right** side is fixed at `3000`/`3001`/`4000` because the config binds those. The wrapper enforces that invariant up front — `validate_inputs()` fails with `canonical config must contain exactly one MCP, A2A, and model bind` if you edit the binds. {{% /collapsible %}}
...
[`loopback-relay.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/scripts/loopback-relay.py) binds only the wrapper-owned bridge address and forwards MCP, A2A, and model traffic to loopback. Its ready-file token gives the wrapper the same ownership check it applies to the container. Only containers attached to that dedicated network can reach the relay; metrics need no relay because Prometheus shares the network and scrapes `agentops-gateway:15020` directly.
