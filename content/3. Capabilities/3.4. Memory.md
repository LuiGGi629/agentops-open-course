...
- **You need:** [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}}) finished; `mise run doctor:model` passing if you want to run the retrieval evaluation.
- **Time:** about 36 minutes, reference. {{% /admonition %}}
...
{{< include path="agents/python/src/agent/memory.py" region="get-runbook" lang="python" >}}
...
The same two functions are also two of the six read-only tools re-exposed over the governed MCP route ([3.3. MCP]({{< relref "/3. Capabilities/3.3. MCP.md" >}})), so one implementation serves the in-process agent and any external MCP client without a second copy to drift.
...
{{% collapsible note "Deeper: the high-latency runbook in full" %}}
...
The `remediation` skill's "follow that runbook's Remediation section" ([3.2. Skills]({{< relref "/3. Capabilities/3.2. Skills.md#how-is-a-skill-different-from-a-runbook" >}})) refers to the section you see here — the runbook supplies the incident-specific what, the skill the reusable how.
...
An **embedding** is a vector representation of text, so a search can match meaning instead of exact words ([0.7. Glossary]({{< relref "/0. Overview/0.7. Glossary.md#embedding" >}})).
...
{{% collapsible note "Deeper: three one-variable experiments" %}}
...
Reset state between measurements (`mise run data:reset`), and remember the gateway streaming path reports no usage — measure on the default non-streaming path. {{% /collapsible %}}
...
{{< include path="agents/python/src/agent/compaction.py" region="compact-history" lang="python" >}}
...
{{% collapsible note "Deeper: why those two properties matter" %}}
...
- **Assuming the context window is unbounded.** The instruction, tool schemas, history, and tool results all compete for one finite serving window; exceeding it causes a provider error or drops old content, depending on the serving path. {{% /admonition %}}
...
Continue to [3.5. Workflows]({{< relref "/3. Capabilities/3.5. Workflows.md" >}}) when you can name, for any fact the agent needs, which of the six stores is supposed to hold it.
