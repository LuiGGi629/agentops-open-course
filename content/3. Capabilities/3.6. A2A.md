...
Use the canonical black-box harness after the raw A2A server and gateway routes are healthy. Transport is a flag on the one evaluation task, not a separate task:

```bash
mise run eval -- --transport a2a --output a2a-results.json
```

This calls the configured model. It sends A2A requests and folds task, status, and artifact events into the same captured `Turn` that REST produces, so one scorer grades either surface. The `--output` override keeps the A2A artifact beside the REST `results.json` instead of overwriting it.
...
- Any live `mise run eval -- --transport a2a` run is identified as model-backed evidence, not an offline gate.
