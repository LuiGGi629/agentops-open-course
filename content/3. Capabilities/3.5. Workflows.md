...
Most short questions do not justify four model calls, so `mise run run` still launches the faster conversational agent.
...
1. Use plain Go when the rules are complete and no judgment is needed.
...
On the pinned ADK 2.x stack, one `workflowagent.New(workflowagent.Config{...})` graph expresses all of these shapes as edges. ADK Go also ships the `sequentialagent`, `parallelagent`, and `loopagent` packages under `agent/workflowagents`, and none of them is marked deprecated in v2.2.0; each fixes one shape over a list of sub-agents, so this repository declares its topology once as edges instead.
...
```bash
cd agents/go
go test ./compose -run Workflow
```

It asserts:

1. With `AGENT_ENTRYPOINT=workflow`, the command selects the shipped graph.
...
1. The app-wide policy plugin still surrounds every stage.
1. The plan, review, and recommendation instructions contain their explicit bounds.

The test does not prove that a model diagnoses INC-001 correctly. For that, run the evaluation harness against `workflow.evalset.json` with `--entrypoint workflow`, which drives the fixed INC-001 evidence path through a configured model. That is scheduled evidence, not a pull-request gate.
...
- **Files to touch**: `agents/go/compose/workflow.go` and `agents/go/compose/workflow_test.go` only.
- **Preflight**: require `git diff --quiet -- agents/go/compose/workflow.go agents/go/compose/workflow_test.go`.
- **Gate that proves completion**: inspect the installed API with `cd agents/go && go doc google.golang.org/adk/v2/workflow`, then make `cd agents/go && go test ./compose -run Workflow` assert the new topology, both read-only boundaries, the app-wide policy, and the unchanged stopping condition.
...
go run ./cmd/agentops-eval run \
  --evalset workflow.evalset.json --entrypoint workflow \
  --repeat 3 --min-pass-rate 0.33 --output workflow-results.json
jq -e '
  .summary.pass_rate >= .summary.minimum_pass_rate and
  .summary.minimum_pass_rate == 0.33 and
  ([.cases[].sample] | unique) == [1, 2, 3]
' workflow-results.json
```

The run is spelled out on the command line because `mise run eval` carries the four operations safety cases, and those case ids do not exist in the workflow evalset. Deterministic scorers own this proof; add `--judge` only when a judge route is configured.

The structural test owns the exact four-node author order. The live artifact deliberately excludes raw events and model content; it proves three behavior samples cleared the floor the command asked for without weakening that privacy boundary.

**You are done when:**

- The focused test exits zero with no network or model call.
- The sanitized local-model artifact records samples 1, 2, and 3 and clears its `0.33` pass-rate floor.
...
Continue to [3.6. A2A]({{< relref "/3. Capabilities/3.6. A2A.md" >}}) when you can choose between plain code, the interactive agent, and the bounded workflow for a new task.
