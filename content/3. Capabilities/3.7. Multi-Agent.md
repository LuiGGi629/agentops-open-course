...
- **You need:** Tools, workflows, and A2A from the preceding pages. No model is required for any of the checks.
- **Time:** about 30 minutes, hands-on. {{% /admonition %}}

## The agent that reads hostile text should not be the one that can act {#why-would-one-agent-become-several}
...
- **Gate that proves completion**: `cd agents/go && go test ./compose -run 'Coordinator|Delegat|Privilege' -count=1` passes with your new agent asserted in both directions, and the full `mise run test` stays green.
