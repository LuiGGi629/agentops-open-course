...
url: "/3-capabilities/3-7-multi-agent/"
---

{{% admonition abstract "In one glance" %}}

- **You will:** Run one coordinator with two specialists, and see how the tools each one lacks contain a prompt injection.
- **You need:** [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}}) and [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md" >}}) finished — the specialists' tool lists come from those pages.
...
1. `HITL confirm` is **human-in-the-loop**: a person approves before a write runs ([0.7. Glossary]({{< relref "/0. Overview/0.7. Glossary.md#hitl" >}})).
...
When the coordinator delegates, ADK _transfers control_ to the named sub-agent. That sub-agent runs in the same process and the same session and hands its result back: there is no new process and no network hop. That is exactly the boundary [3.6. A2A]({{< relref "/3. Capabilities/3.6. A2A.md" >}}) adds.
...
The specialist that _can_ act, `remediation_agent`, has no tool that can fetch raw logs, and its two actions still pause for human confirmation with a rationale ([4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}})). The coordinator holds no write tools either. Acting therefore requires an explicit delegation _plus_ an explicit approval — two capability boundaries a single injected sentence cannot cross on its own.

This is the same lesson as tool allowlists ([3.2. Skills]({{< relref "/3. Capabilities/3.2. Skills.md" >}})) applied per agent: a boundary a model could talk itself across is prose; a boundary enforced by what tools exist is policy. The offline suite pins it down:
...
Least privilege is one layer, not the whole of it; the containment story only holds because the other layers are there too. Both of those layers are owned by [4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}}).
...
Delegation here is sequential by design: remediation must not start before diagnosis. For genuinely independent steps — checking the logs of three unrelated services, say — ADK's `Workflow` graph runtime ([3.5. Workflows]({{< relref "/3. Capabilities/3.5. Workflows.md" >}})) can express parallel branches that join into a summarizing step.
...
This is not an exact token ceiling. The last admitted call can overshoot the threshold, and a provider response without usage metadata is not counted. The A2A request-level model-call cap remains the separate hard loop bound; [7.3. Costs]({{< relref "/7. Observability/7.3. Costs.md" >}}) owns the full limitation.

{{% collapsible note "Deeper: why one budget survives every transfer" %}}

The totals accumulate because the running totals live in session-scoped state — the `budget:` keys carry no `temp:` prefix, so `DatabaseSessionService` persists them ([`budget.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/budget.py)). {{% /collapsible %}}

Debugging also gets harder: a wrong answer now has three candidate authors, and the trace ([7.1. Tracing]({{< relref "/7. Observability/7.1. Tracing.md" >}})) shows hops to attribute instead of one linear tool loop.
...
If none apply, prefer one agent with well-guarded tools. Splitting across a _network_ boundary is a related but separate question of ownership, scaling, and blast radius: how much else breaks when one agent fails. That question is covered in [3.6. A2A]({{< relref "/3. Capabilities/3.6. A2A.md" >}}).
...
Continue to [Quality]({{< relref "/4. Quality/_index.md" >}}) when the tool lists, rather than the coordinator's instruction, are what you would point to as the thing that contains an injection.
