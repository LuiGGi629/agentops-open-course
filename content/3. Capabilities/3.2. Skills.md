...
- **You need:** [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}}) finished, `go test` working inside `agents/go`, and the `.env` from Chapter 1. Every check on this page is model-free; the exercise's closing gate is not, because whether your "Use when…" clause fires is a question only a model can answer.
- **Time:** about 25 minutes, hands-on. {{% /admonition %}}

## The procedure you cannot paste into every prompt
...
Now multiply. Triage, remediation, postmortems, escalation, capacity, the rollback checklist — put them all in the system instruction and every turn pays for every procedure, including the fifteen that do not apply. Instruction, tool schemas, history, and tool results share one fixed window, and procedures are the part of that budget most often wasted.

Two of Ana's procedures already ship with this course, as directories under `agents/data/skills`. No Go file imports them and no test compiles them, which makes it easy to assume nothing checks them either. Delete one line from `agents/data/skills/incident-triage/SKILL.md` and start the agent:
...
One line, exit code 1, and no model client was ever constructed. The deleted line was the `description`, and the rest of this page is largely an argument about why that one line is worth refusing to start over. Put the line back with `git restore -- agents/data/skills/incident-triage/SKILL.md` before you read on.
...
A skill library is now a directory you grow: contents you can audit, cost you can count, and malformed members that stop the process instead of failing silently in production. What you cannot check offline is whether your "Use when…" clause fires, which is why this page's last gate needs a model.
