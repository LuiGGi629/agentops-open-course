...
<a id="what-happens-when-a-tool-hangs"></a>Every blocking read runs under a configured context deadline (`AGENT_TOOL_TIMEOUT_S`, 30 seconds by default). When it expires the call returns a stable failure instead of eating the rest of the turn, and because cancellation travels through `context.Context`, the query or HTTP request underneath is genuinely canceled rather than abandoned on a thread nobody can reach.
