...
<a id="what-changes-when-you-flip-agent_mcp_url"></a>Flipping it changes the implementation location and nothing the model can see. Names and schemas are identical, the client discovers declarations at connect time, keeps only the allowlisted names, and applies the tool timeout. What the agent gives up is the local resilience wrapping: the deadline, retries, and circuit breaker around every in-process call then belong to the gateway, the trade Chapter 5 makes explicit.
