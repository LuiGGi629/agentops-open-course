...
- **You need:** [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}}) finished, plus `curl` on your PATH (the base `mise run doctor` does not check it).
- **Time:** about 30 minutes, hands-on. {{% /admonition %}}

## What problem does MCP solve?

Until [3.2. Skills]({{< relref "/3. Capabilities/3.2. Skills.md" >}}), every tool the agent had was a Python function it imported. That works precisely as long as the tool, the agent, and the framework live in one process and one language. The moment any of those differ, you are writing glue.
...
{{% admonition danger "A protocol is not a security model" %}}

MCP standardizes _how_ a tool is described and called. It says nothing about whether calling it is a good idea. Authentication, authorization, transport security, timeouts, rate limits, and schema validation remain yours to enforce.

Connecting to a third-party MCP server also means letting an untrusted party put tool descriptions into your model's prompt, which is a prompt-injection surface ([4.6. Security]({{< relref "/4. Quality/4.6. Security.md" >}})). This course keeps writes out of MCP entirely for exactly that reason, and puts a policy gateway in front of the reads.

The sharper modern risk runs the other direction: a third-party server can invoke **your** client. Sampling spends your token budget on generations you never asked for; elicitation puts a prompt of the server's choosing in front of your user. This course's client declares neither capability — no sampling handler, no elicitation handler, no roots — so a server that requests one gets a protocol error rather than a completion. Declare a capability only when you can also answer "and what happens when the server abuses it?" {{% /admonition %}}
...
Note what does _not_ cross the boundary: these are the bare functions, not the `with_resilience`-wrapped versions the in-process agent registers ([4.5. Guardrails]({{< relref "/4. Quality/4.5. Guardrails.md" >}})). Retry and deadline bounds move to the client side when the tools become remote — see [What changes when you flip `AGENT_MCP_URL`?](#what-changes-when-you-flip-agent_mcp_url) below.
...
That is the concrete form of the warning in [0.3. Ecosystem]({{< relref "/0. Overview/0.3. Ecosystem.md" >}}): two implementations can each claim MCP and still disagree on a version. The disagreement surfaces at `initialize` if you are lucky, and at a missing field three calls later if you are not.
...
{{% collapsible note "Deeper: hardening and shutdown on the HTTP transport" %}}
...
{{< include path="agents/python/src/agent/mcp_server.py" region="mcp-server-transport" lang="python" >}}

The Uvicorn `timeout_graceful_shutdown` is `AGENT_DRAIN_TIMEOUT_S` (default 10s), so a SIGTERM lets in-flight tool calls finish before the process exits rather than dropping them. Kubernetes' `terminationGracePeriodSeconds` must exceed it. {{% /collapsible %}}
...
{{% collapsible note "Deeper: the probe code, and why a fresh server stays unready" %}}
...
{{< include path="agents/python/src/agent/mcp_client.py" region="ops-mcp-toolset" lang="python" >}}
...
- **Auth is a bearer header, when set.** `AGENT_MCP_TOKEN` is a `SecretStr`; it becomes an `Authorization: Bearer …` header only when set, and the default local route sends no header. That is what a secured gateway route ([5.5. Gateway Security]({{< relref "/5. Gateway/5.5. Gateway Security.md" >}})) authenticates.
...
- **Gate that proves completion**: a test asserts the composed read surface is exactly the reviewed union — the six ops tools plus only the allowlisted external names — and that a tool the server offers but you did not allowlist is absent. Then note, in one sentence, which Chapter 5 gateway allowlist entry ([5.2. MCP Gateway]({{< relref "/5. Gateway/5.2. MCP Gateway.md" >}})) would enforce the same boundary at the data plane rather than in application code.
...
Continue to [3.4. Memory]({{< relref "/3. Capabilities/3.4. Memory.md" >}}) when you can explain why the six reads cross the MCP boundary and the two writes never do.
