...
url: "/3-capabilities/3-0-packaging/"
---
...
{{% collapsible note "Deeper: the pinning decisions behind the two lists" %}}
...
{{% collapsible note "Deeper: the full container build, stage by stage" %}}
...
Wolfi — the minimal Linux base of the runtime stage — is a _rolling_ repository that removes superseded versions, so these pins periodically stop resolving until someone refreshes them by hand: expected drift, documented in the Dockerfile itself, not a broken course. {{% /collapsible %}}
...
Continue to [3.1. Tools]({{< relref "/3. Capabilities/3.1. Tools.md" >}}) when the same import works from outside `src/` and `mise run check` is green.
