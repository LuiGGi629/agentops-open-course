...
The local overlay allows TCP `11434` to any IPv4 address because `host.k3d.internal` resolves to the Docker bridge gateway on the host, outside every pod or namespace selector. The GKE overlay allows TCP `443` to any IPv4 address (Google APIs publish no stable CIDR) plus `169.254.169.252/32` on TCP `987` and `988` for Workload Identity Federation. Every one of those exceptions lands on the gateway pod alone: Tempo and Loki keep their data on a PersistentVolumeClaim, so no telemetry pod needs an object-storage rule and no second workload holds a public HTTPS exception. Those endpoint details match [GKE's standard NetworkPolicy/Calico path](https://cloud.google.com/kubernetes-engine/docs/how-to/network-policy#network-policy-and-workload-identity); Dataplane V2 uses a different endpoint and is not enabled by this course's OpenTofu module. {{% /collapsible %}}
...
**Diagram in words:** The kagent namespace and the agent reach the gateway on MCP `:3000` and model `:4000` only. The gateway alone reaches the raw MCP server, the agent's A2A port, and the collector, and the collector alone writes to Tempo and Loki and scrapes gateway metrics. The one dashed outbound edge is the gateway's port-scoped model exception; the two crossed edges are flows the policies refuse — the agent cannot bypass the gateway to the MCP server, and the MCP server cannot reach the trace store.
...
| `otel-collector` | Tempo `:4318`, Loki `:3100`, gateway `:15020`              | `agentops` + `kagent` `:4317/:4318`    |
| `tempo`          | DNS only                                                   | collector `:4318`                      |
| `loki`           | DNS only                                                   | collector `:3100`                      |

\* The model exception is on the gateway pod only, but vanilla NetworkPolicy scopes it by port rather than hostname: any IPv4 `:11434` locally, or any IPv4 `:443` on GKE, plus WIF `:987/:988`. It is the only such exception in the namespace — both telemetry stores are file-backed and hold no cloud rule. Every pod also keeps a namespace-wide DNS allowance to `kube-system`.
