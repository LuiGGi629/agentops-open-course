...
1. `volumeMounts`/`volumes` — the [state PVC]({{< relref "/0. Overview/0.8. Glossary.md#pvc" >}}) and a small writable `/tmp`.
...
Two rewrites happen before the manifest ever reaches the controller:
...
1. The overlay resolves the model name by name, never by position. Neither overlay reaches into the agent's `env` list with an index: both declare a kustomize `replacements` entry that copies `ModelConfig/agentgateway`'s `spec.model` into the `Agent` at `spec.byo.deployment.env.[name=^AGENT_MODEL$].value`. Selecting the element by its `name` means reordering `infra/kagent/agent.yaml` cannot redirect the override onto a neighbouring variable, and a renamed variable fails the render instead of quietly setting the wrong one. `AGENT_MODEL` happens to sit at index 0 today, which is exactly why an index would be the dangerous choice: it would keep working until someone added a variable above it. Only the source value differs between profiles — the local overlay patches that `ModelConfig`'s `spec.model` to `qwen3:4b-instruct`, while the GKE overlay inherits `gemini-3.5-flash` from `infra/kagent/modelconfig.yaml`. Model backend is a data-plane change (Chapter 6.5).
...
Because the BYO agent brings its own `env` (the `OPENAI_BASE_URL`/`OPENAI_API_KEY` above), the ModelConfig does not inject the model into the BYO pod at runtime. Kustomize does copy `spec.model` into the agent's `AGENT_MODEL` at render time, which is how the two values stay identical without anyone maintaining them twice — but nothing in the running cluster reads the ModelConfig on the BYO path. The optional `incident-reader` does use it to reach agentgateway `:4000`. `apiKeySecret` references the `agentgateway-client` Secret, whose value is the non-secret SDK marker (Chapter 6.5). The local overlay patches `spec.model` to `qwen3:4b-instruct`; the GKE overlay keeps `gemini-3.5-flash` while preserving the same gateway `baseUrl`. {{% /collapsible %}}
...
- read-only root filesystem, a writable 1 Gi [RWO]({{< relref "/0. Overview/0.8. Glossary.md#rwo" >}}) state PVC at `/app/state`, and a 128 MiB `/tmp` `emptyDir`.
...
Expected: one ready agent pod, bound `agentops-agent-state`, and [ClusterIP]({{< relref "/0. Overview/0.8. Glossary.md#clusterip" >}}) `agentops-agent` on 8080.
