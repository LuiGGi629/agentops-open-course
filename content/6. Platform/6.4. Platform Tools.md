...
## The agent can no longer reach its own tools
...
**Diagram in words:** One writer, one reader, one file. The `agentops-agent` pod mounts `/app/state` read-write and commits each confirmed change as an `UPDATE` plus an audit `INSERT` in a single transaction into `incidents.db` on the `agentops-agent-state` claim; on first boot it publishes the immutable image seed into that claim atomically. The `agentops-mcp` pod mounts the same path read-only, so it can neither change the file nor create one. Its `/healthz` probe asks whether `incidents.db` is present and valid: if not, readiness returns `503` and the gateway pulls it from its backends — failing closed — and only when it is does the six read tools serve.
...
In one terminal, and leave it open — a port-forward runs until you stop it:
...
The tool server is no longer a subprocess the agent trusts because it started it: it is a separate service with its own identity, its own failure mode, and one declared caller — and the cluster, not the render, is what enforces that last part.
