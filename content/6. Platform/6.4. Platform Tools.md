...
url: "/6-platform/6-4-platform-tools/"
---
...
1. **One shared MCP surface.** The same FastMCP server backs the ADK agent and any other MCP client. [3.3. MCP]({{< relref "/3. Capabilities/3.3. MCP.md#why-does-mcp-matter-beyond-this-repository" >}}) is the general reuse argument; in the cluster the second client is kagent, below.
...
FastMCP derives each JSON schema from the same type hints and docstrings the ADK function tools already carry; that `add_tool` loop is quoted in [3.3. MCP]({{< relref "/3. Capabilities/3.3. MCP.md#which-tools-does-the-course-server-expose" >}}). These six are the exact set the gateway allowlist re-lists in [5.2. MCP Gateway]({{< relref "/5. Gateway/5.2. MCP Gateway.md#why-re-list-tools-the-mcp-server-already-restricts" >}}).
...
FastMCP is constructed with `TransportSecuritySettings(enable_dns_rebinding_protection=True, …)`, and [3.3. MCP]({{< relref "/3. Capabilities/3.3. MCP.md#which-transports-are-supported" >}}) quotes the full constructor. The allowlist it enforces is not hard-coded: `mcp_server.py` reads it from the `MCP_ALLOWED_HOSTS` environment variable, and the deployment sets exactly the authorities the gateway forwards. That is both the short and the fully-qualified name of `agentgateway` and of `agentops-mcp`, each with and without a port.
