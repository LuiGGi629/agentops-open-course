...
## One volume holds every piece of evidence {#what-state-would-you-lose-if-the-pvc-died}
...
`infra/scripts/platform-backup-drill.sh` is what the Platform workflow runs against its disposable k3d deployment, and it is the one script in this chapter that mutates a running cluster. It validates every argument, context, private directory, running build identity, loopback URL, and Docker authentication boundary before seeding anything; then it creates uniquely named, ownership-labelled Jobs and a ConfigMap, captures all three original replica values, stops the agent and MCP workloads, restores the snapshot, returns every replica authority to its exact prior value, and deletes only its own temporary resources. Run it only on a disposable course cluster, from a clean source revision, with the release-identity environment the workflow exports. Its exit trap stops owned Jobs and restores the captured replica values, and it prints `platform backup restore drill verified` only after cleanup succeeds.

Every argument is required, and the shape below is the exact one the Platform workflow runs — a test asserts that this page and that workflow agree, so the invocation cannot drift out of the prose. The revision it is given comes from the same source-identity command every image build uses, asked for one field at a time:
...
The reader image is pinned by digest rather than tag for the same reason every other image in this course is: the drill's whole claim is that it restored the bytes it snapshotted, and a mutable tag would let the thing doing the reading change underneath that claim. {{% /admonition %}}
