...
- Message-content telemetry capture disabled by default ([5.6. Gateway Observability]({{< relref "/5. Gateway/5.6. Gateway Observability.md#are-prompts-stored-in-traces" >}})).
...
{{< include path="agents/python/Dockerfile" region="base-images" lang="dockerfile" >}}

{{< include path="agents/python/Dockerfile" region="runtime-base" lang="dockerfile" >}}
...
{{% collapsible note "Deeper: why the venv's python symlinks are repointed" %}}
...
The image contains a read-only seed. Kubernetes mounts the 1 Gi [RWO PVC]({{< relref "/0. Overview/0.7. Glossary.md#pvc" >}}) `agentops-agent-state` at `/app/state`.
...
{{% collapsible note "Deeper: verify a published release image yourself" %}}

`cosign` is not part of the pinned toolchain in `mise.toml`. Install it yourself if you want to run these commands.

A published release provides one image, `ghcr.io/mlops-courses/agentops-open-course/agent` — the single entry in `scripts/container-matrix.json` — and signs its public index digest keyless with cosign. GitHub OIDC binds a short-lived certificate to the workflow on protected `main` and the exact source commit, so there is no signing key to leak. A separate `verify` job then re-checks the published signature and attestation from scratch, so the workflow does not merely assert its own output:
...
Continue to [6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}) when the image you built passes its own scan and stays healthy as a non-root, read-only container.
