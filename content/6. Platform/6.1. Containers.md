...
## Suppose someone gets a shell in your agent pod {#how-do-you-build-the-local-image}

They will not get one. There is no shell in this image — no `sh`, no package manager, no `curl`, no compiler, no module cache. The whole runtime is one static binary, a read-only dataset, and an empty writable directory, running as UID 10001. An attacker who reaches code execution inside it inherits a very short list of things to do next.

That property is not a claim; it is something you can measure in about thirty seconds. Build the image and interrogate it:
...
The binary also knows exactly which source it came from, and will tell you:
...
## Two stages, and nothing from the first survives {#why-use-two-stages}

The build stage carries the pinned Go toolchain and module cache, and compiles exactly one executable:
...
## Scan the artifact, not the recipe {#how-do-you-scan-the-exact-artifact}
...
The `version` output at the top of this page said `unknown+dirty` because the tree had uncommitted edits, and a development build was happy to record that. Now ask the same tree for a _release_ build. Predict first: does it build anyway and label the result dirty, or does it refuse — and which of the two would you want the night someone asks which commit is in production?
...
Notice where that refusal happened: in `mise`, before Docker was invoked at all, because `source-identity` resolves the working tree first and will not hand a release build a revision. Untracked counts, which surprises people — a stray scratch file is exactly the kind of thing that ends up baked into an image nobody can reproduce later.
...
An hour ago, "the image" was a build artifact you trusted because the build succeeded. You can now open it, read what it claims about its own origin, check that claim against the tree on your disk, and say why cgo is off, why both bases carry digests, and why one binary serves three roles.
