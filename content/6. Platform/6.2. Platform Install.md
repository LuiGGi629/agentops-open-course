...
## A half-installed cluster is worse than no cluster

The failure this page is designed around is not "the install did not work". It is "the install half worked": k3d has claimed Docker resources, a registry container exists, the CRDs are partly established, and now every later error you hit is a symptom of something you did twenty minutes ago. Backing out of that costs more than starting from nothing.

So the install is a sequence of fail-fast gates, each one a `mise` task backed by a checked-in config or a guarded script, so every learner gets the same topology instead of an ad-hoc pile of flags:
...
That is the whole argument for a preflight in five lines. The pinned k3s image needs the unified cgroup hierarchy, and the machine finds out now rather than after `k3d cluster create` has half-built a node. `scripts/doctor.sh platform` checks the same way for a missing tool, a non-executable gateway wrapper, an unavailable Docker daemon, and the Helm diff plugin, which reports as `helm       helm-diff 3.15.10 ready` when it is installed.

A green run adds two `capacity` lines reporting this machine's RAM and free disk, and ends with a line about the cluster you have not created yet: `cluster    not created yet; run mise run cluster:start when needed`. The doctor only ever reports your kubectl context; it never creates anything. The exact tool inventory and who owns each tool live in [1.3. Kubernetes]({{< relref "/1. Setup/1.3. Kubernetes.md" >}}); the split worth carrying here is that the doctor names two different remedies. For the host tier it tells you to install from a reviewed package source — `git`, `curl`, and a working C toolchain (`cc`, `make`, `install`, `tar`) at the base, plus Docker and `openssl` for the gateway profile and Ollama for the model one. For everything else, including every platform binary this chapter uses, the remedy is `mise run install:platform`.

The cluster itself follows the same principle one step further: reproducible clusters come from a config under review, not from remembered flags. `mise run cluster:start` runs `scripts/cluster-start.sh`, which creates or resumes cluster `local` from `infra/k3d.yaml`:
...
That config turns off three pieces of convenience networking k3d and k3s enable by default: a Traefik ingress controller, the servicelb handler that fakes `LoadBalancer` services, and a load-balancer container fronting the API server. They exist for people who want a public-ish cluster in one command. This course brings its own hardened agentgateway and publishes nothing, so those extras would be dead weight at best and a second, conflicting ingress path at worst.
...
One more choice is worth naming because it looks like an omission: the topology declares one server node and no agent node. The `Agent`, the MCP Deployment, and the backup Job all mount the same ReadWriteOnce claim with no affinity, and k3s's local-path provisioner pins the volume to whichever node created it — so a second schedulable node turns [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) into a coin flip between "runs" and "volume node affinity conflict". One node also frees a few hundred megabytes. This is a learning substrate, not a production edge; do not add an Ingress here expecting the manifests to publish it.

{{% collapsible note "Deeper: how cluster:start stays safe to re-run" %}}

A start task you cannot safely re-run is a foot-gun during a lab. `scripts/cluster-start.sh` checks that the Docker daemon is up, lists the existing clusters and registries as JSON, and branches on what it finds: a running cluster with its registry means nothing to do; a stopped one is resumed with `k3d cluster start local`; an absent one is created from the tracked config.
...
## Install the control plane, not the demo fleet
...
Because `helmDefaults` waits, a clean apply already implies readiness — but verify it independently rather than trusting an exit code:
...
kagent       kagent      oci://ghcr.io/kagent-dev/kagent/helm/kagent@sha256:ec0dacc1a76edbd190a554757c8bdb193ccb0b35deeb35f6d7a7e7ffc76d99fd
kagent-crds  kagent      oci://ghcr.io/kagent-dev/kagent/helm/kagent-crds@sha256:85174e69eab19e05fcf82dbfda86e8e84c2be97a52c645d60cf1ae51ccbca977
```

Notice the `VERSION` column is not there, because it is empty: a digest-addressed release has no version string for helmfile to print, which is precisely why the reviewed chart version `0.9.12` lives in a comment and the digest lives in the release. That listing also reads the local state file rather than the cluster, so it proves what you declared, not what is running — `kubectl -n kagent get pods` is the one that proves the second thing. If a CRD is missing, the `kagent-crds` release did not apply before the controller; re-run `mise run platform:install`, which is safe to repeat.

Install deliberately stops here. It does not deploy the agent — the controller now has nothing to reconcile in `agentops`, which is the expected post-install state.

## The CRD refuses a misspelled field, and you can prove it offline

Establishing a CRD teaches the API server a shape. The cheapest way to feel what that buys is to hand it something almost right. `infra/kagent/fixtures/invalid-modelconfig-field.yaml` is a committed `ModelConfig` with `provider` misspelled as `provder`, and validating it needs no cluster at all — this is an offline command:
...
It exits 1 and the useful fragment is `at '/spec': additional properties 'provder' not allowed`. The full line is quoted inline rather than as a capture because it embeds a `file://` URI carrying your own checkout path, which is not reproducible. Note what the refusal is _not_: not a typo check, not a linter. `additionalProperties: false` in the schema means the API server has an exhaustive list of what a `ModelConfig` may contain, so a field it does not know is a field it rejects — which is the same guarantee whether it arrives from your editor, from a generator, or from a controller.

The other half of an exhaustive schema is that you can read what you are _not_ using. This course sets `description`, `type`, and `byo` on its `Agent`, and the exercise fixture adds seven `declarative` fields. These six are on the API surface and deliberately unused here:
...
One dependency has to be satisfied before the workloads start: pods must be able to reach your model. Ollama's default loopback listener is not reachable from inside k3d, so bind it to the cluster's Docker bridge instead. In one terminal:
...
Skaffold builds exactly one image and applies one overlay. Everything else in the stack — the gateway, Tempo, Loki, the collector — runs an upstream image pinned by digest, so the only thing your source edits rebuild is the agent:
...
The card's `name` field comes back as `AgentOps Agent` — the same discovery document you read on your laptop in [2.4. Sessions]({{< relref "/2. Agents/2.4. Sessions.md" >}}), now served by a pod, through a gateway, over a port-forward you started yourself.

**Ana's agent is now running somewhere that will bring it back at 04:00 without you** — provided the process actually exits. [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md#how-is-the-pod-hardened" >}}) opens by making you delete that pod to watch the replacement arrive, and is equally plain about the edge of the promise: the pinned BYO schema declares no kubelet probes, so a process that is wedged while still holding its socket is a case nothing in this cluster notices. Keep Skaffold running: the next three pages explain and verify the resources you just deployed.

## Your turn: point the install at nothing

`mise run platform:install` is three commands in a row — an assertion about your current context, a `kubectl apply` of the namespace, and the helmfile apply. Predict what happens if the first one cannot answer: does the task create the namespace and fail later on the chart, or stop before it touches anything?
...
- **Goal**: see for yourself that the install's first act is a question about which cluster you are talking to, not a change to it.
...
The failing line is printed above the error, which is the useful part: you can see that it was the assertion and not the chart, so there is no half-installed release to reason about. That is the difference between a task that is safe to re-run and a task that is safe to run at all — the half-installed cluster this page opened with never gets a chance to exist.
...
Every piece of the cluster you now have came from a file you can read, and the agent inside it answers on a port nothing else can reach. The next page is the one file that says what that agent is allowed to be.
