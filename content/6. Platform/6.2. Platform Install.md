...
The exact platform-tool inventory and ownership live in [1.3. Kubernetes]({{< relref "/1. Setup/1.3. Kubernetes.md#which-tools-are-required" >}}). `git`, `curl`, and Docker remain host prerequisites; mise installs the pinned project toolchain.
...
See [`k3d.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/k3d.yaml). Each choice earns its place:
...
This is a learning substrate, not a production edge: do not add an Ingress here expecting the manifests to publish it. [1.3. Kubernetes]({{< relref "/1. Setup/1.3. Kubernetes.md" >}}) makes the same argument from the Setup side, before any cluster exists.
...
{{% collapsible note "Deeper: how cluster:start decides what to do" %}}
...
See [`cluster-start.sh`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/scripts/cluster-start.sh). A guard is a diagnosis, not permission to delete either shared resource. {{% /collapsible %}}

{{% admonition danger "Never auto-delete a mismatched shared cluster" %}}
...
flowchart LR
    ...
    subgraph agentopsns["agentops namespace (data plane)"]
        ...
        Workloads["gateway, MCP, Tempo, Loki, OTel, agent Deployment"]
    end
    Controller -->|"watch + reconcile"| CRs
    CRs --> Workloads
```

**Diagram in words:** The controller and its bundled Postgres live in the `kagent` namespace. They watch the custom resources in `agentops` and reconcile them into the workloads that serve traffic there. Control flows one way only: nothing in `agentops` reaches back into `kagent`.
...
The upstream kagent chart is a demo distribution: it ships a fleet of built-in agents (`k8s`, `kgateway`, `istio`, `promql`, `observability`, `argo-rollouts`, `helm`, and several `cilium` agents), the `kmcp` and `kagent-tools` add-ons, `grafana-mcp`, `querydoc`, and a web UI. On a shared cluster every extra Deployment is footprint you must schedule, patch, and defend for no course value — it is attack surface, not capability. {{% /collapsible %}}
...
{{% admonition danger "On Linux the rebind collides with the Ollama service, and it is not loopback" %}}

The Linux installer ships a systemd unit that already owns `:11434` on loopback ([1.4. Providers]({{< relref "/1. Setup/1.4. Providers.md" >}})), so the `ollama serve` above fails with `bind: address already in use`. Pick one of two paths, and undo it after the lab:
...
{{% admonition danger "A macOS rebind can expose Ollama to your network" %}}
...
{{% collapsible note "Deeper: why macOS needs a different bind" %}}

Docker Desktop and Colima put the container engine inside a VM. k3d injects `host.k3d.internal` for pod-to-host access, but Ollama binds loopback by default. {{% /collapsible %}}

## What does Skaffold deploy?

Skaffold builds one image, pushes it to the local registry, renders the local Kustomize overlay, and applies it. Everything else in the stack — the gateway, Tempo, Loki, the collector — runs an upstream image pinned by digest, so the only thing your edits rebuild is the agent.
...
`mise run platform:dev` resolves the working tree through the source-identity tool, then starts Skaffold's watched loop. A dirty tree receives an `unknown+dirty.<digest>` binary/OCI identity and a valid `development-<tree-digest>` image tag, so it never impersonates `HEAD`. The Dockerfile still rejects any missing identity field because Skaffold renders an unset template as the literal string `<no value>` and otherwise builds on happily.
...
{{% admonition danger "Do not run this during the chapter" %}}
...
Remove the course workload with Skaffold before the Helm release. Chapter 6.6 owns that teardown sequence because deleting the controller and CRDs first would leave an invalid BYO deployment. {{% /admonition %}}
...
Continue to [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) with Skaffold running, because the next page explains the agent resource you can now inspect.
