...
url: "/6-platform/6-2-platform-install/"
---
...
See [`cluster-start.sh`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/scripts/cluster-start.sh). A guard is a diagnosis, not permission to delete either shared resource. {{% /collapsible %}}
...
**Diagram in words:** A source change rebuilds the agent image, tags it with the abbreviated commit SHA, and pushes it to the local registry. Skaffold then renders the local overlay, applies it, and watches for the next change, closing the loop.

`mise run platform:dev` derives `AGENT_SOURCE_COMMIT` from `HEAD`, then starts Skaffold's watched loop. The build refuses that value's absence, so an image cannot lose its revision label. The refusal lives in the Dockerfile rather than in `infra/skaffold.yaml`, and the reason is worth knowing: Skaffold renders an unset `{{.AGENT_SOURCE_COMMIT}}` as the literal string `<no value>` and builds on happily, so a guard in the build file is the only place that sees what actually landed.
