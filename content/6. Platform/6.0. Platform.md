...
You verify this directly in [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}): delete the agent pod and watch the controller rebuild it against the same state volume. The same loop is why you never patch the Deployment kagent generates — reconciliation will overwrite your edit, so config changes go through the `Agent` resource.
...
1. An alpha API. `v1alpha2` from a CNCF Sandbox project can change between releases, and the pinned BYO schema exposes _fewer_ knobs than a Deployment: no container probes, no termination grace period, which is why the image ships its own `/livez` and `/healthz` ([6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}})).
...
kagent watches an `Agent` custom resource. In a **BYO Agent** you ship the container image and kagent only schedules it ([0.8. Glossary]({{< relref "/0. Overview/0.8. Glossary.md#byo-agent" >}})). For `type: BYO`, kagent creates/manages the Deployment and Service for the course's own A2A image. The course also registers an OpenAI-compatible `ModelConfig` and a `RemoteMCPServer` that both point at agentgateway.
...
The controller's blast radius is deliberately small. [`kagent/values.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/kagent/values.yaml) scopes it to one namespace, so it never reconciles resources it should not:
...
{{% collapsible note "Deeper: how Skaffold finds the image inside a custom resource" %}}

One detail matters for the build loop: Skaffold does not edit the pod template but rewrites the logical image reference _inside_ the custom resource, taught to look at `.spec.byo.deployment.image` in [`skaffold.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/skaffold.yaml):
...
[6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}) starts Skaffold after the control plane is ready. {{% /collapsible %}}

Owned by [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) for the probe split, and [6.4. Platform Tools]({{< relref "/6. Platform/6.4. Platform Tools.md" >}}) for the separate MCP deployment.
...
| ------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Agent` (`agents.kagent.dev`)                     | The BYO agent workload: image, replicas, env, security context, state PVC | [`agent.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/kagent/agent.yaml) · [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}})             |
| `ModelConfig` (`modelconfigs.kagent.dev`)         | The OpenAI-compatible model endpoint kagent consumers use                 | [`modelconfig.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/kagent/modelconfig.yaml) · [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) |
| `RemoteMCPServer` (`remotemcpservers.kagent.dev`) | The governed MCP endpoint (via agentgateway), not the raw service         | [`toolserver.yaml`](https://github.com/MLOps-Courses/agentops-open-course-go/blob/main/infra/kagent/toolserver.yaml) · [6.4. Platform Tools]({{< relref "/6. Platform/6.4. Platform Tools.md" >}})     |

You can list the established CRDs after install ([6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}) shows the exact `kubectl get crd` command).

{{% collapsible note "Deeper: how mature is the v1alpha2 API?" %}}

Be honest about maturity: `v1alpha2` is an alpha API from a CNCF Sandbox project, so schemas can change between releases and some fields you would want (BYO container probes) simply do not exist yet. That is a real constraint the readiness discussion in [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) works around, not a bug — pin the chart version and expect churn. {{% /collapsible %}}
...
The host profile from [Chapter 5]({{< relref "/5. Gateway/_index.md" >}}) gave the agent none of these; you were its supervisor, its firewall, and its config manager. In the cluster those roles become declared objects the platform enforces even while you are asleep.
...
The diagram's **[state PVC]({{< relref "/0. Overview/0.8. Glossary.md#pvc" >}})** uses the [RWO]({{< relref "/0. Overview/0.8. Glossary.md#rwo" >}}) access mode so one node mounts it for writing at a time.
...
**Diagram in words:** The kagent controller and its Postgres sit in the control-plane namespace and reconcile the BYO Agent into the `agentops` data plane. There the agent reaches its tools and model only through agentgateway, writes to a state PVC the MCP server reads back read-only, and sends telemetry to the collector, which fans it to Tempo and Loki. Only the gateway leaves the cluster, and only a port-forward comes in.
...
They are named here and explained by [6.5. Platform Gateway]({{< relref "/6. Platform/6.5. Platform Gateway.md" >}}). The map from each cluster piece to the page and manifest that stands it up:

| Sub-page                                                                          | Cluster piece it stands up                     | Owning manifest(s)                                                         |
| --------------------------------------------------------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------- |
| [6.1. Containers]({{< relref "/6. Platform/6.1. Containers.md" >}})               | The non-root agent OCI image                   | `agents/go/Dockerfile`                                                     |
| [6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}})   | k3d cluster, registry, kagent, Skaffold loop   | `infra/k3d.yaml`, `infra/helmfile.yaml`, `infra/kagent/values.yaml`        |
| [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}})     | BYO `Agent`, `ModelConfig`, state PVC          | `infra/kagent/agent.yaml`, `infra/kagent/modelconfig.yaml`                 |
| [6.4. Platform Tools]({{< relref "/6. Platform/6.4. Platform Tools.md" >}})       | `agentops-mcp` Deployment, `RemoteMCPServer`   | `infra/k8s/base/mcp.yaml`, `infra/kagent/toolserver.yaml`                  |
| [6.5. Platform Gateway]({{< relref "/6. Platform/6.5. Platform Gateway.md" >}})   | agentgateway, network policies, quota, secrets | `infra/k8s/base/agentgateway.yaml`, `infra/k8s/base/network-policies.yaml` |
| [6.6. Platform Delivery]({{< relref "/6. Platform/6.6. Platform Delivery.md" >}}) | state recovery, optional GKE plan, teardown    | `infra/scripts/`, `infra/gcp/`                                             |
...
The base deploys Tempo, Loki, and the OTel collector; the `local` overlay adds Prometheus and Alertmanager. Only Grafana and the host Compose profile belong to [Chapter 7]({{< relref "/7. Observability/_index.md" >}}).
...
The host profile ([Chapter 7]({{< relref "/7. Observability/_index.md" >}})) also uses gateway readiness `:15021`. {{% /collapsible %}}
...
- agentgateway, the MCP server, Tempo, Loki, and the OTel collector.
...
- `k8s/overlays/gke`: applies a Workload Identity patch, pins every persistent volume to the `agentops-standard` StorageClass, and includes the GKE agentgateway config with its Vertex egress exceptions.
...
**Skaffold**, the build-and-deploy loop [6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}) starts, selects the overlay with `-p local` or `-p gke` and tags images with the abbreviated Git commit.

The network-policy exceptions each overlay appends are explained in full by [6.5. Platform Gateway]({{< relref "/6. Platform/6.5. Platform Gateway.md" >}}). Here the point is that a provider swap is a data-plane patch, never an application change.
...
A commit tag improves provenance but remains a mutable registry reference. A production promotion policy should deploy a verified image digest and preserve its build/SBOM/signature evidence — the signing and verification path [6.1. Containers]({{< relref "/6. Platform/6.1. Containers.md" >}}) documents for tagged releases.
...
Required drill — the [Chapter 6 checkpoint]({{< relref "/6. Platform/_index.md#what-proves-this-chapter-worked" >}}) asks for its result. Change two values, one in the shared base and one in the local overlay, then let the renders tell you which environment each change reached. No cluster, no image, and no model are involved.
...
- **Files to touch**: `infra/k8s/base/mcp.yaml`, raising the MCP container's memory limit from `512Mi` to `640Mi`; and `infra/k8s/overlays/local/kustomization.yaml`, changing its single `qwen3:4b-instruct` patch value to another tag you have actually pulled — the `replacements` entry carries it into the agent's `AGENT_MODEL` for you.
...
Then inspect the diff: model backend, identity annotations, and persistent-volume storage class should change; application ports, read-tool route, and A2A image contract should not. Drop the `>/dev/null` to read either render.
