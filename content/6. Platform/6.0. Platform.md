...
kagent watches an `Agent` custom resource. In a **BYO Agent** you ship the container image and kagent only schedules it ([0.7. Glossary]({{< relref "/0. Overview/0.7. Glossary.md#byo-agent" >}})). For `type: BYO`, kagent creates/manages the Deployment and Service for the course's own A2A image. The course also registers an OpenAI-compatible `ModelConfig` and a `RemoteMCPServer` that both point at agentgateway.
...
The diagram's **[state PVC]({{< relref "/0. Overview/0.7. Glossary.md#pvc" >}})** uses the [RWO]({{< relref "/0. Overview/0.7. Glossary.md#rwo" >}}) access mode so one node mounts it for writing at a time.
