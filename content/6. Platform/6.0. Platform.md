...
[6.2. Platform Install]({{< relref "/6. Platform/6.2. Platform Install.md" >}}) starts Skaffold after the control plane is ready.
{{% /collapsible %}}
...
Be honest about maturity: `v1alpha2` is an alpha API from a CNCF Sandbox project, so schemas can change between releases and some fields you would want (BYO container probes) simply do not exist yet. That is a real constraint the readiness discussion in [6.3. Platform Agents]({{< relref "/6. Platform/6.3. Platform Agents.md" >}}) works around, not a bug — pin the chart version and expect churn.
{{% /collapsible %}}
...
The host profile ([Chapter 7]({{< relref "/7. Observability/_index.md" >}})) also uses gateway readiness `:15021`.
{{% /collapsible %}}
