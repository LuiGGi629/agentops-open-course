...
Step 2 runs the model-backed evaluation from the standalone Go harness. Its result records the source revision and dirty state, model identity, evalset digest, transport, deterministic scores, content-free usage, and the pass rate, requested minimum, and required-case verdict. A failing run or a later source change stops before any command is emitted. Only green model-backed evidence plus a clean render prints a source-guarded Skaffold command.
