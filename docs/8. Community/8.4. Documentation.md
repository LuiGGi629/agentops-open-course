...
!!! tip "Start the preview before you read"
    ...
    Run `mise run serve` now and keep the preview open at http://localhost:8000 while you read. Every rule below is about something you can see there.
...
The generator is Zensical, from the Material for MkDocs authors, pinned exactly in the root `pyproject.toml`:
...
Zensical is pre-1.0, so its rendering can shift between releases. The exact pin plus `uv.lock` freezes the whole build environment. An upgrade is therefore a deliberate bump: rebuild and visually re-inspect the entire site, never a floating dependency that changes output silently under you.

`mise run serve` runs `zensical serve` for a live-reload preview. `mise run build` writes the static site under `site/`.
...
1. Every collapsible summary starts with `Deeper:`, no page carries more than four, no link label is a bare page number, and every `--8<--` snippet include sits inside a code fence.

The first three keep a page readable on its own. The rest keep 76 pages readable as one course: a learner who has built the reflex on page one should not have to rebuild it on page forty.
...
You can see this working in the deepened chapters: [2.1. First Agent](../2. Agents/2.1. First Agent.md) embeds the `root-agent` region from `agent.py`, and [3.4. Memory](../3. Capabilities/3.4. Memory.md) embeds the `get-runbook` region from `memory.py`.
...
Four commands cover the whole authoring loop:
...
`mise run check:links` is a separate, offline, scoped gate. It runs **lychee** — a link checker — over an explicit file set: `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `CHANGELOG.md`, and the `docs`, `agents`, `infra`, and `skills` Markdown trees. Offline means it validates repository-local relative links and anchors without reaching the network. It never flakes on a slow external host, and it never silently passes a broken internal link.
...
- The preview at http://localhost:8000 shows every page you changed, with its diagrams, tables, admonitions, and internal links rendering as you meant them.
...
Continue to [8.5. Contributions](./8.5.%20Contributions.md) when both gates pass on a page you edited yourself.
