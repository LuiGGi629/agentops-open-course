...
This repository's first stable release is `v1.0.0`. That one name covers four things at once:

- A `v`-prefixed Git tag on the commit that shipped it.
- The dated `## [1.0.0] - 2026-07-30` section in `CHANGELOG.md`.
...
The repository is `1.0.0`, so its repository-owned course, application, state, protocol, and deployment contracts follow the compatibility policy in [`SUPPORT.md`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/SUPPORT.md). Root `pyproject.toml`, `agents/python/pyproject.toml`, and `CITATION.cff` must agree for a release.
...
A `v`-prefixed tag such as `v1.0.0` clearly distinguishes a version tag from other Git refs. GitHub and release tooling widely understand the convention. Choose one convention and keep package metadata, changelog headings, tag, and release title consistent.
...
mise run check:release-metadata v1.0.0
```

Pass the tag the first command prints into the second. Together they verify that the latest dated entry in `CHANGELOG.md` matches the newest `v`-prefixed tag (currently `v1.0.0`) and that `pyproject.toml`, `agents/python/pyproject.toml`, and `CITATION.cff` all carry that version.

A consistent repository prints `release metadata: v1.0.0 (2026-07-30) is consistent`. A drifted one names the mismatch and exits non-zero, for example `release metadata: tag v2.0.0 does not match source version v1.0.0`.

Any new release requires explicit maintainer intent and the full gate above.

**You are done when:**

- `git tag --list 'v*' --sort=v:refname | tail -1` prints `v1.0.0`, the same version as the newest dated `CHANGELOG.md` heading.
- `mise run check:release-metadata v1.0.0` prints the `is consistent` line and exits zero.
