...
Self-hosted Renovate needs one repository secret, `RENOVATE_TOKEN`, and the built-in `GITHUB_TOKEN` cannot stand in for it. A **PAT** is a personal access token: a scoped credential issued to a user account.

??? note "Deeper: which scopes `RENOVATE_TOKEN` needs"
