...
`scan.yml` runs the security surface: a repository job runs `mise run secure` — the git-history and filesystem scanners that the contributor-facing `mise run scan` wraps — over full Git history (gitleaks plus Trivy filesystem vulnerability, secret, misconfiguration, and license scans), and a matrix job builds the agent and MLflow images and Trivy-scans them (vulnerabilities and secrets at HIGH/CRITICAL, licenses at UNKNOWN/HIGH/CRITICAL). It also runs weekly.
