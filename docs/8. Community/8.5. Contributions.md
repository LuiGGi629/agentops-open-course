...
!!! abstract "In one glance"
    ...
    - **You need:** A fork cloned, then `mise install` and `mise run install` finished.
...
The first command installs the repository's pinned tools; the second installs the docs and agent dependencies and enables the Git hooks (`lefthook install`).
...
| You, before opening a pull request | `mise run scan`          | The whole repository: it runs `mise run secure`.                                             |
...
`ci.yml` runs the shared gate — `mise run install`, `format`, `check`, `test` — and then adds four named **merge-gate** steps: checks that must pass before a pull request can be merged. Naming them separately makes one kind of regression its own signal instead of one line inside a long run.
...
- **Image job** — builds the agent and MLflow images and Trivy-scans them: vulnerabilities and secrets at HIGH/CRITICAL, licenses at UNKNOWN/HIGH/CRITICAL.
...
- **Files to touch**: the one page under `docs/` you are fixing. Break one of its rules on purpose first — an H2 that no longer ends in `?`, or a link to a page that does not exist — read the failure, then `git restore` that break and keep only your real fix. `scripts/check_conventions.py` holds every page rule the checker enforces.
...
Continue to [8.6. AAIF](./8.6.%20AAIF.md) when your local gate is green and every remaining line of the diff is one you can explain.
