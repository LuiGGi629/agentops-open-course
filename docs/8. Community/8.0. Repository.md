...
Everything the course produces lives in one repository, [`MLOps-Courses/agentops-open-course`](https://github.com/MLOps-Courses/agentops-open-course). Five directories carry it:
...
Those three usually live in three separate repositories. A course that teaches an agent's whole lifecycle keeps them in one place, so a reader can jump from a sentence to the exact file that backs it. Everything else at the root is the tooling that binds them.

??? note "Deeper: every file at the repository root"
    ...
    ├── GOVERNANCE.md        # maintainership, decisions, review, and access
    ├── ACCESSIBILITY.md     # keyboard, contrast, diagram, and reporting contract
    ...
    ├── scripts/             # repo-level gates: check_conventions.py, doctor.sh, … (see scripts/README.md)
    └── .github/             # ci/docs/eval/scan/release/freshness workflows, dependabot, templates
    ```
...
The dual-license split — CC BY 4.0 prose, MIT code — is the subject of [8.1. License](./8.1. License.md).
...
- When [Chapter 3](../3. Capabilities/index.md) teaches a tool, the exact tool lives in `agents/`.
- When [Chapter 6](../6. Platform/index.md) deploys the agent, the exact manifests live in `infra/`.
...
`lefthook.yml` makes the binding literal: `pre-commit` formats affected markup and Python sources, then runs `mise run check:core` and `mise run secure:staged`; `pre-push` runs `mise run test`. Add a payload or a gate and you extend the vocabulary once, in `mise.toml`, and every consumer inherits it.
...
Inside `agents/python/src/agent/` there is almost no package hierarchy to navigate: twenty-five flat modules, each owning exactly one concern.
...
| Guarded writes and safety             | `actions.py`, `guardrails.py`, `pii.py`, `resilience.py`, `circuit.py` |
...
??? note "Deeper: the one exception to the flat layout"

    The lone exception is the tiny `structured_report/` subpackage: a five-line ADK-discovery adapter that re-exports `triage_report_agent` as `root_agent` so the report eval gets its own discoverable entrypoint ([3.0. Packaging](../3.%20Capabilities/3.0.%20Packaging.md#how-is-the-project-organized)).
...
The full module-to-chapter map is tabulated in the [3. Capabilities index](../3. Capabilities/index.md), and [8.7. Capstone](./8.7. Capstone.md) reuses it as the change surface. So when you build your own domain you edit the same flat boundaries rather than inventing new ones.
...
- **Python excerpts are pulled from the real source at build time.** They are not copy-pasted prose that can rot. A standalone `pymdownx.snippets` line of the form `--8<-- "agents/python/src/agent/memory.py:get-runbook"` inlines the exact `get-runbook` region from `memory.py`. That is the line behind the excerpt in [3.4. Memory](../3. Capabilities/3.4. Memory.md).
- **Rename or delete a region and the site build fails.** The page and the source cannot silently diverge.
...
The FAQ format contract, the snippet-mirroring, and the docs CI that enforce all of this are the subject of [8.4. Documentation](./8.4. Documentation.md).
...
It is a vendor-neutral repository convention adopted by multiple coding tools, not an AAIF protocol. This course teaches the convention in [1.5. Workspace](../1. Setup/1.5. Workspace.md) and uses it as a worked example; who governs it is owned by [8.6. AAIF](./8.6. AAIF.md).

The rest of this chapter walks through the other things that turn a repository into a shareable, sustainable project:

- its [license](./8.1. License.md)
- its [releases](./8.2. Releases.md)
- reusable [templates](./8.3. Templates.md)
- its [documentation pipeline](./8.4. Documentation.md)
- the [contribution](./8.5. Contributions.md) workflow
- the [AAIF](./8.6. AAIF.md) it supports
...
Continue to [8.1. License](./8.1.%20License.md) when you can point at the directory that owns a claim before you look for the page that made it.
