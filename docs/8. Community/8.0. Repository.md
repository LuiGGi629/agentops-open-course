...
??? note "Deeper: the one exception to the flat layout"
    ...
    The lone exception is the tiny `structured_report/` subpackage: an ADK-discovery adapter that re-exports `triage_report_agent` and builds its governed `App`, so the report eval gets its own policy-protected entrypoint ([3.0. Packaging](../3.%20Capabilities/3.0.%20Packaging.md#how-is-the-project-organized)).
