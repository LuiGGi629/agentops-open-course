...
Open `http://127.0.0.1:8000`, ask one read-only question, and save the provider, model, prompt, Events trajectory, and result as your baseline evidence. Stop the server with `Ctrl+C`.
...
Milestone 1 is not a commit-sized change, and its entry gate deserves an honest number. Measure it in your own clone before you plan anything:
...
Measured on the committed seed on 30 July 2026: 16 of the 33 files under `agents/python/tests/` carry domain literals — 131 `INC-*` identifiers, 146 service names, 20 runbook slugs. Two structural facts make that count bite:
...
1. **The seed.** Rewrite `sql/schema.sql` and `sql/seed.sql`, then `mise run build` and `mise run check`. Everything below reads this, so a seed that changes twice costs you the work twice.
1. **The domain constants.** Give the literals one owner instead of sixteen files: a `tests/domain.py` exporting the identifiers, service names, and slugs your assertions need, imported everywhere they were typed. The reference does not ship that seam — adding it during the port is what makes the _next_ domain change a one-file edit.
...
1. **The evidence.** `agents/python/evals/ops.evalset.json` and its siblings carry the domain in their prompts and expected trajectories. They move last, because until the seed is stable their expected answers are not.

Step 1 alone turns most of those sixteen files red. That is the suite telling you precisely which assertions were about the domain rather than about the behavior — which is the information you need, and the reason to change the seed first.
...
- **Files to touch**: `agents/data/sql/seed.sql` only — rename one incident id, for example `INC-002` to `INC-902` — then `cd agents/data && mise run build`, because the tests read the generated database and not the SQL.
- **Gate that proves completion**: `cd agents/python && uv run pytest -q` now fails, and the failures name the identifier you changed rather than an unrelated stack. Then `git restore agents/data`, rerun `mise run build`, and confirm `uv run pytest -q` and `mise run check:data` are both green again — the committed database back byte-for-byte.
...
1. **Bound the domain.** The largest milestone by far: it replaces the seed, the identifier patterns, and the domain literals in roughly half the test suite, in the order the porting section above sets out. _Done when_ your own sanitized seed passes `mise run check:data`, no `INC-*` literal survives under `agents/python/`, and `mise run test` is green again at the unchanged 95% branch floor. Document one useful user outcome plus one case where deterministic code is better than an agent.
