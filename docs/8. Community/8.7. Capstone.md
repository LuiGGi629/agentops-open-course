...
One worked pivot makes the boundaries concrete. To turn the reference into a developer-support agent, thread a single capability through the layout above: seed `agents/data/sql/` with `ticket` rows instead of the `INC-*` incidents, add a typed `get_ticket` read tool in `tools.py` mirroring `get_incident` (validate the id, cite provenance), re-list `get_ticket` in the MCP allowlist so the gateway will pass it, and add one case to `agents/python/evals/ops.evalset.json` that exercises the new trajectory. That is one read capability crossing data, tools, gateway policy, and evidence — the same four boundaries every milestone touches.
