...
- Generator path only: a grep of each rendered tree returns no `agentops-agent`, no `agentops-open-course`, and no seed fixtures from `agents/data/`.

Continue to [8.4. Documentation](./8.4.%20Documentation.md) when you have decided, in one sentence, whether you are forking this repository once or maintaining a generator.
