...
!!! tip "On the local model path? You can skip this page for now."
    ...
    Skip ahead now. When you reach Chapter 5, come back, run `docker info`, then `mise run doctor:gateway`.
...
Continue to [1.3. Kubernetes](./1.3.%20Kubernetes.md) when `mise run doctor:gateway` reaches your engine — or right away, if you are on the local model path and will return at Chapter 5.
