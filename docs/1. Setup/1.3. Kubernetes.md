...
!!! abstract "In one glance"
    ...
    - **You need:** Nothing from this chapter — skip the page entirely until Chapter 6 if you are on the local model path.
...
One command is the whole action on this page, and it creates nothing:
...
The check covers the container engine, the pinned platform CLIs, the project virtualenvs, and the `helm-diff` plugin. It applies no **manifest** (a YAML file describing one Kubernetes object) and creates no cluster. It fails on exactly five things:
...
If it names a missing tool, `mise install` provides the pinned binaries.
...
There is deliberately no ingress node in that picture. A **Service** is the stable in-cluster address Kubernetes gives a set of pods. The only path from your host to one is a temporary `kubectl port-forward`, which tunnels a local port into the cluster.

??? note "Deeper: the whole k3d.yaml, line by line"
    ...
    See [`infra/k3d.yaml`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/k3d.yaml). The four values the visible list does not cover:
...
`mise install` provides every tool below, at the exact version the root `mise.toml` pins.

| Tool         | Version | What it does here                                                    |
| ------------ | ------- | -------------------------------------------------------------------- |
| k3d          | 5.9.0   | Runs the k3s cluster nodes as containers on your machine.            |
| kubectl      | 1.36.3  | The command-line client that talks to the Kubernetes API.            |
...
| Kustomize    | 5.8.1   | Renders the shared manifests through the `local` or `gke` overlay.   |
...
| agentgateway | 1.3.1   | The Chapter 5 gateway binary, also used to validate gateway configs. |

Two easy-to-miss requirements complete the set: `doctor:platform` also needs the Docker engine reachable and the Helm `helm-diff` plugin at 3.15.10 (installed by `mise run install`). Use the container engine verified in [1.2. Containers](./1.2.%20Containers.md).
...
Continue to [1.4. Providers](./1.4.%20Providers.md) when `mise run doctor:platform` passes, or right now if you are on the local model path.
