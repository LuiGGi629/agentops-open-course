...
[`infra/k3d.yaml`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/k3d.yaml) is the complete source. It also pins k3s, creates one server plus one worker, binds the API to loopback, and waits up to 120 seconds for readiness.
...
Two easy-to-miss requirements complete the set: `doctor:platform` also needs the Docker engine reachable and the Helm `helm-diff` plugin at 3.15.10 (installed by `mise run install:platform`). Use the container engine verified in [1.2. Containers](./1.2.%20Containers.md).
