...
The fields above already default to this path, so configuration is one install and one pull, not four variables to type.
...
sh ollama-install.sh   # equivalent one-liner: curl -fsSL https://ollama.com/install.sh | sh
```
