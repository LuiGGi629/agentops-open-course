...
!!! abstract "In one glance"
    ...
    - **You need:** The 1.1. Python checkpoint passing (`mise run check` and `mise run test` from `agents/python`).
...
Both branches use the same three settings, with the same defaults: `AGENT_MODEL_TIMEOUT_S=60`, `AGENT_MAX_RETRIES=2`, and `AGENT_RETRY_BACKOFF_S=0.5`.

Neither branch accepts SDK defaults for timeouts and retries; both map those three resilience settings onto their client. Guarded write actions — the two tools that require human confirmation — are never retried; this policy covers idempotent model calls only, meaning calls that are safe to repeat.
...
`agents/python/mise.toml` splits its tasks in two:

- **Tasks that load the root `.env`**, each with `redact = true`: `config:check`, `run`, `web`, `a2a`, and the `eval*` family.
- **Offline gates that load no dotenv at all**: `install`, `check`, `test`, `redteam`, `eval:validate`, and the stdio `mcp` server.

That split is deliberate: provider configuration can never change whether the deterministic suite passes.
...
The agent reads the committed seed under `agents/data`, where exactly three incidents are `open`: INC-002, INC-005, and INC-010. A correct reply names those three ids. The `list_incidents` call behind them stays invisible here, because `mise run run` prints only the text of each event; `mise run web` shows the call itself in its Events timeline. Type `exit` at the `[user]:` prompt to leave the CLI.

The first answer is slow, and that is not a hang: Ollama loads the model into memory on demand, so the first token can take tens of seconds on a CPU.

**You are done when:**

- `mise run doctor:model` exits zero and its `ollama` line reads `qwen3:4b-instruct ready on 127.0.0.1:11434`.
- The agent answered `List open incidents` with INC-002, INC-005, and INC-010; the `list_incidents` call behind that answer is visible in `mise run web`, not in `mise run run`.
...
- You left the CLI with `exit` and attempted no write action.

Continue to [1.5. Workspace](./1.5.%20Workspace.md) when your chosen provider answered `List open incidents` with the three ids from the seed.
