...
There is deliberately no second CI script that re-implements the checks. Learners and hooks run the core tasks; CI installs the maintainer tier and runs the full format, check, test, and smoke tasks. Each layer composes the same `mise` tasks instead of copying their logic into workflow YAML.
...
`mise run install` builds the docs and agent environments, then wires the hooks into `.git/hooks` with **lefthook**, the git-hook manager pinned in `mise.toml`. The two `test` commands are silent passes; a missing hook exits non-zero.
...
The hook runs `mise run check:core`, the same Docker-free gate used throughout the learner path. A documentation or agent-code commit can therefore receive fast feedback before you install the platform tier.

The full maintainer gate remains `mise run check`. It adds MLflow and platform validation and belongs in CI or after `mise run install:maintainer`. A local hook is an early signal, not proof that every optional surface is ready to release.
