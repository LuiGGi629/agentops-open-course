...
The agent requires Python 3.13, declared as `requires-python = ">=3.13"` in `agents/python/pyproject.toml`. uv provisions a compatible interpreter and creates `agents/python/.venv`; do not depend on the operating system Python, whose version drifts per machine and per distro. The container path pins the interpreter even harder — the Dockerfile builds on `python:3.13.14-slim-trixie` and the Wolfi runtime installs `python-3.13` at an exact apk revision — so the version you develop against and the version you ship are the same major.minor line.
