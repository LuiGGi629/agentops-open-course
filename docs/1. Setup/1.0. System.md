...
Platform chapters add the separate locked MLflow server environment and Kubernetes CLIs with `mise run install:platform`. The optional GKE path adds the pinned Google Cloud CLI and its kubectl authentication plugin with `mise run install:gcp`. Maintainers use `mise run install:maintainer`, which composes all three tiers before full-repository validation. None belongs on the first-hour learner path.
