...
??? note "Deeper: the full control flow, and what the gateway, platform, and gcp tiers add"
    ...
    **Diagram in words:** Every profile verifies its commands and Python environments, reports dotenv state, then runs only its model, container, local-platform, or cloud-specific probes.
    ...
    - **gcp** builds on gateway and adds the Kubernetes render tools, `tofu`, `tflint`, `gcloud`, and `gke-gcloud-auth-plugin`. It requires the Helm plugin, a billing-enabled project, and working Application Default Credentials; it does not require the local-cluster-only tools.
