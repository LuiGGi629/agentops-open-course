...
| Tool trajectory              | `mise run eval`          | Each ADK case requires `1.0` `IN_ORDER`; the wrapper requires at least `25%` overall plus individual skill-loading, restart-confirmation, and resolution-confirmation passes |
...
| Tool trajectory  | `mise run eval`        | 4/15 (27%); three critical passes | evidence + floors | pass                      |
...
- `mise run eval` clears the wrapper's `25%` aggregate floor and its individual skill-loading, restart-confirmation, and resolution-confirmation gates.
