...
| Tool trajectory              | `mise run eval`          | Each ADK case requires `1.0` `IN_ORDER`; the wrapper requires at least `25%` of cases to pass          |
...
| MLflow deterministic scorers | `mise run eval:mlflow`   | Means clear `0.25` / `1.0` / `0.15` / `0.60`; `AGENT_EVAL_MIN_SCORE` raises all four configured floors |

A **scorer** is a function that grades one recorded conversation. [4.4. Evaluations](./4.4.%20Evaluations.md#what-does-the-mlflow-evaluation-add) owns how the four scorer means are enforced and where that command runs.
