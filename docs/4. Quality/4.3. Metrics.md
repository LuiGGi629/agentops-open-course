...
| MLflow deterministic scorers | `mise run eval:mlflow`   | Means clear `0.25` / `1.0` / `0.15` / `0.60`; `AGENT_EVAL_MIN_SCORE` raises lower floors, never lowers one |
...
| Tool trajectory  | `mise run eval`        | 4/15 strict cases (27%)    | evidence + floor  | pass                      |
| Scorer means     | `mise run eval:mlflow` | 0.27 / 1.00 / 0.20 / 0.80  | evidence + floors | pass                      |
...
1. `mise run eval` prints the aggregate strict-case tally; its saved eval-history JSON contains the per-case results.
