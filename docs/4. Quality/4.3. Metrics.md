...
A **scorer** is a function that grades one recorded conversation. The four MLflow scorer means are real gates, not dashboard numbers: if any of them slips below `1.0`, the script exits non-zero and marks the logged model `FAILED`.
...
- `mise run eval` reports `tool_trajectory_avg_score` at `1.0` against the fixed evalset.
