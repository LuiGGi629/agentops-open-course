...
The same six boundaries again, now with the **residual risk** each control leaves — what an attacker can still do once it is in place:
...
| Tool / log / runbook / skill text | Instructions injected back into the model        | `secure_tool_output`: NFKC → neutralize → spotlight → redact          | A novel phrasing can slip the marker list (best-effort)             |
...
```text
# illustrative sample output — the ten test functions expand to 21 parametrized cases
collected 21 items
...
The task is `uv run pytest --no-cov tests/test_security.py`, and that file holds ten test functions, several parametrized further, each pinning one defense. Three of them carry the whole lesson:
...
??? note "Deeper: the other seven cases in the suite"
...
??? note "Deeper: why the license scan needs its own command"

    **SPDX** is the standard vocabulary for naming an open-source license, and Trivy leaves some valid terms unclassified.
    ...
    `trivy.yaml` permits only reviewed OSS license terms — the copyleft runtime licenses plus valid SPDX expressions, legacy metadata labels, and public-domain grants Trivy leaves as UNKNOWN — and every other HIGH/CRITICAL or UNKNOWN term still fails.
...
- `mise run redteam` ends in `21 passed`, with no model, key, or network involved.
- `mise run secure` prints `no leaks found` and a report summary with no counted findings.
- You can name, for each of the six boundaries in the table above, the control that stands there and the residual risk it leaves.
