...
`format:core` may modify files; review the diff afterward. `check:core` runs the docs rules and build, dprint, Python lint/types/audit, shell checks, and workflow lint without intentionally rewriting sources.

`check:core` is the model-, container-, cluster-, and cloud-free half of the gate. It can still use the network because its Python vulnerability check runs `pip-audit`. The root `mise run check` adds `check:infra`, which renders and validates the Kubernetes, Compose, and OpenTofu infrastructure with the tooling you install from Chapter 5 onward.
...
The same standard applies to the dependency audit. The `check:vuln` task in `agents/python/mise.toml` currently runs plain `pip-audit` with no finding-specific ignore. If a future exception is unavoidable, its justification belongs directly beside the task and must name the id, reachability, residual risk, and removal condition. [4.6. Security](./4.6.%20Security.md#what-do-the-scanners-deliberately-not-fail-on) owns that policy.
...
flowchart TD
    ...
    CI["ci.yml (push + pull_request)<br/>install:maintainer"] --> FULL["mise run format + check<br/>core + infrastructure"]
    CI --> TST
...
- **`ci.yml`** runs `install:maintainer`, then the full `format`, `check`, and `test`, followed by `smoke:host`, `redteam`, and `eval:validate`.
