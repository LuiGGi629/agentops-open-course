...
!!! abstract "In one glance"
    ...
    - **You need:** Chapter 3 finished; `mise run doctor:model` and — because that step also starts the gateway container — `mise run doctor:gateway` passing for the browser-approval step at the end.
...
A guardrail is deterministic or separately evaluated policy at an input, model, tool, or output boundary. No single guardrail makes an agent safe. The AgentOps Agent layers model/tool callbacks, typed validation, **least-privilege tool exposure** — each agent holds only the tools its job needs — human confirmation, transactions, and audit evidence. Most of the terms on this page also have a one-line entry in [0.7. Glossary](../0.%20Overview/0.7.%20Glossary.md).
...
| PII redaction             | Model + tool output           | Presidio + pinned spaCy recursively mask personal data             | **On** (unconditional)                          | `test_pii_is_removed_from_nested_untrusted_output`       |
...
| Writes kill-switch        | Guarded write                 | `AGENT_WRITES_DISABLED` refuses all writes before approval         | **Off** — `AGENT_WRITES_DISABLED=false`         | `test_kill_switch_freezes_writes_before_approval`        |
...
Presidio can miss domain-specific identifiers and can over-redact operational data. What it actually recognizes depends on the boundary:

- **At the model boundary**, `redact_pii` runs Presidio's full default recognizer set.
- **At the persistence boundary**, the policy that scrubs an approval rationale before it hits the audit log is deliberately narrower.

`redact_persisted_text` in [`pii.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/pii.py) analyzes only the explicit `_AUDIT_PII_ENTITIES` allowlist: the concrete personal-data classes `EMAIL_ADDRESS`, `PERSON`, `PHONE_NUMBER`, `IP_ADDRESS`, `US_SSN`, `CREDIT_CARD`, and `IBAN_CODE`. It pointedly omits Presidio's broad `ORGANIZATION` recognizer, so domain identifiers such as `INC-002` survive into the audit trail.

On top of it sit three credential tripwires — patterns that fire on anything shaped like a secret — each rewritten to `<SECRET>`:

- Labeled secrets (`api_key=…`, `token:…`, `password=…`).
- `Bearer <token>`.
- Provider key shapes (`ghp_…`, `sk-…`, `AIza…`).

`test_persisted_text_redacts_pii_and_credentials_but_keeps_domain_ids` locks the split: `INC-002` kept, email masked, two secrets redacted.
...
Before it calls a guarded action, the agent must gather the relevant incident/service/runbook evidence and explain the proposal. The course eval cases require those evidence reads before the write call.
...
Retries fix a _flaky_ dependency; they are exactly wrong for a _dead_ one. If a gateway or MCP server is hard down, every read still pays the full `AGENT_MAX_RETRIES` budget — deadline, backoff, deadline — before failing, so a broken dependency turns each turn into a pile of slow, doomed calls.
...
It is **opt-in** — `AGENT_CIRCUIT_BREAKER_ENABLED` defaults to `false`, so the shipped behavior stays retry-only until a builder measures a persistently-failing dependency and turns it on.
...
Set `AGENT_WRITES_DISABLED=true` and every guarded write refuses at once, with no redeploy and no code change.

Human confirmation gates each write one decision at a time. Sometimes you need to stop _all_ of them together: a compromised session, a misbehaving prompt proposing bad remediation, a change window you must not touch. When that kill-switch is set, both guarded actions refuse _before_ the approval flow even runs:
...
1. It short-circuits before any state read, so a frozen agent touches nothing.
...
1. Reads keep working — triage, runbooks, and log search still answer, so the on-call engineer can keep investigating while writes are frozen.

`test_kill_switch_freezes_writes_before_approval` asserts an approved restart _and_ resolve both refuse with the audit-log row count unchanged. Because it is a single environment variable, an incident responder can pull it in seconds. The [7.7. Incident Response](../7.%20Observability/7.7.%20Incident%20Response.md#what-can-you-change-to-mitigate-without-a-redeploy) page lists it first among the mitigations that need no redeploy.
...
!!! warning "Common mistakes"
    ...
    - **Relying on Presidio for domain identifiers.** Presidio recognizes personal-data classes, not `INC-002` or a service slug — and `redact_persisted_text` deliberately keeps domain ids so the audit trail stays useful. Never assume it masks (or protects) your operational identifiers.
...
- The four-file pytest command above passes. It still exits 1 on the repository-wide 95% coverage floor; only the full `mise run test` clears it.
