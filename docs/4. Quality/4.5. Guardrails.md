...
- **Gate that proves completion**: `cd agents/python && uv run pytest tests/test_actions.py tests/test_pii.py tests/test_security.py` passes, and temporarily weakening the guard makes your new test — and only the intended ones — fail. Restore the guard before you commit; the weakening is the experiment, not the deliverable.
