...
Nine tasks cover different layers. Only one is a merge gate; the rest are evidence you run deliberately. Reach for the cheapest one that answers your question:
...
`mise run eval:mlflow` registers the `INSTRUCTION` prompt, links it to a logged model, checks all four deterministic means against those floors, and marks the model `FAILED` when any required metric is missing or lower. A regression therefore exits non-zero instead of logging a false green.
...
flowchart TD
    ...
    Ollama --> Runs["eval · eval:report · eval:workflow · eval:mlflow<br/>eval:cost · eval:ground · no judge"]
    Runs --> Artifacts[upload artifacts, never gates a PR]
```

On every push and pull request, `.github/workflows/ci.yml` runs only model-free gates. The dependency audit may use the network; the two evaluation-related suites named here are fully offline so a regression is a distinct signal, not a line lost in the full run:
