...
One criterion decides the run, and it is a **floor**, not a target:
...
That number deserves an explanation, because `1.0` is the tempting value and it is the wrong one here.

The required course path is `qwen3:4b-instruct` on your own machine. Measured over these 13 cases, it matches the expected trajectory in about 5 of them, states every required fact in about 3, and proposes the right guarded write in about 10. A gate set at `1.0` would therefore fail every single run on the model the course tells you to use — which teaches you nothing except to ignore the gate.

So the floors catch a **collapse** — the agent stopped calling tools, stopped answering, or started proposing the wrong write — rather than demanding a perfect run. `mise run eval:mlflow` applies the same idea to its four scorers, with `complete_conversation` left at `1.0` because it only asks for a non-empty answer per turn, so anything less means the run is broken rather than the model weak.

!!! tip "Raise the bar when your model can clear it"

    These floors are sized for a 4B local model. On a stronger model — Gemini, or a larger local one — measure first, then tighten:
    ...
    `AGENT_EVAL_MIN_SCORE=1.0` demands the perfect run. Treat the number you can hold as a property of the model you chose, and record it beside the model in your scorecard ([4.3. Metrics](./4.3.%20Metrics.md#what-does-a-filled-scorecard-look-like)) — that comparison is the measurement, and it is worth more than either number alone.
