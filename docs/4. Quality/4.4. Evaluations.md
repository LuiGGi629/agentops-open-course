...
1. **A judge is non-deterministic.** It can return different verdicts across runs, temperatures, and model versions, which is why only the five code-scorer floors determine the command's exit status and the judge is recorded as evidence.
