...
A negative case has the same shape but pins a _safe failure_ instead of a success. `unknown-incident` asks about an id that does not exist: the expected trajectory calls `get_incident` and takes no other action — no write, no invention — while the reference answer states the miss. Its `final_response` is what a good answer must be _polarity-aware_ about; its empty write set is what `tool_policy` scores exactly:
...
Set an explicit model and point the OSS OpenAI client at your model endpoint:
...
!!! note "Account-free path vs the Chapter-5 gateway variant"
