...
!!! abstract "In one glance"
    ...
    - **You will:** Create the local k3d cluster and install the kagent control plane, then check its three custom resource types and its controller are ready.
...
The second line varies with your machine: if you later create a `.env`, it reads `env        .env available to explicit live/config tasks` instead. The last line only reports your kubectl context; the doctor never creates a cluster. It fails on a missing tool, a missing project virtualenv, a non-executable gateway wrapper, an unreachable Docker engine, or a missing `helm-diff` plugin. The doctor verifies presence, the pins verify reproducibility.

??? note "Deeper: which tools the platform profile requires"

    The platform profile requires `git`, `uv`, `dprint`, `curl`, `docker`, `jq`, `rg`, `yq`, then the Kubernetes toolchain: `k3d`, `kubectl`, `helm`, `helmfile`, `skaffold`, `kustomize`, `kubeconform`, `kube-linter`, and `agentgateway`. It also confirms both Python virtualenvs exist, that the gateway wrapper is executable and the Docker daemon answers, that the `helm-diff` plugin `3.15.10` is installed (helmfile uses it to diff on apply), and it reports which kube context is currently selected or that the cluster is not created yet. Most of these tools are version-pinned in `mise.toml` (uv, dprint, jq, rg, yq, and the whole Kubernetes toolchain), so `mise run install` provisions the exact versions the tasks were written against; `git`, `curl`, and `docker` are host prerequisites mise does not manage, so the doctor only checks that they are on `PATH`.
...
1. One server node plus one agent node.
1. The k3s image pinned to `v1.35.5-k3s1`.
...
1. One server plus one agent node keeps the lab cheap to schedule.
...
1. `helmfile list` shows both releases at exactly `0.9.11`.
...
- `helmfile -f infra/helmfile.yaml list` shows `kagent-crds` and `kagent` at exactly `0.9.11`.
- The `agentops` namespace exists and holds no workload yet, which is the expected post-install state.

Continue to [6.3. Platform Agents](./6.3.%20Platform%20Agents.md) when the three CRDs are established and the controller is Ready, because the next pages declare custom resources of exactly those types.
