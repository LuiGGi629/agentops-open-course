...
??? note "Deeper: which tools the platform profile requires"
    ...
    The platform profile requires `git`, `uv`, `dprint`, `curl`, `docker`, `jq`, `rg`, `yq`, then the Kubernetes toolchain: `k3d`, `kubectl`, `helm`, `helmfile`, `skaffold`, `kustomize`, `kubeconform`, `kube-linter`, and `agentgateway`. It also confirms both Python virtualenvs exist, that the gateway wrapper is executable and the Docker daemon answers, that the `helm-diff` plugin `3.15.10` is installed (helmfile uses it to diff on apply), and it reports which kube context is currently selected or that the cluster is not created yet. Most of these tools are version-pinned in `mise.toml`, so `mise run install:platform` provisions the exact platform versions the tasks were written against; `git`, `curl`, and `docker` are host prerequisites mise does not manage, so the doctor only checks that they are on `PATH`.
...
!!! danger "Never auto-delete a mismatched shared cluster"
    ...
    First run `k3d cluster list`, `k3d registry list`, and, when the cluster is reachable, `kubectl get namespaces`. Identify who owns the surviving resource and whether another project still uses it. Delete an orphan only when you created it and have proved it carries no other project; never delete the shared `local` cluster merely to make this task green. Chapter 6.6 owns the dedicated-lab teardown. If ownership is unclear, stop and repair the pair with its owners.
...
`skaffold dev` watches the source and repeats the loop after a change. `skaffold run -p local` performs one build-and-deploy pass without watching.
