...
!!! abstract "In one glance"
    ...
    - **You need:** [6.2. Platform Install](./6.2.%20Platform%20Install.md) finished; the lockout drill and the checkpoint need the [6.6. Platform Delivery](./6.6.%20Platform%20Delivery.md) deploy running.
...
The base pins agentgateway `v1.3.1` by image digest and runs it as UID/GID 65532 with a read-only root and dropped capabilities. It mounts the selected config read-only and exposes one **ClusterIP** service (one address reachable only from inside the cluster):
...
Use `http://localhost:3001` for A2A clients. Direct agent port 8080 is reserved for diagnosis. The model listener enforces the demo API key from the `agentgateway-client` Secret, so a forwarded call to `:4000` needs `-H "Authorization: Bearer agentgateway"` ([5.5. Gateway Security](../5.%20Gateway/5.5.%20Gateway%20Security.md)).
...
Exercise: give two namespaces different reachability on the same service, then prove both overlays still render clean.

- **Goal**: `otel-collector-ingress` lists `agentops` and `kagent` in a single rule, so both namespaces reach all three ports — including `8889`, the scrape endpoint only the local overlay's Prometheus uses. Split it into two rules, the shape `agentgateway-ingress` already uses: `agentops` keeps `4317`, `4318`, and `8889`, while `kagent` keeps only the OTLP ports `4317` and `4318`.
- **Files to touch**: `infra/k8s/base/network-policies.yaml` only — both overlays inherit the base, so one edit covers `local` and `gke`.
- **Gate that proves completion**: `mise run check:infra` still exits 0, having rendered both overlays, validated every object with `kubeconform -strict`, linted them with kube-linter, and re-asserted the pinned gateway ingress contract. No cluster and no model are involved. Then misspell one field name as `namespaceSelectors` and rerun it: the same gate fails, naming the object and the rejected property.
