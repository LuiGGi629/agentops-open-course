...
!!! abstract "In one glance"
    ...
    - **You will:** Understand what a cluster adds to an agent you were starting by hand, why kagent declares it instead of you running it, and when a plain Deployment is the better answer.
...
- **Gate that proves completion**: the three commands below, then `git restore infra/k8s` and the same three again — a drill you cannot undo is a mess, not an experiment. Predict each command's result before you run it.

```bash
kubectl kustomize infra/k8s/overlays/local | grep -e qwen3 -e 640Mi
kubectl kustomize infra/k8s/overlays/gke | grep -e gemini -e 640Mi
