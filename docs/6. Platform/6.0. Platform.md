...
!!! abstract "In one glance"
    ...
    - **You will:** Understand what a cluster adds to an agent you were starting by hand, and why kagent declares it instead of you running it.
...
When verifying, expect the pinned stable chart `0.9.11` and the `kagent.dev/v1alpha2` API version.
...
??? note "Deeper: how Skaffold finds the image inside a custom resource"
    ...
    That selector only becomes actionable in [6.6. Platform Delivery](./6.6. Platform Delivery.md), where Skaffold runs.
...
**Skaffold**, the build-and-deploy loop [6.6. Platform Delivery](./6.6. Platform Delivery.md) sets up, selects the overlay with `-p local` or `-p gke` and tags images with the abbreviated Git commit.
...
Continue to [6.1. Containers](./6.1.%20Containers.md) when declaring the agent reads to you as a safer default than starting it yourself.
