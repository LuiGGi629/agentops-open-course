...
!!! abstract "In one glance"
    ...
    - **You need:** [6.2. Platform Install](./6.2.%20Platform%20Install.md) finished; the checks on this page need the [6.6. Platform Delivery](./6.6.%20Platform%20Delivery.md) Skaffold deploy running.
    - **Time:** about 18 minutes, reference.

!!! note "Nothing is deployed yet"

    [6.6. Platform Delivery](./6.6.%20Platform%20Delivery.md) is the page that applies these manifests with Skaffold. In page order, `kubectl -n agentops get agents.kagent.dev` still returns no resources here: that means you are early, not broken.

    Read this page now and come back for the checks, or run the 6.6 Skaffold deploy first and keep this page open beside it.
...
Health polling never creates state. On a fresh volume the A2A startup publishes the database atomically; MCP readiness remains false until that valid database appears on the shared claim.
...
These commands need the Skaffold deploy of [6.6. Platform Delivery](./6.6.%20Platform%20Delivery.md). After it is running:
...
Continue to [6.4. Platform Tools](./6.4.%20Platform%20Tools.md) when the pod you deleted has come back on the same state claim — or, if you are reading ahead of the 6.6 deploy, when you can name the file kagent would rebuild it from.
