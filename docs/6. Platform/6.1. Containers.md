...
```mermaid
flowchart LR
    UV["Stage: uv 0.11.32<br/>/uv /uvx"] --> B1
...
```dockerfile
FROM cgr.dev/chainguard/wolfi-base@sha256:003627df3c1e1bba0c4116afcddb314aca9594ee2328c7e876a8081a6c988b2e AS runtime
```

Renovate bumps the tag and its digest together, so the pins stay current without ever floating. That is a distinct mechanism from the `apk` version pins _inside_ the runtime stage. `apk` is Wolfi's package manager: the `FROM` digest freezes the base filesystem, while `apk add python-3.13=3.13.14-r3` freezes the packages layered on top of it.

Note the two different failure modes. A superseded `FROM` digest still pulls, because registries retain old digests indefinitely. A superseded `apk` pin eventually stops resolving, because Wolfi is a rolling repository — covered next.
...
Every published release image carries one, generated from the exact bytes that were pushed. Nothing on this page asks you to produce an SBOM by hand; CI does it.
...
??? note "Deeper: verify a published release image yourself"
    ...
    cosign verify \
      ...
      --certificate-identity "https://github.com/MLOps-Courses/agentops-open-course/.github/workflows/release.yml@refs/tags/v0.1.1" \
      ghcr.io/mlops-courses/agentops-open-course/agent:v0.1.1
    ...
    A successful verification proves the artifact was built by that workflow at that tag, and the attestation carries its SBOM. When consuming a published image, pin it by the verified digest (`ghcr.io/...@sha256:...`), not the tag — the same tag-versus-digest distinction the `FROM` lines rely on.
