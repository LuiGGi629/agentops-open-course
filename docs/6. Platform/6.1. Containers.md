...
The runtime interpreter here (`python-3.13=3.13.14-r2` from Wolfi) is a different **patch** release from the build stage's `python:3.13.13-slim-trixie`, and that is fine: only the `3.13` **minor** must match, because the venv is portable across patch releases of the same minor and the runtime stage relinks it against its own interpreter. The two pins drift independently — Debian and Wolfi ship patches on their own cadence — so do not expect the digits after the second dot to line up.
