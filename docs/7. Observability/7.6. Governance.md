...
Read the rows the agent wrote. One has to exist first: approve a guarded action through the confirmation flow of [4.5. Guardrails](../3.%20Capabilities/3.1.%20Tools.md#how-does-human-confirmation-work) before you run either command below.
...
- The mutation and its audit row commit in one `BEGIN IMMEDIATE` transaction in `data.py`, so there is no window with a state change and no record — the atomicity [4.5. Guardrails](../3.%20Capabilities/3.1.%20Tools.md#why-are-mutation-and-audit-one-transaction) owns.
