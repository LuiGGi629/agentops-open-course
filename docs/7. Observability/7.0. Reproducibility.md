...
In the scheduled workflow, the capture is bound to the provider/model digest, prompt selection, normalized eval contract, and source revision. The workflow also fixes the Ollama serving window at 8,192 tokens and records it in `model.json`. A local `eval:mlflow` run writes a capture too, but reuse fails closed unless `EVAL_MODEL_DIGEST` and `GITHUB_SHA` identify its model artifact and source revision.
