...
In the scheduled workflow, the capture is bound to the provider/model digest, prompt selection, normalized eval contract, and source revision. A local `eval:mlflow` run also writes a capture, but reuse fails closed unless `EVAL_MODEL_DIGEST` and `GITHUB_SHA` identify its model artifact and source revision.
