...
The scrape path then differs by deployment profile:

1. Both collector profiles expose OTLP/span-derived metrics at `:8889`. The Kubernetes collector also scrapes agentgateway `:15020`, so its endpoint includes gateway metrics.
1. In the host Compose profile, Prometheus scrapes collector `:8889`, MLflow `/metrics`, and gateway `:15020` directly.
1. The local Kubernetes overlay ships its own single-replica Prometheus that scrapes only `otel-collector:8889` and evaluates the same alert rules.
1. The GKE overlay still leaves `:8889` on a ClusterIP for a separately operated Prometheus-compatible scraper.
...
An **error budget** is the share of requests an SLO allows to fail — here, 1% of spans. The SLO alert uses a multiwindow burn rate: both the 5m and 1h error ratios must exceed 14.4 times the 1% budget. A single flaky request therefore does not page, but stopping the model provider does within minutes:
...
!!! info "What is a burn rate?"
    ...
    A burn rate is how fast you are spending an error budget relative to the SLO window. A burn rate of `14.4` against a 30-day 1% error budget means that, at the current error ratio, you would exhaust the whole month's budget in about two days — fast enough to warrant a page. The multiwindow `and` requires the fast 5m window and the slow 1h window to agree before firing, so one flaky request that spikes the 5m ratio never pages on its own, while a genuine outage trips both within minutes.
...
| -------------------------------- | -------- | -------------------------------------------------- | ------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------- |
| `AgentErrorBudgetBurn`           | page     | `agentops_calls_total`                             | multiwindow error-budget burn                                            | 2m     | [error budget](#how-do-i-respond-when-agenterrorbudgetburn-fires)                                       |
