...
- The budget is **per conversation**, not per turn: a single long session cannot spend forever.
- A new session starts the counter at zero. That is the intended reset, and it is also the easy bypass — a client that opens a fresh session per request never hits the ceiling.
