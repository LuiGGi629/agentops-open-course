...
A log line records that something happened; a trace records _how the work was structured in time_. One agent request fans out across eight boundaries:
...
Nothing on this page asks you to switch tracing on. `agent.agent` calls `setup_telemetry()` at module import, so `adk run`, `adk web`, the evals, and the standalone A2A server all share one setup path and cannot drift.
