...
!!! abstract "In one glance"
    ...
    - **You will:** Cause one incident on purpose, walk the signals in order, write the postmortem, and add the cheapest automated evidence that catches it.
...
flowchart TD
    ...
    Which -->|"token/cost blowout"| Baseline["Recommit cost baseline<br/>scheduled eval:cost evidence"]
...
- A cost incident becomes a recommitted **cost baseline** — the per-case token and model-call counts scheduled `eval:cost` compares against.
...
- The foreground agent and host observability stack are stopped.
