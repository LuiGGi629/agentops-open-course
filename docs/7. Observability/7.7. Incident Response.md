...
**The application symptom.** Set `AGENT_MAX_TOKENS_PER_SESSION=1` in the gitignored `.env` at the repository root, then start a session and send two turns:
...
| A read dependency is hard down                                                                    | **Stop paying the full retry budget** on every doomed call.                                                                                                           | `AGENT_CIRCUIT_BREAKER_ENABLED=true`                         | [4.5. Guardrails](../3.%20Capabilities/3.1.%20Tools.md#how-does-the-agent-shed-load-when-a-dependency-stays-down)           |
