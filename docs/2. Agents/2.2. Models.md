...
!!! tip "Try a different model — but measure, do not assume"
    ...
    - **`gemma4:e4b`** — Google's Gemma 4, also Apache-2.0 and ungated, with native function calling plus vision and audio. It is a genuinely strong model. It is **not** a like-for-like swap: despite the "E4B" name meaning ~4.5B *effective* parameters, it carries 8B parameters with embeddings and lands around **9.6 GB** at full precision — roughly four times the 2.5 GB default — or about 2.4× if you pull the quantized `e4b-it-qat` build (~6.1 GB). That is the only reason it is not the default here, and if your machine has the memory it is well worth comparing.
...
Fill this scorecard from your own runs on the same fixed inputs — the baseline column carries the only two numbers this page can cite up front (the published benchmark proxies), and every operational row is yours to measure locally so the comparison is on _this_ agent, not a leaderboard:
