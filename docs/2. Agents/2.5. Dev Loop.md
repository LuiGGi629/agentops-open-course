...
`mise run test` takes well under a minute; it prints `Required test coverage of 95% reached` and ends on a passed count with no failures. `mise run redteam` takes seconds; it replays a fixed set of hostile inputs — path traversal, prompt injection, a write that skips approval — against the same code. Any red line here is a real failure, not a setup gap.
...
`mise run check` is a contributor gate rather than a learner one: it runs the formatting and lockfile checks, the linter, the type checker, and a dependency vulnerability audit. If you changed no code, it should have nothing to say.
