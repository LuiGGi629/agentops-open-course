...
- **Files to touch**: `agents/python/src/agent/composition.py` only. Nothing else, and you revert it at the end with `git checkout -- src/agent/composition.py`.
- **Step 1 — the offline suite**: run `cd agents/python && mise run test`. It stays green. No assertion pins that rule's wording, so the whole 95%-covered suite is blind to its removal. That is the lesson: an offline test proves the agent is assembled, never that it behaves.
- **Step 2 — the eval case**: `recommend-fix` in `agents/python/evals/ops.evalset.json` expects the trajectory `get_incident → search_service_logs → get_runbook`. Run that one case against your model:
...
- **Gate that proves completion**: the case passes on the committed instruction and fails without the rule, because the model stops calling `get_runbook` and the tool trajectory no longer matches. Restore the rule, re-run the same command, and watch it pass again. If it passes both times, say so — a 4B model is not a deterministic oracle, and one flaky case is itself the argument for [4.4. Evaluations](../4.%20Quality/4.4.%20Evaluations.md)'s "you ran it once" warning.
- **Then place it**: the rule you deleted sits in the table above with `—` in the enforcement column. Say in one sentence what a user could have made the agent do with that rule missing, and why the answer is "nothing dangerous, only worse advice".
...
- You completed the required drill above: `mise run test` stayed green with the runbook rule deleted, and you can name the eval case that did not, and the tool it stopped calling.
