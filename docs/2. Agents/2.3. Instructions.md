...
flowchart LR
    ...
    Investigate --> Approve{"write approved?"}
    Approve -->|no| Recommend["recommend or escalate"]
    Approve -->|yes| Act["run guarded action"]
...
- **Files to touch**: `agents/python/src/agent/composition.py` (the `INSTRUCTION` string) and `agents/python/tests/test_smoke.py` (one test asserting your rule text is in `root_agent.instruction`).
- **Gate that proves completion**: `cd agents/python && uv run pytest tests/test_smoke.py -q` passes with your new test, and deleting the rule from `INSTRUCTION` makes that test — and only that test — fail. No model is involved: the test reads the prompt the agent was built with.
