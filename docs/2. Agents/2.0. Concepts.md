...
1. **The model can only propose.** ADK lets a model request any registered tool; it does not itself distinguish a read from a state change. That line is drawn by _your_ `require_confirmation=True` flags and human approval, not by the framework ([4.5. Guardrails](../3.%20Capabilities/3.1.%20Tools.md#how-does-human-confirmation-work)).
