...
This page's job is to show the two boundaries: tools belong to the `Agent`, while cross-cutting policy belongs to the `App`. [4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md#where-is-pii-redacted) explains every hook; token budgeting and cost attribution are [7.3. Costs](../7.%20Observability/7.3.%20Costs.md).
