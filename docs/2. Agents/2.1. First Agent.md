...
This page runs it first and explains it second. That order is deliberate: the wiring makes far more sense once you have watched the thing work. Everything the agent can do is assembled in one module, `agent.py` — the model it calls, the instruction it follows, its tools, and the checks that run around them.

## What is in the dataset the agent reads?

Look at the ground truth before the first run. It is what lets you tell a real answer from an invented one.

Every example in the course references ids from one immutable seed ([`agents/data`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/data/README.md)). Here are the ten incidents and the runbook each one points at:
...
**Three rows are `open`: INC-002, INC-005, and INC-010.** Those are the three ids your first run should return.
...
No `.env` file and no account are needed. The default settings already use `AGENT_MODEL_PROVIDER=openai-compatible`, `AGENT_MODEL=qwen3:4b-instruct`, `OPENAI_BASE_URL=http://127.0.0.1:11434/v1`, and a non-secret local marker.

Start with `List the open incidents`. Here is what the _runtime_ does on a correct first turn: the model asks for one tool, the runtime runs it against the seed, and the answer names only ids the tool returned. `adk run` prints the final `model` text and nothing else, so read the two `tool` lines as the steps behind that answer rather than as terminal output:
...
To watch the tool call itself instead of inferring it from the ids, run `mise run web` rather than `mise run run` and open the **Events** timeline, which shows every model call, tool call, and argument ([2.5. Dev Loop](./2.5.%20Dev%20Loop.md#what-do-adk-web-and-adk-run-actually-do)).
...
- `*ACTION_TOOLS` — the two guarded writes `restart_service` and `resolve_incident`. The tools are [3.1. Tools](../3.%20Capabilities/3.1.%20Tools.md#which-read-tools-does-the-agent-expose); their approval flow is [4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md#how-does-human-confirmation-work).
...
Quoted verbatim from [`agent.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/agent.py). With `AGENT_MCP_URL` unset — the default you just ran — the agent registers six in-process functions: the four incident reads (`ALL_TOOLS`) plus the two runbook lookups (`KNOWLEDGE_TOOLS`).
...
??? note "Deeper: how is the import kept cheap?"
    ...
    The `adk` CLI is given the package path and resolves the conventional `agent.agent.root_agent`. The lazy `__getattr__` imports the ADK-heavy `agent` submodule only when that attribute is first touched, so pure data and MCP modules import without initializing ADK at all.

    Safe import-time setup. `agent.py` does call one function at import, but a safe one:
...
!!! warning "Common mistakes"
    ...
    - **Reading the composition root as behavior.** `agent.py` only _wires_ the object. If you go looking for what a callback does inside `Agent(...)`, you are in the wrong file — every symbol resolves to the sibling page that owns it.
    ...
    - **Assuming the served agent runs the workflow graph.** `mise run run`, `web`, and `a2a` all launch `root_agent`. The `triage_workflow` graph ([3.5. Workflows](../3.%20Capabilities/3.5.%20Workflows.md#how-do-you-actually-run-the-workflow)) is exercised only by its tests and is wired to no serving entrypoint.
