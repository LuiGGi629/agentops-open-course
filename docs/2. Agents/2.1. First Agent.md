...
!!! warning "Common mistakes"
    ...
    - **Assuming policy lives in the prompt.** The six callbacks are the deterministic policy boundary, attached at the runtime edge, not smuggled into `INSTRUCTION`. Approval, budgeting, and PII redaction are code that runs whether or not the model cooperates.
