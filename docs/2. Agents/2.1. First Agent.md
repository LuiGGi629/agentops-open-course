...
The composition is declarative and build-checked against [`agent.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/agent.py): nothing about the agent's behavior is hidden in a base class or a decorator. Read top to bottom it names the model, a stable `name` the runner and telemetry key on, a one-line `description`, the instruction, the tool list, and the callback pipeline. Policy is attached here at the runtime boundary rather than smuggled into the prompt — the six callback slots are the guarded pipeline owned by [4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md#where-is-pii-redacted), which quotes the nested callback region. The rest of this page walks each argument to the sibling page that owns it, so this one object doubles as a map of the whole agent.
...
The six callback slots are the deterministic policy boundary. Order is load-bearing because the lists chain first-non-`None`-wins: the budget check runs first (a refused turn needs neither compaction nor redaction), `compact_history` then trims an over-long window, redaction masks only the surviving messages, and on the return path `record_token_usage` returns `None` so the PII pass still sees every response.
...
A correct first turn looks like this, annotated. The model asks for one tool, the runtime runs it against the seed, and the answer names only ids the tool returned:
...
Those three ids are exactly the `open` rows in the seed table above — `INC-001` and `INC-009` are `investigating`, not `open`, so a correct run leaves them out.

!!! danger "What a wrong run looks like"

    A run that answers `There are open incidents such as INC-042 and INC-101…` **without** a `list_incidents` tool call, or that invents an id absent from the seed, is a grounding failure — the model predicting a plausible-looking incident instead of reading one ([2.2](./2.2.%20Models.md#what-is-a-large-language-model-really)). The tool call is the evidence; no call, no grounding.
...
!!! warning "Common mistakes"
    ...
    - **Reading the composition root as behavior.** `agent.py` only _wires_ the object; nothing about its behavior hides in a base class or decorator. If you go looking for what a callback does inside `Agent(...)`, you are in the wrong file — every symbol resolves to the sibling page that owns it.
    - **Assuming policy lives in the prompt.** The six callback slots are the deterministic policy boundary, attached at the runtime edge, not smuggled into `INSTRUCTION`. Approval, budgeting, and PII redaction are code that runs whether or not the model cooperates.
    - **Assuming the served agent runs the workflow graph.** `mise run run`, `web`, and `a2a` all launch `root_agent`; the `triage_workflow` graph ([3.5](../3.%20Capabilities/3.5.%20Workflows.md#how-do-you-actually-run-the-workflow)) is exercised only by its tests and is not wired into any serving entrypoint.
