...
Open `http://127.0.0.1:8000`, select the AgentOps Agent, and ask `List the open incidents`. This is the first live model request in the course. Stop the server with `Ctrl+C` when you finish.
...
!!! note "The first turn on CPU is slow — that is normal"
    ...
    If a run seems to hang, it is almost always working. Confirm with `ollama ps` or the Ollama server log rather than assuming a failure. The serving window that also governs speed and truncation is covered in [3.4. Memory](../3.%20Capabilities/3.4.%20Memory.md#why-can-a-long-conversation-fail-or-forget-its-start).
