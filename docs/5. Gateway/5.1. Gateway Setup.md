...
!!! abstract "In one glance"
    ...
    - **You need:** Chapters 1-4 finished, Docker running, and `mise run doctor:gateway` plus `mise run doctor:model` passing.
...
The first two commands are not gateway steps. `mise run check` and `mise run test` confirm the repo you cloned still passes its own gates, so skip them if you ran them in Chapter 4. The two doctor profiles are this page's real gate, and they check exactly what the next steps assume and nothing else:
...
??? note "Deeper: how the Linux bridge relay works"
