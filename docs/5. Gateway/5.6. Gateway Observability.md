...
Prometheus and the host gateway share the wrapper-owned Docker bridge. Prometheus scrapes the stable `agentops-gateway:15020` network alias directly, while the wrapper also publishes `15020` on loopback for your host-side `curl`. The native-Linux relay is only for loopback-bound MCP, A2A, and model upstreams ([5.1. Gateway Setup](./5.1.%20Gateway%20Setup.md#how-do-you-start-the-host-data-plane)).
