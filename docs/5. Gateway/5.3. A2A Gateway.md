...
The default A2A path creates no public endpoint, client authentication, TLS, per-tenant authorization, request-size policy, or distributed task store. Port-forwarding/loopback keep the lab private. Chapter [5.5](5.5.%20Gateway%20Security.md) adds an opt-in local authenticated/TLS profile, but it does not turn this into a public edge — and by default it does not propagate the validated caller identity into the action audit; an opt-in header contract ([5.5](5.5.%20Gateway%20Security.md#how-does-the-gateway-verified-identity-reach-the-audit-row)) closes that gap. Adding public exposure before production identity and abuse controls would be a regression.
