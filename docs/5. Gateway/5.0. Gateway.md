...
Before starting a process, compare the three config files. Confirm they expose the same three protocol ports (`3000`/`3001`/`4000`), the same six MCP allowlist entries, the same rate limits and prompt guards, and the OTLP destination appropriate to the environment.
...
Continue to [5.1. Gateway Setup](./5.1.%20Gateway%20Setup.md) when you can say what each of the three ports carries without looking it up.
