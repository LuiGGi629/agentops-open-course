...
The ports below are fixed across the course so every task, probe, and troubleshooting step names the same numbers. The gateway listeners are the front door; the raw upstreams stay off the workstation's routable interfaces; the observability ports surface on loopback under the host Compose profile. Only ports the repository verifiably binds are listed here:
