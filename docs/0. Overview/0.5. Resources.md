...
After the clone in [0.0. Course](./0.0.%20Course.md), run `ls` at the repository root once so the paths below have a shape. Read these in-repo artifacts as first-class references, in rough order of authority:
...
??? note "Deeper: what does `CITATION.cff` record?"
    ...
    It records version `0.1.0`, the author, the repository and site URLs, and both the `MIT` and `CC-BY-4.0` grants; on GitHub, the **Cite this repository** menu exports the current metadata directly. Cite the version you actually used — the reference is a moving target, and `0.1.0` is where the pinned contracts below were captured.
...
Any term you do not recognize is defined in one line in [0.7. Glossary](./0.7.%20Glossary.md). The list is alphabetical and covers every course term — agent, tool, MCP, A2A, RAG, HITL, guardrail, trace, WIF, and the rest. Each entry links back to the section that introduces it, so start there when a term is unfamiliar.
