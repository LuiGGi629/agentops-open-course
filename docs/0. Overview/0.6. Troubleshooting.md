...
| ----------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| A task exits with a "tool not found" error      | `mise install`                                                | [tool not found](#why-does-mise-run-install-or-a-task-fail-with-tool-not-found)       |
...
| Model calls time out or refuse to connect       | `mise run doctor:model`                                       | [reach Ollama or the gateway](#why-cant-the-agent-reach-ollama-or-the-model-gateway)  |
...
1. Run `mise install` once to materialize the pinned toolchain.
...
1. Use `mise x -- <tool>` to run a pinned tool that is not on your `PATH`.
...
This is deliberate. The **dotenv** — the root `.env` file — is loaded only by the configuration and model-backed tasks: `config:check`, `run`, `web`, `a2a`, and the `eval*` tasks. The offline gates (`test`, `check`, `redteam`, `mcp`) load no dotenv at all, so a passing suite proves the committed defaults hold rather than whatever is in your local `.env`.

Confirm the agent's resolved configuration with `mise run config:check` (from `agents/python`), which is one of the tasks that does read `.env`. To exercise a variable inside the offline suite, set it in the test itself or pass it inline for that one command.
...
Run `mise run doctor:model`: Ollama must be reachable and `ollama list` must contain your `AGENT_MODEL`, normally `qwen3:4b-instruct`. Then check that `OPENAI_BASE_URL` matches the path you are on:
...
A persistent failure after bounded retries is a connectivity problem, not a transient blip.
...
Something already owns the port. The stable network contract is fixed (MCP `:3000`, A2A `:3001`, model `:4000`, gateway metrics/readiness `:15020/:15021`, raw MCP `:8000`, raw A2A `:8080`, MLflow `:5000`, Prometheus `:9090`, host Grafana `:3002`).
...
- You can find any of the seven symptoms in the table above and land on the section that fixes it.
