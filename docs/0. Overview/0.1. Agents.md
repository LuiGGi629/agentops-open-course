...
Here is one real turn, grounded in the committed seed (incidents `INC-001`..`INC-010`). It makes the shape of a correct answer concrete:
...
> investigate INC-002
  ...
  → get_service_status(service="inventory")
      inventory: down
  → get_runbook(slug="service-down")

  Agent (final message):
  INC-002 is a SEV1: the inventory service is down, pods are crash-looping, and
  stock lookups return HTTP 503. The service-down runbook's first step is to
  restart the affected service to clear the crash loop.
  Proposed action — restart_service(service="inventory"). Approve to proceed.
  [awaiting human approval]
```

Every claim traces to a tool result, and the diagnosis cites the runbook slug it read — `service-down` is that runbook's short id. The state-changing `restart_service` is only _proposed_: it does not execute until a human approves.
...
The agent in that transcript is the reference **AgentOps Agent**, an on-call assistant for a fictional platform. Its persona and operating rules are not implied by a diagram; they live verbatim in the `INSTRUCTION` string in `agent.py`, kept explicit "so behavior is reproducible and evaluable".

Those rules bind the loop to a specific job: ground every claim in a tool, never invent an incident, service, or status. They also bind it to a specific toolbox. Read tools like `get_incident` only observe. Guarded actions like `restart_service` change things, so the instruction requires the agent to _propose_ them with evidence and call them only after a human approves ([4.5. Guardrails](../4. Quality/4.5. Guardrails.md)).
...
??? note "Deeper: where each ingredient lives in this repository"
    ...
    1. **Policy** — ADK callbacks: validation and error handling from `guardrails.py`, PII redaction from `pii.py`, and the token budget from `budget.py` — all attached to the agent at the runtime boundary rather than buried in the prompt.
...
This repository's real brake is `enforce_token_budget`, a `before_model_callback` — a function ADK runs just before every model call. It refuses the next model call once a session has spent `AGENT_MAX_TOKENS_PER_SESSION` tokens, and returns an actionable message instead of an open-ended bill.

That ceiling is not optional, because the loop is non-deterministic: the same prompt can produce a longer **trajectory** — the sequence of tool calls the agent makes — on the next run. The guard is a property of the system, not of any one input.
...
You have seen the answer; this is the machinery that produced it. Below is the same "investigate INC-002" turn, driven by the operating rules in `INSTRUCTION`. The model decides each tool call, ADK executes it and returns the result, and the loop repeats until the model has enough to answer.
...
First, **the model never runs anything.** It only asks for a tool by name. ADK decides whether to obey, validate, or, for a guarded write, pause for a human approval — the HITL (human-in-the-loop) step at the bottom of the diagram. [2.2. Models](../2. Agents/2.2. Models.md) covers that mechanism in full.
...
Agents are built from a small set of reusable patterns. The shipped agent runs two of these; the rest are demonstrations or absent.

| Pattern                      | Status in the shipped agent   | Where it lives                                                                                                                                                                          |
| ---------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool use                     | Interactive — the served loop | The tool-calling loop; `ALL_TOOLS` in `tools.py` plus the knowledge tools in `memory.py` ([3.1. Tools](../3. Capabilities/3.1. Tools.md))                                               |
| Memory / long-term notes     | Interactive — the served loop | `MEMORY_TOOLS` (`recall_incident_context`, `save_incident_note`) in `memory.py` ([3.4. Memory](../3. Capabilities/3.4. Memory.md))                                                      |
| Fixed workflow graph         | Test-only / demonstration     | `triage_workflow` in `workflow.py`, wired to no entrypoint ([3.5. Workflows](../3. Capabilities/3.5. Workflows.md))                                                                     |
| Multi-agent delegation & A2A | Test-only                     | `delegation.py` coordinator + specialists, in-process ([3.7. Multi-Agent](../3. Capabilities/3.7. Multi-Agent.md)) or across a boundary over [3.6. A2A](../3. Capabilities/3.6. A2A.md) |
| Reflection                   | Not implemented               | No self-review loop in `agent.py`                                                                                                                                                       |
| Planning                     | Not implemented               | No planner in `agent.py`                                                                                                                                                                |

??? note "Deeper: what each pattern does, and why reflection and planning are absent"
    ...
    Two more patterns live in the codebase as demonstrations exercised by tests, not on the default interactive path:
...
The two are not mutually exclusive. The ADK version this repository pins provides a graph-based `Workflow` runtime. The course expresses `triage → diagnose → recommend` as an explicit linear graph, so an evaluation can say _which_ stage failed instead of "the agent was wrong". Reach for model autonomy only where the flexibility pays for its latency, cost, and risk.
...
??? note "Deeper: how the keyword scorer ranks runbooks"

    The scorer is TF-IDF-style, a classic keyword-ranking formula: rarer terms weigh more, a slug match gets a strong boost, and ties break on slug so evals stay reproducible.
