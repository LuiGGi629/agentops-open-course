...
All source paths are under `agents/python/src/agent/`, and the agent-scoped gates (`mise run run`, `mise run eval`, `mise run redteam`) run from `agents/python/`, while repository-wide gates (`mise run test`, `mise run check`) run from the repository root. The point of the table is not to memorize file names; it is that "harden it" and "watch it" are not aspirations here — they are `guardrails.py` and `telemetry.py`, and a red gate tells you when either stops working. Each chapter walks its row in full.
...
Each defense also has an edge the chapter is explicit about: the audit trail is append-only through SQLite triggers but not tamper-proof against an administrator, spotlighting is best-effort defense-in-depth rather than a guarantee, and the token budget bounds one conversation rather than one client. That is why each defense is layered rather than trusted alone.
...
1. **Every change re-runs the same gates.** The continuity is enforced by re-running `mise run format`, `mise run check`, and `mise run test` (≥95% branch coverage) after a change, so the reference cannot silently rot into a broken snapshot.
