...
This course does not teach AgentOps in the abstract. Every claim below is grounded in one completed reference: the **AgentOps Agent** under `agents/python/`. A phase is never a bullet point you have to imagine — it is a file you can open, a command you can run, and a gate that fails when the property breaks.
...
What makes it a discipline rather than a slogan is that every node has an artifact that implements it and a gate that proves it still holds:
...
Each phase also has a command that fails loudly when the phase's property breaks. Most of those commands belong to later chapters, so they are collapsed here rather than dropped:

??? note "Deeper: the command that proves each phase"

    A red gate tells you when a phase stops working. You cannot run most of these yet — read the column as a preview, not a to-do list.

    | Phase        | Gate that proves it                                                                  |
    | ------------ | ------------------------------------------------------------------------------------ |
    | Build        | `mise run run` for an interactive turn; `mise run test` for the offline suite        |
    | Capabilities | `mise run test`, then `mise run eval` scores the tool trajectory over the seed       |
    | Quality      | `mise run test` (≥95% branch coverage), `mise run redteam`, `mise run eval:validate` |
    | Gateway      | `mise run gateway:host` + `mise run smoke:host`, then curl the governed A2A listener |
    | Platform     | `mise run cluster:start` + `mise run platform:install`, then `skaffold dev -p local` |
    | Observe      | `mise run observability:up`, then query Prometheus and read the trace in MLflow      |

    The agent-scoped gates (`mise run run`, `mise run eval`, `mise run redteam`) run from `agents/python/`, while repository-wide gates (`mise run test`, `mise run check`) run from the repository root.
...
| **Actions have consequences**: a tool call can change state          | Mutating tools pause for a human yes (`require_confirmation=True`). On approval they write the state change plus an audit row that can never be updated or deleted, in one SQLite transaction — `actions.py`, schema triggers | [4.5. Guardrails](../4. Quality/4.5. Guardrails.md)                                                  |
| **Cost and latency compound**: every loop step is another model call | Per-session token accounting that blocks the next call after tracked usage reaches `AGENT_MAX_TOKENS_PER_SESSION` — `record_token_usage`/`enforce_token_budget` in `budget.py`                                                | [7.3. Costs](../7. Observability/7.3. Costs.md)                                                      |
...
1. **A bad trace becomes a new gate.** When a trace shows a wrong trajectory or an unsafe proposal, you distil it into one eval case that pins that single behavior. [4.4. Evaluations](../4. Quality/4.4. Evaluations.md) calls this "grow the set from real failures." The next `mise run eval` then fails until the regression is fixed.
...
- You can describe the Observe-to-Build edge concretely: a bad trace becomes one eval case, and `mise run eval` then fails until it is fixed.
