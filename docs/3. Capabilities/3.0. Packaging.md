...
Six surfaces downstream in this course execute the _same_ installed package:
...
Here is the whole Python track: one project directory, with the agent's modules under `src/agent/`.
...
Two of these entries hide a packaging decision rather than a feature. `__init__.py` never imports ADK on a plain `import agent`, and `structured_report/` is a package whose only job is to expose a second discoverable `root_agent`. Both are explained below.
...
`--cov-branch` turns on **branch coverage**: each `if`/`else` direction must be exercised, not only each line. Together with `--cov-fail-under=95`, that means an unreachable or untested code path fails the gate — the src layout is what makes `--cov=agent` point at exactly the code the container will ship.

## Why does importing `agent` not import ADK?

`import agent; agent.root_agent` raises `AttributeError`. The spelling that works is `from agent.agent import root_agent`.

That is deliberate, and the rest of this section is why.

Importing a package should be cheap and free of surprises. ADK is not: building `root_agent` pulls the full runtime and, in this repository, runs `setup_telemetry()` as an import side effect (`agent/agent.py`).

If `import agent` triggered that, the pure data and MCP paths would drag ADK and OpenTelemetry into every process that only wanted a helper. Many tools and the whole MCP server depend on those paths. So `__init__.py` defers the ADK import behind a module-level `__getattr__`:
...
Only accessing `agent.agent` triggers the real import; anything else raises. That is why the intuitive `import agent; agent.root_agent` does **not** work: `root_agent` is not a package attribute, so `__getattr__` rejects it.
...
- After `import agent` and `import agent.mcp_server`, it asserts `'agent.agent' not in sys.modules` and that no warning was emitted.
- It asserts that both ADK CLI discovery paths still resolve the lazy agent: `AgentLoader('src').load_agent('agent')` and `get_root_agent('src/agent')` each return an agent named `agentops_agent`.

The lazy attribute is invisible to callers, but the CLI depends on it.
...
The last one runs an **ASGI** application: Python's asynchronous web-server interface.
...
- Every model-backed task (`run`, `web`, `a2a`, the `eval*` family) loads the repository-root `.env` with `redact = true`, so secrets never print in task logs.
- The offline gates (`check`, `test`) load no dotenv at all.
...
??? note "Deeper: the full container build, stage by stage"
    ...
    Wolfi — the minimal Linux base of the runtime stage — is a _rolling_ repository that removes superseded versions, so these pins periodically stop resolving until a Renovate bump refreshes them: expected drift, documented in the Dockerfile itself, not a broken course.
...
## How would you add a third discoverable entrypoint?

Exercise: give an existing agent its own ADK-discoverable `root_agent`, the way `structured_report/` does.

- **Goal**: make `coordinator_agent` loadable by the ADK CLI without disturbing the primary `agent.agent.root_agent`.
- **Files to touch**: a new package under `agents/python/src/<your-package>/`, beside `agent/` rather than inside it, holding an `__init__.py` and an `agent.py` that re-exports `from agent.delegation import coordinator_agent as root_agent`, plus a new case in `agents/python/tests/test_import_boundaries.py`. The picker lists only directories directly under `src`, which is why this one sits beside `agent/` even though `structured_report/` sits inside it ([3.7. Multi-Agent](./3.7.%20Multi-Agent.md#how-do-you-run-the-coordinator-locally) walks the same recipe).
- **Gate that proves completion**: `cd agents/python && uv run pytest tests/test_import_boundaries.py` passes with a case that loads the new entrypoint in a fresh interpreter and asserts its agent name, and `adk web src` lists it in the agent picker.

The coordinator itself is owned by [3.7. Multi-Agent](./3.7.%20Multi-Agent.md).
...
Expected name: `agentops_agent`. Use `from agent.agent import root_agent` — the shorter `import agent; agent.root_agent` raises `AttributeError` by design, because the lazy `__getattr__` above only ever binds the `agent` submodule.
...
```bash
uv run --project agents/python python -c 'from agent.agent import root_agent; print(root_agent.name)'
```
...
- You can say why `import agent; agent.root_agent` raises `AttributeError`, and which spelling to use instead.
- You can name the two modules that expose a discoverable `root_agent`: `agent/agent.py` and `agent/structured_report/agent.py`.
