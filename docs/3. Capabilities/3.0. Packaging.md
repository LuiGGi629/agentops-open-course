...
The Dockerfile is a three-stage, non-root build (a digest-pinned `uv` image, a build stage, and the runtime stage) whose choices exist to keep the image small, reproducible, and correct about its own metadata. The build context is `agents/`, not `agents/python/`, so both `python/` and `data/` are visible to `COPY`.

```mermaid
flowchart LR
    U["uv image<br/>(digest-pinned)"] --> B
    subgraph B["build stage — python:3.13.13-slim-trixie"]
