...
| `mise run web`         | `AGENT_ENTRYPOINT=agent uv run adk web src`             |
| `mise run mcp`         | `uv run python -m agent.mcp_server` (stdio)             |
...
??? note "Deeper: the pinning decisions behind the two lists"
    ...
    Development dependencies are audited too: `mise run check:vuln` runs plain `pip-audit` over the locked environment with no finding-specific ignore. A dev-only package is not exempt merely because the production image omits it.
