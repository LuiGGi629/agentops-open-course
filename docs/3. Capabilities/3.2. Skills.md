...
1. **Never auto-run** `restart_service` or `resolve_incident`. Explain the target and impact, then wait for explicit approval with an attributable approver.
...
Notice the skill defers the hard invariants to runtime: it _asks_ for approval, and the approval gate in [4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md) is what actually enforces it.
...
One line in the root agent's `INSTRUCTION` closes the loop by teaching the model that skills exist at all, from [`agent.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/agent.py):
...
Every test passes, but this single-file subset still exits non-zero on the project-wide 95% coverage gate that `mise run test` applies to the whole suite, so read the `passed` summary line, not the exit code.
...
- You can explain why the `remediation` skill asks for approval but cannot enforce it.
