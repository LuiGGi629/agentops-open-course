...
!!! abstract "In one glance"
    ...
    - **You will:** Write the order of steps down as a graph instead of hoping the model invents it, and check the shipped triage → diagnose → recommend chain.
    - **You need:** [3.1. Tools](./3.1.%20Tools.md) and [3.4. Memory](./3.4.%20Memory.md) finished — the graph's nodes use those tools.
    - **Time:** about 22 minutes, concept.
...
The distinction that matters is **who decides what happens next**. In the table below, **topology** means the shape of the graph: which steps exist, and in what order.
...
Here, `triage → diagnose → recommend` is not something the model should get to reconsider each run. It is how incident response works. Encoding it as a graph buys three things:

1. The order is visible in code, not inferred from a transcript.
1. Each step gets only the tools it needs.
1. Each step produces its own events and traces, so an evaluation can say _which_ stage failed instead of "the agent was wrong".

One thing to know before you read further: nothing in this repository serves this graph. `triage_workflow` is a teaching artifact. You will assert its shape offline here, and a later section shows the ten lines that drive it yourself if you want to watch it run.
...
These compose: a router whose branches are sequential chains, one of which contains a bounded loop, is an ordinary and healthy design. [3.7. Multi-Agent](./3.7.%20Multi-Agent.md) covers delegation, which is the router pattern with agents as the branches.

One version fact that saves hours: on this stack (`google-adk[a2a]>=2.4.0`) all four shapes are one construct. The `workflow.py` docstring states that `Workflow` is the ADK 2.x graph runtime and "supersedes the classic `SequentialAgent` / `ParallelAgent` / `LoopAgent` (now deprecated) and also expresses parallel, looping, and dynamic DAGs." A **DAG** is a directed graph whose steps never loop back to an earlier step.

!!! warning "Ignore pre-2.0 ADK tutorials"

    Nearly every ADK tutorial you find online predates 2.0 and wires those three deprecated agents; ignore them. You express sequential, parallel, and loop by the _edges_ of a single graph, not by picking a different agent class.
...
`triage_workflow` is three focused agents and one linear edge:
...
That is one tuple, so one path: everything listed after the `"START"` sentinel runs in that order.

Each of `triage`, `diagnose`, and `recommend` is an ordinary `Agent` (an `LlmAgent`) with its own `instruction` and `tools`; the `Workflow` is the only thing that fixes their order. The source remains the version authority — the ADK 2.x `Workflow` API is still evolving, so recheck the edge form when you upgrade `google-adk`.

## How does each node get least privilege?

**Least privilege** means each step holds only the tools it needs, nothing more. The three nodes hold progressively fewer tools, because each needs less than the one before:
...
"the incident chosen by triage" and, in `recommend`, "the diagnosed incident" are the entire contract. This is cheap and flexible, and it is also the pattern's soft spot: if `triage` phrases its pick ambiguously, `diagnose` can pick a different incident and nothing type-checks the mismatch.

When a step's output genuinely must be structured — an id, a severity, a decision — reach for an `output_schema` node: one whose final answer must match a declared shape. `triage_report_agent` in `report.py` is that node, so its handoff is validated rather than merely hoped for. Owned by [4.0. Typing](../4.%20Quality/4.0.%20Typing.md#what-happens-when-the-model-violates-the-schema).
...
The second option is about ten lines, built on `InMemoryRunner`: ADK's runner with in-memory session storage, meant for testing and development. Running it prints one message from each of `triage`, `diagnose`, and `recommend`, in that order.

??? note "Deeper: the ten lines that drive the graph yourself"

    This snippet is illustrative — it drives a live model, so it is not part of the build-checked, model-free test gate — but the imports and names match `workflow.py` and `server.py`:
    ...
    async def main() -> None:
        runner = InMemoryRunner(agent=triage_workflow, app_name="triage-demo")
        session = await runner.session_service.create_session(app_name="triage-demo", user_id="demo")
        prompt = types.Content(role="user", parts=[types.Part(text="Triage the open incidents.")])
        async for event in runner.run_async(user_id="demo", session_id=session.id, new_message=prompt):
    ...
    Run it from `agents/python` with a model backend up (`ollama serve`), and you will see a message from each of `triage`, `diagnose`, and `recommend` in order — the same three nodes the checkpoint below asserts, now actually executing.
...
flowchart LR
    ...
    subgraph root["root_agent, delegation, report"]
      direction TB
      r1["before_model: enforce_token_budget → compact_history → redact_request_pii"]
...
1. **Verify the API first**: the ADK 2.x `Workflow` edge form for fan-out is not the linear tuple used here, and the API is still evolving — read the installed version with `uv run python -c "import google.adk; help(google.adk.Workflow)"` rather than guessing the parallel-edge syntax.
...
That is: the edge runs `triage → diagnose → recommend` from `START`, and each node has a truthy `.model` and `.tools`. Nothing here asserts the callbacks, the per-node tool _sets_, or the absence of action tools — those are claims this page makes from reading the source, not facts the checkpoint enforces.

Closing that gap is a good first extension. Add a test that `recommend.tools == KNOWLEDGE_TOOLS` and that no node's tool names include `restart_service` or `resolve_incident`, then run an eval case over a real triage prompt.

**You are done when:**

- `uv run pytest tests/test_workflow.py` passes both tests from `agents/python`.
- You can name what the suite checks — the `START → triage → diagnose → recommend` edge order, and a truthy `.model` and `.tools` on each node — and what it leaves unchecked.
- You can say why `mise run run`, `mise run web`, and `mise run a2a` never launch this graph.

Continue to [3.6. A2A](./3.6.%20A2A.md) when you can say which decisions this graph took away from the model, and which it deliberately left.
