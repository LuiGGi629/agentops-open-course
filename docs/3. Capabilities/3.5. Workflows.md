...
The second option is about ten lines. This snippet is illustrative — it drives a live model, so it is not part of the build-checked, model-free test gate — but the imports and names match `workflow.py` and `server.py`:
