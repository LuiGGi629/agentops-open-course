...
!!! abstract "In one glance"
    ...
    - **You will:** Split one agent into a coordinator plus two specialists, and see how the tools each one lacks contain a prompt injection.
...
A full triage-to-remediation turn, with both transfers of control visible:
...
The specialist that _can_ act, `remediation_agent`, never sees raw log content, and its two actions still pause for human confirmation with a rationale ([4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md)). The coordinator holds no write tools either. Acting therefore requires an explicit delegation _plus_ an explicit approval — two boundaries a single injected sentence cannot cross.
...
You cannot, out of the box — and that is worth knowing before you go looking for a task.

1. `coordinator_agent` is not the exported `root_agent`, and no `mise` task starts it.
1. ADK CLI discovery (`adk web src`, `adk run`) resolves only the two entrypoints it can load a `root_agent` from: the `agent` package and `agent.structured_report` (through its `agent` submodule, as [3.0. Packaging](./3.0.%20Packaging.md) shows).
1. Today the coordinator's only exerciser is `tests/test_delegation.py`, which asserts the wiring structurally with no model call.
...
Mirror that for the coordinator. Create `agents/python/src/coordinator/agent.py`, a new package beside `agent/`, holding two lines:

```python
from agent.delegation import coordinator_agent as root_agent

__all__ = ["root_agent"]
```

`adk web src` then lists `coordinator` next to `agent` in its agent picker, because that picker offers every package directory directly under `src`. Until you add that entrypoint, the checkpoint below is the way you verify the topology.
...
The shipped defense against a delegation chain running away is a hard ceiling. All three agents carry `enforce_token_budget`/`record_token_usage`, and the totals they keep accumulate across every transferred sub-agent. A chain that keeps hopping between specialists still hits one `AGENT_MAX_TOKENS_PER_SESSION` limit rather than billing without bound.
...
1. **Goal**: add a `verification_agent` that re-checks the affected service after remediation, with `get_service_status` and `search_service_logs`. It reports whether the incident is truly cleared and, like `diagnosis_agent`, holds no write tool. Give it a `description` distinct enough that the coordinator's model routes verification (not diagnosis) to it, and update the coordinator's instruction to delegate a verification pass after `remediation_agent` acts.
1. **Files to touch**: `agents/python/src/agent/delegation.py` (define the agent with `tools=ALL_TOOLS`, attach the same shared callbacks, add it to `coordinator_agent`'s `sub_agents`, extend the coordinator instruction). Then `agents/python/tests/test_delegation.py` (assert the new sub-agent wiring and that `verification_agent` holds none of `_WRITE_TOOLS`).
...
Four tests run, and the summary line reads `4 passed`. Running one file on its own still applies the repository-wide coverage gate from `pyproject.toml` (`--cov-fail-under=95`), so the command also prints a coverage failure and exits non-zero. That gate is calibrated for the whole suite, `mise run test`, not for a single file.

**You are done when:**

- `uv run pytest tests/test_delegation.py` reports `4 passed`, with no model call and no network access.
