...
Exercise: split post-action verification into a dedicated read-only specialist, instead of reusing `diagnosis_agent`.

1. **Goal**: add a `verification_agent` that re-checks the incident and affected service after remediation, with `get_incident`, `get_service_status`, and `search_service_logs`. It compares fresh observations with expected recovery evidence and, like `diagnosis_agent`, holds no write tool. Give it a `description` distinct enough that the coordinator routes post-action verification to it.
