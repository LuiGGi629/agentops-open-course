...
[`agent.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/agent.py) composes the final `tools=[*_read_tools(), *ACTION_TOOLS, *MEMORY_TOOLS, skill_toolset()]`. `_read_tools()` returns `[*ALL_TOOLS, *KNOWLEDGE_TOOLS]` in-process, or a single governed MCP toolset when `AGENT_MCP_URL` is set.
...
Every test passes, but this two-file subset still exits non-zero on the project-wide 95% coverage gate that `mise run test` applies to the whole suite, so read the `passed` summary line, not the exit code.
