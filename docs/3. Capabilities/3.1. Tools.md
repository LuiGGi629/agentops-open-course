...
!!! abstract "In one glance"
    ...
    - **You will:** Read the four read-only tools the agent can call, see how each one rejects bad input at the boundary, follow a guarded write from proposal to its audit row, and run their tests.
    ...
    - **Time:** about 30 minutes, reference.
...
## Your turn: how do you ship a `get_oncall_schedule` read tool?

This is the chapter's required drill, and the chapter checkpoint gates it. Ship a new read-only tool end to end; the existing suite is your oracle.
...
- **Then answer one question**: your tool is a read, so `with_resilience` is safe. What would you have to add instead if it wrote the schedule?
