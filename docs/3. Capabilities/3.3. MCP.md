...
Before reading more theory, start the server. It is a plain HTTP process, so you can exercise its readiness before wiring any model. Start the Streamable HTTP transport, then probe it from a second terminal:
...
Two transports matter here: stdio for a local child process, streamable HTTP for a networked one. `mcp_server.py` defaults to stdio, and `MCP_TRANSPORT` also accepts SSE (Server-Sent Events, an older one-way HTTP streaming mode).
...
??? note "Deeper: hardening and shutdown on the HTTP transport"
    ...
    `mcp_server.py` supports SSE and streamable HTTP over a single FastMCP instance:
...
??? note "Deeper: the probe code, and why a fresh server stays unready"
    ...
    One subtlety worth internalizing: the read-only MCP pod does **not** initialize state. `probe_runtime_database()` reports "not initialized" rather than copying the seed, because the A2A owner publishes the runtime database first ([3.6. A2A](./3.6.%20A2A.md)). A fresh MCP pod therefore stays `unready` until the writer has published state — the correct ordering for a read replica, and `tests/test_mcp.py` asserts the probe never creates the file as a side effect.
...
Exercise: make the injection-surface warning above concrete by wiring a second server and admitting only the tools you have reviewed.
...
- **Files to touch**: a second factory in `agents/python/src/agent/mcp_client.py` that passes `tool_filter=[...]` (ADK `McpToolset` accepts a list of allowed tool names) so only vetted tools are exposed, its composition into `_read_tools()` in `agent.py` behind an explicit setting, and a case in `agents/python/tests/test_mcp.py`.
...
A pass ends with pytest's `passed` summary line and no failures. This two-file subset still exits non-zero on the project-wide 95% coverage gate that `mise run test` applies to the whole suite, so read the summary line, not the exit code. A test that merely opens TCP port 8000 is not an MCP protocol test.

**You are done when:**

- `mise run mcp:http` stays up, and `curl -fsS http://127.0.0.1:8000/livez` returns `{"status":"alive"}` from a second terminal.
