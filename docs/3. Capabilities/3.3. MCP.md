...
- **Files to touch**: a second factory in `agents/python/src/agent/mcp_client.py` that passes `tool_filter=[...]` (ADK `McpToolset` accepts a list of allowed tool names) so only vetted tools are exposed, its composition into `_read_tools()` in `composition.py` behind an explicit setting, and a case in `agents/python/tests/test_mcp.py`.
