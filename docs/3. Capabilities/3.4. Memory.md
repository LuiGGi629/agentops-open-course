...
!!! note "This page has two halves"

    Part A (here through the retrieval checkpoint) covers the five stores and how a runbook is fetched, scored, and evaluated. Part B is the one context window they all compete for — [What is actually inside my agent's context window?](#what-is-actually-inside-my-agents-context-window), [What do I drop first when I run out?](#what-do-i-drop-first-when-i-run-out), [How do you bound the history automatically?](#how-do-you-bound-the-history-automatically), and [Why does my agent silently forget the start of a long conversation?](#why-does-my-agent-silently-forget-the-start-of-a-long-conversation). Storing memory and spending the window that holds it are two different problems.
