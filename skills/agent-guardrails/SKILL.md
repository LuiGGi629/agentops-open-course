...
1. **Ship a kill-switch.** A single flag (e.g. `AGENT_WRITES_DISABLED`) that makes every guarded write refuse before approval, so an incident responder can freeze mutations in seconds while reads keep working.
