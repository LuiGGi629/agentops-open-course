...
# The documentation-gate environment, not a reference agent's: everything this script asks
# Python for (port arbitration, the loopback relay, the deterministic fake model) is
# repository tooling that must keep working whatever language the agent is written in.
readonly repo_python="${repo_root}/.venv/bin/python"
readonly curl_image="curlimages/curl:8.21.0@sha256:7c12af72ceb38b7432ab85e1a265cff6ae58e06f95539d539b654f2cfa64bb13"

[[ -x "${repo_python}" ]] || {
	echo "smoke-host: repository environment is missing; run 'mise run install' first" >&2
	exit 1
}
allocated_ports="$(
	"${repo_python}" - <<'PY'
...
declare -a gateway_environment=(
	...
	"AGENTOPS_GATEWAY_PYTHON=${repo_python}"
...
# `uv run --script` resolves the fake model's PEP 723 dependencies, the same way
# `mise run model:fake` does, so there is one spelling of how that script is started.
(
	cd -- "${repo_root}"
	exec uv run --quiet --script load/fake_model.py --port "${fake_port}"
