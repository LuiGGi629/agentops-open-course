...
Automation behind the `mise run` vocabulary. Nothing here is meant to be run by hand as a first resort: every entry has a task, and the task is what the course, the git hooks, and CI all call.
...
| `doctor.sh`            | `doctor`, `doctor:base\|model\|gateway\|platform\|gcp` | each     | Asserts one prerequisite tier is present, with a one-line remedy per failure.    |
