...
- `cost_eval.py` measures every case, records token and model-call usage in `cost-observed.json`, and compares it to `cost_baseline.json` (regenerated from real measurements with `--update`, so no counts are committed until you measure them). Both documents retain the source revision, prompt selection, normalized eval-contract digest, provider/model/digest, serving context, Ollama version, and sampling temperature that make the counts comparable. A prompt, eval contract, model/runtime identity, unavailable usage metadata, or new case requires a reviewed baseline; tune growth tolerance with `AGENT_COST_TOLERANCE`.
