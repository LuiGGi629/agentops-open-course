...
| ADK trajectory         | `mise run eval`          | Yes             | Strict per-case tools/arguments; 25% aggregate plus skill/restart/resolve cases on the 4B baseline. |
...
- `cost_eval.py` measures every case, records token and model-call usage in `cost-observed.json`, and compares it to `cost_baseline.json` (regenerated from real measurements with `--update`, so no counts are committed until you measure them). Both documents retain the source revision, prompt selection, normalized eval-contract digest, provider/model/digest, serving context, Ollama version, and sampling temperature that make the counts comparable. The source revision identifies where a baseline came from; it may differ on the candidate revision whose drift the baseline is meant to detect. A prompt, eval contract, model/runtime identity, unavailable usage metadata, or new case requires a reviewed baseline; tune growth tolerance with `AGENT_COST_TOLERANCE` (default 0.25).
...
- The scheduled workflow serves Ollama with an explicit 8,192-token window and `temperature=0`, records both beside the model digest and Ollama version in `model.json`, and captures each full-conversation result once in `model-observed.json`. ADK forwards that temperature through `GenerateContentConfig` to the OpenAI-compatible request. Greedy sampling lowers variance but does not make model execution bit-reproducible. The workflow gives the exact transcript to the cost and groundedness checks, and the cost baseline copies and validates both evidence documents' provenance. Running `eval:cost` or `eval:ground` alone still calls the configured model; without `GITHUB_SHA` and `EVAL_MODEL_METADATA_PATH`, its source and serving-runtime provenance is explicitly `null` and therefore weaker local evidence.
